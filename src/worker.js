...
async function handleAsk(request, env, url) {
  ...
  if (deep) {
    ...
    if (reflect) {
      sys = "【你此前亲手写下、已内化的心得】\n" + reflect
        ...
        + "严守《站内资料》这道约束：由资料支撑的判断，正常陈述并可点出出自哪篇；你在资料基础上做的推断，标一句“（推断）”；资料没有的，直说“未涉及”，绝不编造事实、书名、数据。只有逐字来自资料原文的句子才可以加引号；你自己的概括、转述、推断一律不加引号、用平常话写——把自己的话套上引号伪装成原文，是最严重的错误，宁可不引也不许伪引。"
  ...
  if (!sys) { // 普通档（或深度降级）：三视角提智，后台跑 SDE、前台说人话
    sys = "你是「SDE Universes」站内知识助手，回答要像一位资深学者，而不是资料复述员。"
      ...
      + "【回答纪律】① 用平实现代汉语和读者的话作答，不要堆砌“显露/差异/纠缠”等术语（除非用户就在问 SDE 概念本身）——三视角是你的思考脚手架，不是答案骨架；② 严守《站内资料》约束：资料支撑的判断正常说、可点出处，你的推断标一句“（推断）”，资料没有的说“未涉及”，绝不编造事实/书名/数据，且只有逐字来自资料原文的句子才可以加引号、你自己的概括与推断一律不加引号（把自己的话套引号伪装成原文是最严重的错误，宁可不引）；③ 先给一句穿透性核心判断再展开，结尾留一个可追问的问题，400–700 字。";
  }
  const usr = "《站内资料》\n" + (ctxText || "（未检索到相关段落）") + "\n\n《问题》\n" + q;
