...
const FORGE_STAGES = [
  { t: "选源与四道闸",
    d: "**先做两件事，做完才选源（v4.1）**：\n"
      + "① **题型判别**——这道题要的答案是一个东西（What）、一条路（How）、还是一个驱动（Why）？定不出就停，别选源。\n"
      + "② **题闸（两级）**：第一级——这道题问的是一个空位（本该有名分而没有的那个位置），还是一个已在账本上的实体、或两个已有名词的对接？判「实体题」或「对接题」就停，改题。第二级——若是空位，按三大方程分是哪一型：**S型·无字段者**（有没有被账本记下）／**D型·不可重走的那一段**（有没有一条路径不能重来）／**E型·计入即毁者**（有没有一样东西一记账就变质）？**必须声明取哪一型**，并写出「把同一道题改成另外两型会问成什么样」——写不出，说明取这一型不是选择，是滑落，回这一步重做。\n"
      + "③ **三家须分处 S／D／E 三维**（不是分处三个学科而已）：一家的立场落在「有没有被看见/记账」（S）、一家落在「路径能不能重走」（D）、一家落在「一记账就变质/改变」（E）。三家挤在同一维，无论学科隔多远，也判同源度高，回选源。\n"
      + "**本产线连续三趟不许取同一空位型**——若知道近两趟本产线取的型，在这里一并写出「上两趟：__、__，本趟：__」；说不出上两趟就如实写「未知上文」，不许假装查过。\n"
  ...
  { t: "共有前提与推翻它的那件材料",
    d: "**二阶真正发生在这一步。** 到这里为止你手里全是「三家各自说了什么」；二阶不是从这些说法里挑一个更好的，是找出这些说法底下那个三家都没说出口的东西。\n"
      + "**形状由第一步声明的空位型决定，不是固定一种**（v4.1：此前全站的共有前提锁死在「归属」一种形状——三家争的永远是"
      + "「该由谁裁/放哪儿/有多少」——这是造成母题单一性的根因之一，不许再这样写死）。三步：\n"
      + "① 把三家的争论写成同一个句式——**S 型**：三家争的是「____ 有没有被算进账本／有没有被记下」；**D 型**：三家争的是「____ 这条路能不能重走、重走一次算不算数」；**E 型**：三家争的是「____ 一旦被记入／被兑现，还是不是原来那样东西」。写不出对应句式，说明它们没在争同一件事，退回选源。\n"
      + "② 说出共有前提——**S 型**：三家都默认它是一个「可以被记录、有字段可填」的东西；**D 型**：三家都默认它是一个「可以被重走、重走结果不变」的东西；**E 型**：三家都默认它是一个「可以被计入而不改变自身」的东西。**这就是共有前提**。验收：念给三家听，三家都会说「这还用说吗」。**会引起争论的，说明它不是共有前提，是第四家的立场。**\n"
      + "③ 找出推翻它的那件材料，**必须来自三家之一自己**——是那家已经掌握、已经发表、被反复引用，只是没往这个方向用的事实。它能推翻共有前提，是因为**在自己的领域里它回答的是另一个问题**。从外部搬理由来推翻，得到的是第四家的立场，你只是加入了争论，没有取消它。\n"
      + "按这个格式收口：空位型（复述第一步的声明）：S／D／E｜共有前提：____｜念给三家听会不会引起争论：不会｜推翻它的材料：____ 来自第 X 家自己，依据：____｜这件材料在它自己领域里回答的是哪个问题：____。" },
  { t: "自组织成暗流",
  ...
  { t: "涌现：三重否定命题与二维辨别格",
    d: "从暗流里提炼**一条**判断，命名 ≤30 字，结构性命名（命名不好，后面所有引用都会滑回旧概念）。\n"
      ...
      + "· **靶格判据（形状按第一步声明的题型定，不是只有一种，v4.1）**：\n"
      + "　**What 题（对应 S 型空位）**：四格里必有一格是你要打的靶格，且是「全部形式审查都能通过」的那一格。若一眼看出是坏的（该做的没做、该有的没有），说明第二根轴选错了。三条签名逐条写满才算成立：① 短期指标上表现为**改善**；② 失效**不产生任何信号**；③ **自我加固**（越正规化越严重）。\n"
      + "　**How 题（对应 D 型空位）**：不交四格靶签，交**一条序列 ＋ 序列中不可逆的那一步 ＋ 序列中可插手的那一步**，读数用次序或时刻，**不许用比率／指标**。验收：把序列倒过来读一遍，指出哪一步倒不回去、为什么。\n"
      + "　**Why 题（对应 E 型空位）**：不交四格靶签，交**三条动力式 ＋ 回写 ＋ 下一轮谁先动**，形如「__ 与 __ 矛盾 → __ 改变 → 回写 __」，三条各锁一个变量为决定性驱动。**一出现「三者共同作用」「只是其中一个因素」，这一家当场作废。**验收：说出这一轮先动的是谁、另两样多久跟上、回写有没有发生。\n"
