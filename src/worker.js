  ...
  if (deep) {
    ...
    // ===== 模式：最终综合提炼（涌现流水线末环，也是整条产线最要紧的一步）=====
      ...
      sys = (neigong ? neigong + "\n\n═══════════\n【你此前带着上面这套完整底盘先验、亲手写下并已内化的心得】\n" + (reflect || "（心得暂缺）") + "\n\n═══════════\n" : "")
        ...
        + "\n① 先把这场问对里所有**一阶产物**捞出来：每一个新命名、每一句「其实它是……」、每一处「原来这两件事不是一回事」——那些就是候选金点子。一场好问对通常散着三到八个，分布在各轮里，不会自己站出来；漏掉它们，这份资料就退化成会议纪要。"
        ...
        + "\n⑥ 【工序在后台走，栏目里只留结果】这份资料下一步会被「成文一篇」读去写投稿论文，那篇论文的正文不许出现内部环节词。所以栏目内容里的承重命题、分离点与判据一律用**目标学科的母语**写（已改姓），不要把「S 维度／D 维度／三视角／逮先验」这类词写进栏目正文；工序名只在交代「这一条是怎么来的」时用一句话带过。"
