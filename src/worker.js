    ...
    // ===== 模式：最终综合提炼（涌现流水线末环，也是整条产线最要紧的一步）=====
      ...
      if (mode === "polish") {
          ...
          + "\n\n【抬 I 的唯一办法】把核心命题压到 50 字以内，问它能不能被某个已有概念一比一替换。能替换，就说明真正的增量还没被说出来——这时不要去改措辞，要去找那个「只有本文预测得到、而那个已有概念预测不到」的具体场景，把它写成正文里的一节。【抬 F 的唯一办法】两条彼此独立、分属不同检验路径的证伪条款，每条都带可观测代理（怎么测、测什么、多少算数、多大差异才算数），其中至少一条是现在就能跑的；并且必须在正文里交出你这次真跑的结果——查了哪些国家、哪段历史、哪类已发表统计，命中了什么，或到什么范围为止仍未命中。只描述检索式、然后断言「检索必然落空」，F 一分不涨。";
        {
          ...
          const PN = { 1: "第一段", 2: "第二段", 3: "第三段", 4: "第四段", 5: "第五段" };
          const PE = { 1: "〔第一段完·待续〕", 2: "〔第二段完·待续〕", 3: "〔第三段完·待续〕", 4: "〔第四段完·待续〕", 5: "〔全文完〕" };
          const PMAX = Object.keys(PN).length;
          ...
          sys = pbase
            + "本稿分" + PMAX + "段打磨，本次改【" + PN[P] + "】（共" + PMAX + "段，改后全文仍约两万字）。"
        ...
        const PART_NAME = { 1: "第一段", 2: "第二段", 3: "第三段", 4: "第四段", 5: "第五段" };
        const PART_MAX = Object.keys(PART_NAME).length;
        ...
        const PART_SPEC = {
          ...
          5: "本段交付（以下每一项都是必交项，一项不落；本段是全篇的交付件，不再展开新论证）：【十、结论】（收口前删净「唯一变量／这段对话本身就证明了它」式自封）；"
           ...
           + "本段约 3200–4200 字即可，不必凑满五千：交付件写全比写长要紧。"
        };
        sys = base
          + "本篇分" + PART_MAX + "段连续写成，本次写【" + PART_NAME[P] + "】（共" + PART_MAX + "段，两万字）。"
