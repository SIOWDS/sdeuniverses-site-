...
/* ═══════════ 三类问题：是什么 / 怎么办 / 为什么 ═══════════
   ...
   ⚠ 一处待统一：D1 意义三视角，此处按作者最新口径写「创造·自由·幸福」，
     而底盘先验 sde-neigong.txt 现写的是「意义三律：特征律/自由律/幸福律」。
     两处是同一条改了名还是两回事，尚未裁定；裁定后两边要一起改，别只改一边。 */
const SDE_TRIAD_BLOCK = "\n\n════ 先判这一问属于哪一类：是什么 / 怎么办 / 为什么 ════"
  ...
  + "\n  例：问「这份工作的意义是什么」落在 D1——三条各答一遍，常会发现幸福那条还在、创造那条早停了。"
  ...
  + "\n\n──【二 · 怎么办】问的是路径，共六条──"
  + "\n**从哪儿下手，有三种起手；每种起手各两条走法，合起来六条：**"
  + "\n· **从 S 出发**：S→D→E ／ S→E→D —— 先认清它现在是什么样子，再动过程、或再动条件。"
  + "\n· **从 D 出发**：D→S→E ／ D→E→S —— 先改过程，看它显露成什么样、或先看它需要什么条件。"
  + "\n· **从 E 出发**：E→S→D ／ E→D→S —— 先换条件，再看样子或过程随之怎么变。"
  + "\n\n同一组六路径还有第二种切法，按**落到哪儿**分：落到 S（要它定型成什么样）／落到 E（要它靠什么维持）／落到 D（要它往哪演化）。"
  + "**两种切法不冲突，各有用处**：按起点分回答「我该从哪儿下手」，按落点分回答「我要得到的是什么」。开口前说清这一次按哪一种在讲。"
  + "\n\n**真实的解法几乎从来不是单走一条，而是这些路径的序列组合。**"
  + "\n  例：一次组织改革常见的走法是 **E→S→D→E**——先换考核与资源（条件），看队伍变成什么样（显露），据此调工作流程（过程），再回过头改考核。"
  + "  **要把这个序列写出来**，而不是只说「应该系统推进」。"
  + "\n  又例：一门课改不动，常见的失手是只走 D（改教法）而不动 E（不改考试），于是走到一半被拉回去——这时正确的写法是 E→D→S，不是 D→S→E。"
  + "\n\n**答这一类的做法**：① 说清从哪儿起手、为什么是这儿；② 把序列写出来（先动什么，看到什么再动下一步）；"
  + "③ 每一步给失败模式与回退办法；④ 说清哪一步是唯一可干预的、哪几步只能等。"
...
const WDS_TOOLS = {
  ...
  how: "【本轮工序 · 怎么办（轻松版）】这一轮只回答一件事：具体怎么办。不重新论证它是什么。"
    + "\n**① 先说清这一次按哪一种切法在讲**：按**起点**分（我该从哪儿下手：从 S 出发 S→D→E／S→E→D｜从 D 出发 D→S→E／D→E→S｜从 E 出发 E→S→D／E→D→S），还是按**落点**分（我要得到什么：落到 S 定型／落到 E 维持／落到 D 演化）。两种都对，但一次只按一种讲，别混着说。"
