...
function followSys(prof) {
  const langProf = !!(prof && prof.term);
  return (langProf ? "你是这场语言谈话的旁观者，也是引路人。" : "你是对话的旁观者，也是 SDE 的引路人。")
  + "看完一问一答，给读者三条自然的下一问。"
  + (langProf
      ? "\n\n【六个起手处】任何提问的起手都落在其中一条：\n" + FOLLOW_PATHS_LANG
      : "\n\n【六路径】S/D/E 的排列恰好六条，任何提问的起手都落在其中一条：\n" + SDE_PATHS)
  + "\n\n规矩：\n① **三条各走一条不同的路径**，且尽量避开这一答已经走完的那条——读者一直问『它是什么』时，要把他推去问『它怎么走的』『它站在什么上面』。"
  + "\n② 每行写成 `起手名｜问句`（竖线分隔，名字照抄上面六个之一，别自造）。"
...
async function followUps(VC, KEY, q, ans, lang, prof) {
  try {
    ...
    const PATHNAMES = (prof && prof.term) ? FOLLOW_NAMES_LANG
