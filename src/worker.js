  ...
  if (deep) {
    ...
    if (mode === "paper") {
      ...
      const base = (neigong
        ...
        + "\n\n【出稿前自检规程 v1 —— 逐条硬性执行，任一条不过关即返工重写，不得出稿】"
      ...
      if (part === 1) {
        sys = base + "本次写【上半篇】：第一行只写论文题目（不加书名号、不加任何前缀），空一行后依次写【摘要】（280–350 字，含核心判断与方法路线）、【关键词】（4–6 个，用「；」分隔）、【一、引言】（问题、既有解释＝把 2-3 个敌意最近邻在此指名请进来、其中至少一位须是自检规程（一）要求的外文占位者、本文的增量）、【二】（代理坍缩：逐个拆开最近邻握着的代理变量、指出与本文判断的分离点）与【三】（提取控制变量 Z，承重命题立成「X 不是 Y₁也不是 Y₂而是 Z」）两个论证章节。写满约 4800–5400 字，在第三章末尾的自然节点停笔，最后单独一行输出：〔上半篇完·待续〕";
        usrOverride = "《站内资料》\n" + (ctxText.slice(0, 22000) || "（未检索到相关段落）")
          + "\n\n《读者的问题》\n" + q
          + "\n\n《你此前的问对回答（思考底稿——成文必须超越它，不许扩写复读）》\n" + (seed || "（无）");
      } else {
        sys = base + "本次写【下半篇】：从《上半篇结尾》停笔处无缝续写（不重复已写内容，不重写题目与摘要），完成【四】（第二轴强制：把控制变量 Z 撞一条结构独立的第二轴、升成二维辨别格）（及必要时的【五】）论证章节、【可裁决判据】（一张会让敌意最近邻预测相反的 2×2 或等价判别式＋一个可观测代理，写进正文）、【证伪条件】（至少两条彼此独立、分属不同检验路径的证伪条款，写进正文而非附注）、【结语】（收口前删净『唯一变量／这段对话本身就证明了它』式自封）、【参考文献】（站内来源列「篇名 — URL」；站外只列你能确证存在的经典著作与作者，绝不编页码、不编引文）。【结语】与【参考文献】是必交项：若篇幅吃紧，优先压缩前面论证章节的字数，也必须把结语与参考文献完整写完；绝不允许在半句话中途停笔。写满约 4800–5400 字，参考文献之后单独一行输出：〔全文完〕";
