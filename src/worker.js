...
async function askCore(request, env, url, body, SINK) {
  ...
  const part = (body.part >= 1 && body.part <= 4) ? (body.part | 0) : 1;
  ...
  if (deep) {
    ...
    // ===== 模式：最终综合提炼（涌现流水线末环，也是整条产线最要紧的一步）=====
      ...
      /* 【四段两万字，不是一段三千字 —— 2026-08-10 改】
         入口资料是整条产线的枢纽：论文水平主要由它决定，而它原来只有两三千字，
         等于让一份三千字的清单去支撑两万字的论文，成文那一步只能靠基底现编补上差额。
         为什么同样是四段：单次调用有 ~120 秒的平台时钟上限，5000 字/段是线上真跑量出来的安全区，
         字数只能靠加段数（见本文件下方 WDS_TOK 那段：加长单段必被杀在思考阶段、正文 0 字）。
         ⚠ 九栏的栏名与栏号一个字都不许改：成文（paper 的 briefKind==="distill" 分支）按栏名＋栏号发指令，
            前端「思想库存」也按栏名取料，改名即全线取空。字数加在每一栏内部的密度上，不是加栏。 */
      MAXTOK = 32000;
      const P = (part >= 1 && part <= 4) ? part : 1;
      const BRIEF_NAME = { 1: "第一段", 2: "第二段", 3: "第三段", 4: "第四段" };
      const BRIEF_END = { 1: "〔第一段完·待续〕", 2: "〔第二段完·待续〕", 3: "〔第三段完·待续〕", 4: "〔全文完〕" };
      const BRIEF_SPEC = {
        1: "本段交付〔一〕〔二〕〔三〕三栏，栏标题原样照抄。\n"
          + "一、缘起之问与行进轨迹（约 1800 字）——先原样写出缘起之问，再逐轮各写一段：这一轮把问题推到了哪儿（不是这一轮讲了什么，是它把问题往前推了多远）、靠哪一句推动的、推完之后问题的形状变成了什么。某一轮原地打转就直说原地打转并指出卡在哪。末尾一段：整场问对整体上从哪儿走到了哪儿，哪一轮是真正的转折。\n"
          + "二、已经立住的核心判断（约 1800 字）——5 到 8 条，按承重程度排序。每条先用一句陈述句写出判断本身，再写三小段：出自第几轮、由什么撑着（论证／例子／直觉，据实写）、它现在最薄的一处在哪。只是说得漂亮而没有支撑的，照列但标明属于「提到但没论证的」。\n"
          + "三、候选承重命题 X（约 1600 字）——给出 2 到 3 个候选，每个写成一句可以被反驳的陈述句（能被人指着说「这句话错了」的那种）。每个候选后附三件：压到 50 字以内的压缩版；一句自答「它能不能被某个已有概念一比一替换」；它若成立会强制推翻什么。最后判一句：本场最该拿去成文的是哪一个、为什么。若还没长出可反驳的那种句子，就写出最接近的半成品并指出它差在哪一步。",
        2: "本段只交付〔四、反复被触到的分离点〕这一栏，约 5200–6200 字，栏标题原样照抄。这是整份资料最要紧的一栏，也是论文里可裁决判据的唯一原料，所以单独占一段篇幅写透。\n"
          + "列出 6 到 10 条「现有解释在这里失灵」的位置，每条 500–700 字，逐条按同一格式写五件：\n"
          + "（a）失灵现场——对话里的哪一处、第几轮，用一两句把那个具体现象说清楚；\n"
          + "（b）触发条件——在什么条件下通行的解释开始说不通；条件必须写成可以被指认的形态，不许写「在复杂情况下」这类话；\n"
          + "（c）谁在这里失灵——点名那一家解释、写出它握着的代理变量，并替它写一段最可能的辩护词（它会怎样把这个现象强行收编）；\n"
          + "（d）可裁决草案——试写一句「若⟨条件⟩，本文预测 A，而⟨它⟩预测 B」；写不出就写明卡在哪一环；\n"
          + "（e）离可观测还差什么——要测什么、多少算数、现成材料里有没有。\n"
          + "末尾一段把这 6 到 10 条排序：哪几条最便宜、现在就能查，哪几条要等条件。",
        3: "本段只交付〔五、敌意最近邻清单〕这一栏，约 5200–6200 字，栏标题原样照抄。分两组写。\n"
          + "第一组〔已在对话中点到的〕：逐位写，每位 250–350 字——他握着的代理变量、他解释到哪一步为止、他会怎样解释这里的同一个现象、以及他手上还有哪张牌你们没碰。凡对话里被当成已被击倒的靶子的人，这里必须把他最强的那个版本补出来。\n"
          + "第二组〔应当交手而尚未交手的〕：按题材与按机制各补 3 到 5 位，共 6 到 10 位，每位 300–450 字，写清五件：原名与原题（外文占位者必须给出外文原题与原作者姓名，不许只给中译名）；他命名的是什么；他会怎样解释你们这个现象（写成他自己的话）；一句可裁决差异草案（当⟨条件⟩时他预测 A、本文预测 B）；请他进正文的具体位置（第几章）。至少两位不在承重命题所属的那门学科里，至少一位是外文占位者。\n"
          + "末尾一段自查：这批人是不是全落在同一门学科、是不是全是最容易被击倒的那几位。若是，就直说本场还没找到真正占着这块地的人，并写出下一步该往哪个学科找。",
        4: "本段交付〔六〕〔七〕〔八〕〔九〕四栏，合计约 5200–6200 字，栏标题原样照抄，四栏一栏不许省。\n"
          + "六、尚未解决的张力与前后不一致（约 1800 字）——逐条列出对话里自己打架的地方、被绕过去的反例、含糊其辞的接口。每条写清：出自第几轮、两边各说了什么、为什么它们不能同时为真、论文里可能的三条出路（各附一句代价）。不许写「基本一致」。\n"
          + "七、可裁决判据的线索（约 1400 字）——从第四栏挑出最便宜、最可观测的 2 到 3 条，逐条写成完整形态「若⟨条件⟩，本文预测 A，而⟨某最近邻⟩预测 B」，各配一段可观测代理（怎么测、测什么、多少算数、多大差异才算数）与一段误诊阻挡（哪些情形读数相同却不算数）。其中至少一条标明「现在就能跑」，并写出跑它需要的现成材料。\n"
          + "八、经验材料清单（约 1200 字）——对话中出现过的每一个数据、案例、事实逐条列出，各标来源等级〔一手来源〕〔站内自引〕〔未核验〕，并各写一句「它在论文里能承担什么、不能承担什么」。标为后两种的，在论文里不得当作已证实的事实去支撑被解释项。若本场几乎没有经验材料，就把这件事本身写清楚，并列出成文时最该补的三类材料。\n"
          + "九、明确不写什么（约 800 字）——这篇论文的边界：哪些相邻问题不在射程内、为什么；再列出三条明知诱人但本次不碰的岔路，各附一句不碰的理由。"
      };
      sys = (neigong ? neigong + "\n\n═══════════\n【你此前带着上面这套完整底盘先验、亲手写下并已内化的心得】\n" + (reflect || "（心得暂缺）") + "\n\n═══════════\n" : "")
        ...
        + "③ 不写客套、不写导语、不做总结陈词，直接从本段该写的第一栏开始；"
        + "④ 全文不用 Markdown 标记（不写 #、不写 **、不画表格、不写 --- 分隔线）；"
        + "⑤ 哪一栏在这场问对里确实没有产出，就写「本场问对未产出」并接着写清它缺在哪、要补上需要什么，不许拿话填满；"
        + "⑥ **这是入口资料不是论文**——它是一张清单和一份作业单，不是可以直接扩写成文的初稿；谁把它当参考答案抄一遍，谁就停在一阶；"
        + "⑦ 【篇幅靠密度不靠注水】每一条都要能被追到出处（第几轮），并各带一句「它在论文里承担什么职能」；凡是把已说过的话换个说法再说一遍的段落一律不许写。宁可条目多、每条短而实，也不许条目少而每条注水——注水的入口资料比短的入口资料更坏，因为它会把成文那一步一起带偏；"
        + "⑧ **栏标题原样照抄，一个字不许改、栏号不许重排**：下游的成文与库存都按栏名取料，改名即取空。"
        + "\n\n本份资料分四段连续写成（合计约两万字），本次写【" + BRIEF_NAME[P] + "】。"
        + (P === 1 ? "" : "先从《已写部分·结尾》停笔处无缝续写：不重复已写内容、不重写前面几栏、栏号顺着往下编。若那最后一句断在半句（末尾不是句号、问号、感叹号或右引号），第一件事是把它补成完整句子再往下写。")
        + "\n" + BRIEF_SPEC[P]
        + "\n写满约 5000–6000 字，在一个完整句子的句号（或问号、感叹号、右引号）之后停笔——不许停在半句话、逗号、顿号，也不许停在一个只写了栏标题的空栏上。最后单独一行输出：" + BRIEF_END[P];
  ...
  // 【时钟必须早于平台，而且要早到"兜底还跑得完"】
  // 实测平台在约 128–133 秒把整个请求无声杀掉（连 [DONE] 都没有）——那一刀之后什么保险都没用了，
  // 零正文兜底根本没机会跑。所以：
  //   · 深度档问答：75 秒掐断。掐了之后关思考重跑一遍只要二三十秒，仍在墙内，**答案照样交得出来**；
  //   · 成文／打磨／提炼／盲评：这些本来就要跑八九十秒（实测 72–99 秒），闸设在 115 秒，
  //     只比平台早一点点——它们的兜底本就赶不上，闸的意义是把"无声"换成"一句能读的话"。
    ...
    if (url.pathname === "/api/wds/distill") {
      ...
      const SPEC = {
        ...
        paper: { name: "一万字论文", tok: WDS_TOK_MAX, spec:
          ...
          + "· 用 Markdown，主标题 #，各节 ##。约一万字——写不到一万字就如实写到哪儿，**不要靠复述凑字数**。\n"
          + "· 只输出这篇论文本身，前后不要任何说明。" },
