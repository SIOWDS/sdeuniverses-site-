              ...
              if (dStage === "plan") {
                ...
                try {
                  if (ancPick(plan.ancestors.join(" ; ")).n < 8) {
                    const _asys = BASE + "\n\n【只做一件事】把已经占着这块地的人点出来，只输出一个 JSON 数组，前后不要任何说明、不要代码围栏。"
                      + "\n格式：[\"姓 年份 —— 他手里的那一个判断，一句话\", …]，十条以内。"
                      ...
                      + "\n⚠ 先把这个现象在不同传统里的**三个不同叫法**各想一遍再列——只按自己习惯的那个词去想，"
                      + "一定会漏掉隔壁那一行里早已把话说完的人。";
                    const _ausr = "题目：" + String(plan.title || "")
                      ...
                      + "\n\n现在只输出那个 JSON 数组。";
                    const _aj = looseJSON(await llmText({ url: VC.url, model: VC.model, name: VC.name }, KEY, _asys, _ausr, 1800, 25000));
