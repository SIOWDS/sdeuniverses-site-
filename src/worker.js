...
// ── DECK_TPL：六套「PPT 模板」。一套模板 ＝ 页面骨架 ＋ 写作纪律 ＋ 视觉主题，三件绑死。
//    为什么必须绑死：只给一份通用写作要求、再靠版式去猜，产出必然是十页一样的要点页
//    （2026-07-30 读者那份 12 页里 9 页落在普通要点页，就是这么来的）。
//    模板一定，这一场"该有哪几页、每页该给什么形状的内容"就定了，基底照着写，版式自然对得上。
const DECK_TPL = {
  brief: {
    name: "工作汇报", theme: "slate", pages: "9–12",
    spec:
      "【本套模板：工作汇报】听众是要做决定的人，最关心「结论是什么、凭什么、我要批什么」。\n"
      + "页面骨架（按此顺序，缺一页不可）：\n"
      + "① 封面。② 一句话结论——**只写一条要点**，就是最承重那句判断（会自动放大成一句话页）。\n"
      + "③ 问题是什么：3–4 条要点，每条是判断句不是现象罗列。\n"
      + "④ 关键数字：**必须写成 3 条 `- 数字 ｜ 这个数字是什么`**（自动出大数字卡片）；对话里若无数字，改写成「三个可核验的事实」同样用竖线格式。\n"
      + "⑤ 证据页：若对话里有可比较的数值，**必须插一个 ```chart``` 图表块**（bar 比大小 / line 看趋势）。\n"
      + "⑥ 对比页：`- 现状说法 ｜ 我们的说法` 成对写 2–3 条，标题里带「对比」。\n"
      + "⑦ 风险与边界：写清什么情况下这套判断会失效——**不许只报喜**。\n"
      + "⑧ 下一步：末页标题写「下一步」，3–4 条可动手的动作，每条带责任范围或时限。\n"
      + "语气：短句、先说结论后说理由；不用形容词堆砌；每页讲稿写「站上会怎么说这一页」。",
  },
  teach: {
    name: "教学讲义", theme: "forest", pages: "10–14",
    spec:
      "【本套模板：教学讲义】听众是学习者，目标是让他学会一件事，不是让他佩服你。\n"
      + "页面骨架：\n"
      + "① 封面。② 学完能做什么：3 条 `- 动词开头的能力`。\n"
      + "③ 一个概念：只讲一个核心概念，**只写一条要点**（会放大成一句话页）。\n"
      + "④ 常见误解：`- 多数人以为 ｜ 实际上` 成对写 3 条，标题里带「对比」。\n"
      + "⑤ 怎么发生的：标题里带「流程」或「步骤」，3–4 条 `- 步骤名 ｜ 一句说明`。\n"
      + "⑥ 例子页：一个具体到能复述的例子，3 条要点。\n"
      + "⑦ 辨别格（可选）：标题带「四格辨别」，正好四条 `- 格名 ｜ 说明`。\n"
      + "⑧ 自测：3 条 `- 问题` ——读者答不上来就说明没学会。\n"
      + "⑨ 一句话小结（只写一条要点）。⑩ 末页「下一步」：给两三个可练的动作。\n"
      + "语气：口语、举例子、不用学派术语；讲稿写「讲到这里要停下来问什么」。",
  },
  pitch: {
    name: "路演提案", theme: "clay", pages: "9–12",
    spec:
      "【本套模板：路演提案】听众在判断「值不值得投入」，注意力只有前三页。\n"
      + "页面骨架：\n"
      + "① 封面：主标题是主张不是名称。② 痛在哪：3 条，每条写清谁在痛、痛的代价。\n"
      + "③ 一句话洞见（只写一条要点）：别人没看见而我们看见的那件事。\n"
      + "④ 方案：标题带「流程」或「步骤」，3–4 条 `- 步骤 ｜ 说明`。\n"
      + "⑤ 凭什么是我们：`- 通行做法 ｜ 我们的做法` 成对 3 条，标题带「对比」。\n"
      + "⑥ 证据：3 条 `- 数字 ｜ 含义`；有可比数值时**必须插 ```chart``` 图表块**。\n"
      + "⑦ 什么情况下不成立：诚实写出失败条件——这一页反而最能取信。\n"
      + "⑧ 末页「下一步」：明确要什么（资源/决定/时间），每条一句。\n"
      + "语气：短、具体、可核验；禁止「赋能/闭环/生态」这类空词。",
  },
  research: {
    name: "研究汇报", theme: "ink", pages: "10–14",
    spec:
      "【本套模板：研究汇报】听众是同行，会追问方法与反例。\n"
      + "页面骨架：\n"
      + "① 封面。② 研究问题（只写一条要点，写成可裁决的问句或命题）。\n"
      + "③ 已有说法与它的缺口：`- 现有说法 ｜ 缺口` 成对 2–3 条，标题带「对比」。\n"
      + "④ 方法：标题带「流程」，3–4 条 `- 步骤 ｜ 怎么做`。\n"
      + "⑤ 发现：**有数值必须出 ```chart```**；无数值则 3 条 `- 数字/事实 ｜ 含义`。\n"
      + "⑥ 辨别格：标题带「四格辨别」，四条 `- 格名 ｜ 说明`（把发现放进二维里）。\n"
      + "⑦ 最近邻：`- 谁已经说过类似的 ｜ 本文与他的分离线` 成对写 2–3 条，标题带「对比」。\n"
      + "⑧ 证伪条件：什么观察出现就说明本结论错——至少两条，其中一条现在能跑。\n"
      + "⑨ 结论（只写一条要点）。⑩ 末页「下一步」。\n"
      + "语气：克制、不夸大、区分「已验证」与「仍是假设」。",
  },
  review: {
    name: "复盘总结", theme: "plum", pages: "9–12",
    spec:
      "【本套模板：复盘总结】目的不是回顾，是找出下次该改什么。\n"
      + "页面骨架：\n"
      + "① 封面。② 原定目标：3 条。\n"
      + "③ 实际结果：3 条 `- 数字 ｜ 说明`；有可比数值**必须出 ```chart```**（计划 vs 实际）。\n"
      + "④ 时间线：标题带「阶段」，3–5 条 `- 时间 ｜ 发生了什么`。\n"
      + "⑤ 差距归因：标题带「四格辨别」，四条 `- 归因类别 ｜ 具体事例`。\n"
      + "⑥ 做对了什么 / 做错了什么：`- 做对 ｜ 做错` 成对 3 条，标题带「对比」。\n"
      + "⑦ 学到的一句话（只写一条要点）。⑧ 末页「下一步」：3–4 条具体改动，写清谁在什么时候改什么。\n"
      + "语气：对事不对人；不许把失败归因于「重视不够」这类空话。",
  },
  talk: {
    name: "观点演讲", theme: "night", pages: "8–11",
    spec:
      "【本套模板：观点演讲】听众来听一个观点，不是来听汇报。节奏比信息量重要。\n"
      + "页面骨架：\n"
      + "① 封面：主标题就是那句反直觉的主张。\n"
      + "② 常识是什么（只写一条要点）。③ 常识为什么不成立：3 条要点。\n"
      + "④ 过渡页：只写 `## 一、` 加章节名，不写要点。\n"
      + "⑤⑥⑦ 三页证据：每页 3 条要点；其中至少一页写成 `- 数字 ｜ 含义`，有数值则出 ```chart```。\n"
      + "⑧ 引文页：第一条用「」包住一句原话，第二条写出处。\n"
      + "⑨ 最强的反对意见与我的回应：`- 反对 ｜ 回应` 成对 2 条，标题带「对比」。\n"
      + "⑩ 末页「下一步」：留给听众的一个动作，不超过 3 条。\n"
      + "语气：有节奏、敢下判断、每页只推进一步；讲稿写「这里停顿几秒」。",
  },
};
  ...
  if (deep) {
    ...
    if (mode === "collide") {
      ...
      sys = neigong
        ...
        + "\n\n【不许做的三件事】① 不许在三个观点里挑一个当结论（那是择优不是碰撞）；② 不许把三个观点调和成一个更周全的说法（周全＝张力被抹平＝没有涌现）；③ 不许编造书名、页码、引文与数据。全文 1400–2000 字。";
    ...
    // ===== 模式：最终综合提炼（涌现流水线末环，也是整条产线最要紧的一步）=====
      ...
      sys = neigong
        ...
        + "\n\n【纪律】① 只写前面材料里真出现过的东西，第六栏〔尚未交手〕那一栏是唯一例外（那一栏就是要你补出来的）；② 每一条都要能被追到出处（第几轮、哪个典范、评分卡哪一条）；③ 区分「已经立住的」「提到但没论证的」「还得补的」，不许把后两类写得像第一类；④ 某一栏确实没长出来，就写「本场未产出」，不许凑；⑤ **这是入口资料不是论文**——它是一张清单和一份作业单，不是可以直接扩写成文的初稿；谁把它当参考答案抄一遍，谁就停在一阶。全文 2600–3600 字。";
      usrOverride = "《缘起之问》\n" + (String(body.origin || q).slice(0, 300))
        + "\n\n《整场问对（历轮观点与落点）》\n" + (histTxt || "（缺）")
        + "\n\n《胜出的典范》\n" + (winner || "（缺）")
        + "\n\n《落选的两个典范》\n" + (others || "（缺）")
      ...
      sys = (neigong ? neigong + "\n\n═══════════\n【你此前带着上面这套完整底盘先验、亲手写下并已内化的心得】\n" + (reflect || "（心得暂缺）") + "\n\n═══════════\n" : "")
        ...
        + "\n\n硬性纪律：① 只写这场问对里确实出现过的内容，一个字也不许编——但「应当交手而尚未交手的最近邻」这一栏例外，那一栏本来就是要你补的清单；② 严格区分三种东西：**问对里已经立住的**、**问对里提到但没论证的**、**论文还必须补上的**，每一条都要标明属于哪一种；③ 不写客套、不写导语、不做总结陈词，直接从第一栏开始；④ 全文不用 Markdown 标记（不写 #、不写 **、不画表格）；⑤ 哪一栏在这场问对里确实没有产出，就写「本场问对未产出」，不许拿话填满。"
        + "\n\n按下列九栏输出，栏标题原样照抄：\n"
        + "一、缘起之问与行进轨迹——每轮一行，写这一轮把问题推到了哪儿（不是这一轮讲了什么，是它把问题往前推了多远）；末尾一句：这十轮整体上是从哪儿走到了哪儿。\n"
        + "二、已经立住的核心判断——3 到 5 条，每条一句陈述句，按承重程度排序；每条后面用括号注明它出自第几轮。\n"
        + "三、候选承重命题 X——写成一句可以被反驳的陈述句（能被人指着说「这句话错了」的那种）；若这场问对还没长出这样一句，就写出最接近的半成品并指出它差在哪。\n"
        + "四、反复被触到的分离点——列出对话里那些「现有解释在这里失灵」的位置，每条写清触发条件：在什么条件下，通行的解释开始说不通。这是论文里可裁决判据的原料，最要紧的一栏。\n"
        + "五、敌意最近邻清单——分两组：〔已在对话中点到的〕逐个写出它握着的代理变量；〔应当交手而尚未交手的〕按题材与按机制各补三到五位，外文占位者必须给出原题原名，并逐位写一句「它会怎样解释同一个现象」。\n"
        + "六、尚未解决的张力与前后不一致——对话里自己打架的地方、被绕过去的反例、含糊其辞的接口，逐条列出并注明出自第几轮。论文必须正面处理这些，不许装作没有。\n"
        + "七、可裁决判据的线索——从第四栏里挑出最便宜、最可观测的一到两条，写成「若⟨条件⟩，本文预测 A，而⟨某最近邻⟩预测 B」的形态。\n"
        + "八、经验材料清单——对话中出现过的每一个数据、案例、事实，逐条标注来源等级：〔一手来源〕〔站内自引〕〔未核验〕。标为后两种的，在论文里不得当作已证实的事实使用。\n"
        + "九、明确不写什么——这篇论文的边界：哪些相邻问题不在射程内，为什么。\n";
      usrOverride = "《站内资料》\n" + (ctxText.slice(0, 14000) || "（未检索到相关段落）")
        + "\n\n《缘起之问》\n" + originQ
      ...
      const brief = String(body.brief || "").trim().slice(0, 9000);   // 十轮问对提炼出的《论文入口资料》
      ...
      const base = (neigong
          ...
          : "")
        + (brief
            ? "你是一位以 SDE 方法论为隐性引擎的资深学者。你与读者刚刚完成了一场连续多轮的问对，并已把它提炼成一份《论文入口资料》；现在读者点击了「成文一篇」，你要把这一整场问对推进成一篇独立的学术论文（全文目标约一万字，分上下两半各约五千字写成）。"
      ...
      if (mode === "polish") {
        ...
        if (part === 1) {
          sys = pbase + "本次改【上半篇】：输出改后的上半篇全文（不是修改说明、不是清单、不是差异对照，就是可以直接排版的正文）。第一行只写论文题目（沿用原题；除非评分卡指出标题里的比喻没落地，那就换一个不欠债的题目），空一行后依次给出【摘要】【关键词】【一、引言】【二、…】【三、…】，写满约 5200–6200 字，在第三章末尾一个完整句子的句号之后停笔，绝不许停在半句话或只写了标题的空章上。最后单独一行输出：〔上半篇完·待续〕";
          usrOverride = "《评分卡（上一轮独立盲评的结果，本次打磨的作业单）》\n" + (card || "（无评分卡：按目标五维自查一遍再改）")
            + "\n\n《读者的原初问题》\n" + q
            + "\n\n《待打磨原稿》\n" + (orig.slice(0, 24000) || "（缺）")
            + "\n\n《站内资料（补文献与最近邻用）》\n" + ctxText.slice(0, 12000);
        } else {
          const head = String(body.head || "").slice(0, 1200);
          const tail = String(body.tail || "").slice(0, 1100);
          sys = pbase + "本次改【下半篇】：从《改后上半篇·结尾》停笔处无缝续写（不重复已写内容、不重写题目与摘要）。若那句停在半句，第一件事是把它补完。依次完成【四、…】（第二轴强制：把控制变量 Z 撞一条结构独立的第二轴、升成二维辨别格）与必要时的【五、…】、【可裁决判据】（会让敌意最近邻预测相反的判别式＋可观测代理，写进正文）、【证伪条件】（至少两条彼此独立、分属不同检验路径，各带可观测代理，其中一条现在就能跑并当场交出执行结果）、【结语】、【参考文献】。结语与参考文献是必交项，篇幅吃紧就压缩前面的论证章节，也必须完整写完。写满约 5200–6200 字，参考文献之后单独一行输出：〔全文完〕";
          usrOverride = "《评分卡（本次打磨的作业单）》\n" + (card || "（无评分卡）")
            + "\n\n《读者的原初问题》\n" + q
            + "\n\n《改后上半篇·题目与摘要》\n" + (head || "（缺）")
            + "\n\n《改后上半篇·结尾（你的续写起点）》\n" + (tail || "（缺）")
            + "\n\n《原稿的下半部分（写对的地方沿用，失分的地方按评分卡改）》\n" + (orig.slice(-16000) || "（缺）")
            + "\n\n《站内资料（补文献与最近邻用）》\n" + ctxText.slice(0, 10000);
        }
      }
      else if (part === 1) {
        sys = base + "本次写【上半篇】：第一行只写论文题目（不加书名号、不加任何前缀），空一行后依次写【摘要】（280–350 字，含核心判断与方法路线）、【关键词】（4–6 个，用「；」分隔）、【一、引言】（问题、既有解释＝把 2-3 个敌意最近邻在此指名请进来、其中至少一位须是自检规程（一）要求的外文占位者、本文的增量）、【二】（代理坍缩：逐个拆开最近邻握着的代理变量、指出与本文判断的分离点）与【三】（提取控制变量 Z，承重命题立成「X 不是 Y₁也不是 Y₂而是 Z」）两个论证章节。写满约 4800–5400 字，在第三章末尾的自然节点停笔——停笔处必须是一个完整句子的句号（或问号、感叹号、右引号）之后，绝不许停在半句话、逗号、顿号，也不许停在一个只写了标题的空章上。最后单独一行输出：〔上半篇完·待续〕";
        usrOverride = "《站内资料》\n" + (ctxText.slice(0, 22000) || "（未检索到相关段落）")
          + "\n\n《缘起之问》\n" + originQ
          + (qlist ? "\n\n《这场问对走过的路（问题清单）》\n" + qlist : "")
          + (brief ? "\n\n《论文入口资料（十轮问对的提炼——这是清单，逐条兑现）》\n" + brief : "")
          + "\n\n《你此前的问对回答（思考底稿——成文必须超越它，不许扩写复读）》\n" + (seed || "（无）");
      } else {
        sys = base + "本次写【下半篇】：从《上半篇结尾》停笔处无缝续写（不重复已写内容，不重写题目与摘要）。先检查《上半篇·结尾》的最后一句是否完整：若它断在半句（末尾不是句号、问号、感叹号或右引号），你的第一件事就是把那半句补成一个完整句子，再往下写，绝不允许直接另起标题、把断句丢在那里，完成【四】（第二轴强制：把控制变量 Z 撞一条结构独立的第二轴、升成二维辨别格）（及必要时的【五】）论证章节、【可裁决判据】（一张会让敌意最近邻预测相反的 2×2 或等价判别式＋一个可观测代理，写进正文）、【证伪条件】（至少两条彼此独立、分属不同检验路径的证伪条款，写进正文而非附注）、【结语】（收口前删净『唯一变量／这段对话本身就证明了它』式自封）、【参考文献】（站内来源列「篇名 — URL」；站外只列你能确证存在的经典著作与作者，绝不编页码、不编引文）。【结语】与【参考文献】是必交项：若篇幅吃紧，优先压缩前面论证章节的字数，也必须把结语与参考文献完整写完；绝不允许在半句话中途停笔。写满约 4800–5400 字，参考文献之后单独一行输出：〔全文完〕";
        usrOverride = "《站内资料》\n" + (ctxText.slice(0, 18000) || "（未检索到相关段落）")
          + "\n\n《缘起之问》\n" + originQ
          + (brief ? "\n\n《论文入口资料（下半篇要兑现的仍是同一份清单：第四栏的分离点、第五栏未交手的最近邻、第六栏的张力）》\n" + brief.slice(0, 5000) : "")
          + "\n\n《上半篇·题目与摘要》\n" + (head || "（缺）")
          + "\n\n《上半篇·结尾（你的续写起点）》\n" + (tail || "（缺）")
