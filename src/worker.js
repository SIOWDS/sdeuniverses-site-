    ...
    if (url.pathname === "/api/wds/distill") {
      ...
      const SPEC = {
        ...
        novel: { name: "长篇小说（12万字·四部）", tok: 12000, noHead: 1, parts: 16, world: 1, threads: 1, timeline: 1, fixed: [
            ...
            { h: "一部·起手", words: 6000, ask: "【第一部·旧规矩】本部第一趟：先写一行 `# 书名`，再写一行 `## 第一部　旧规矩还在`，然后进正文。从事情已经开始的地方进去（S-1）：不交代前情、不介绍身份。让主角所处的那件『大家都守着的规矩』自己转起来——**绝不许把规矩说出来，只演它**；让读者看见主角在意什么。" },
          ...
          + "不写题记、不写作者的话、不出现任何议论段。第一行是书名（`# 书名`）。只输出小说本身，前后不要任何说明。" },
          ...
          async start(controller) {
            ...
            try {
              ...
              const ssys = BASE + NBRB + writerBlock(styleId)   // 分节各趟也要带风格，否则前后两节像两个人写的
                ...
                + (NOHD
                    ? ((partIdx === 0
                        ? "\n· 第一行写这篇的标题（单独一行，不加「标题：」二字），然后直接进正文。"
