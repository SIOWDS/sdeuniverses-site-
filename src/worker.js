...
async function askCore(request, env, url, body, SINK) {
  ...
  if (histTxt && mode === "answer") {
    sys += "\n\n【这是同一场连续问对的第 " + roundNo + " 轮（最多十轮，之后可一键提炼成《论文入口资料》）】"
      ...
      + "\n⑥ **正文写完，另起一行交一个结账块**（四行，字段名一字不改）：〔第 " + roundNo + " 轮·结〕／落点：…／走法：补了反例还是切了差异还是逼到边界／"
      + "欠账：这一轮没解决或绕过去的那一处（**不许写「无」**）／下一问：踩着上面哪一个具体说法往下追（一句，不超过 60 字）。"
      + "整块 150–250 字。它不是总结，是把这一轮记成一笔账——后面几轮与「提炼精华」都直接取用它，所以宁可写得干、不许写得漂亮。";
  }
