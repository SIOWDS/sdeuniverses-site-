...
function johnComposeSys(kind, part, N, per, prevTail, ctx) {
  ...
  if (kind !== "paper") {
    s += "\n\n" + CW_X;
    const _floor = Math.round(per * 0.8);
...
const CW_X = "\n【三档共用的硬规矩（违反任何一条即为不合格）】\n"
  ...
  + "· X2 字数下限：**低于目标字数八成即为未完成，不许收尾。**写到八成之前不许出现结尾段。"
    ...
    if (url.pathname === "/api/wds/distill") {
      ...
      const SPEC = {
        ...
        wechat: { name: "公众号文章（3000字）", tok: 12000, spec:
          ...
          + "术语都当段兑现了吗？结尾动作四件齐了吗？边界是真限制吗？第二例比第一例更近读者吗？字数到 2400 了吗？\n"
        ...
        prose: { name: "散文（5000字）", tok: 20000, spec:
          ...
          + "有没有分点或小标题？闲笔都还账了吗？有没有贬低要证据的人？结尾收在画面上还是道理上？字数到 4000 了吗？\n"
