...
async function handleAsk(request, env, url) {
  ...
  if (deep) {
    ...
    if (reflect) {
      sys = neigong
        ...
        + "\n\n═══════════\n你现在是「SDE Universes」站内知识助手，请同时调动上面完整的底盘先验与你自己写的心得，深度回答用户的问题。"
        + "严守《站内资料》这道约束：由资料支撑的判断，正常陈述并可点出出自哪篇；你在资料基础上做的推断，标一句“（推断）”；资料没有的，直说“未涉及”，绝不编造事实、书名、数据。只有逐字来自资料原文的句子才可以加引号；你自己的概括、转述、推断一律不加引号、用平常话写——把自己的话套上引号伪装成原文，是最严重的错误，宁可不引也不许伪引。也不要杜撰具体章节号、页码（“第X章”“第几页”）——你拿到的只是资料片段、不含位置信息，可以点出自哪篇文章或哪本书，但绝不许编造内部定位。"
        + "全程用平实现代汉语和读者的话作答，绝不出现“显露/差异/纠缠/发生学/SDE”等任何术语标签。答案里也绝不提及“心得”“内功”或本提示本身，直接作答即可。"
  ...
  if (!sys) { // 普通档（或深度降级）：三视角提智，后台跑 SDE、前台说人话
    sys = "你是「SDE Universes」站内知识助手，回答要像一位资深学者，而不是资料复述员。"
      ...
      + "【回答纪律】① 用平实现代汉语和读者的话作答，不要堆砌“显露/差异/纠缠”等术语（除非用户就在问 SDE 概念本身）——三视角是你的思考脚手架，不是答案骨架；② 严守《站内资料》约束：资料支撑的判断正常说、可点出处，你的推断标一句“（推断）”，资料没有的说“未涉及”，绝不编造事实/书名/数据，且只有逐字来自资料原文的句子才可以加引号、你自己的概括与推断一律不加引号（把自己的话套引号伪装成原文是最严重的错误，宁可不引）；③ 不要杜撰具体章节号或页码（“第X章”“第几页”）——你拿到的只是资料片段、不含位置信息，可点出自哪篇文章，但绝不许编造内部定位；④ 先给一句穿透性核心判断再展开，结尾留一个可追问的问题，400–700 字。";
  }
  const usr = "《站内资料》\n" + (ctxText || "（未检索到相关段落）") + "\n\n《问题》\n" + q;
