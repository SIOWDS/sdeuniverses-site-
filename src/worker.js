...
async function handleAsk(request, env, url) {
  ...
  const _MODES = { recommend: 1, paper: 1, iq: 1, polish: 1, distill: 1 };
  ...
  const deep = body.deep === true || mode === "paper" || mode === "polish" || mode === "distill";
  const K = mode === "recommend" ? 48 : (mode === "distill" ? 40 : (deep ? 120 : 20));              // 取多少块（深度档广撒网；retrieve 只收相关块、clamp 兜底，窄问题不会被噪声塞满）
  const CTX_MAX = mode === "distill" ? 14000 : (deep ? 50000 : 12000);   // 《站内资料》字数上限
  // 检索用问句：连续问对时把「缘起之问」并进去做锚——第七轮问「那这一条呢」这种
  // 指代式短问，单独拿去召回只会漂走；提炼档则用全场问题一起定向。
  const rq = mode === "distill"
