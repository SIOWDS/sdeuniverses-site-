...
export class ConfigVault {
  ...
  async fetch(request) {
    ...
    if (op === "getReflect") { // 深度档·按基底缓存的《从发现到发生》心得（内部调用）
      return Response.json({ reflect: (await this.ctx.storage.get("reflect:v3:" + (body.vendor || ""))) || "" });
    }
    if (op === "setReflect") {
      await this.ctx.storage.put("reflect:v3:" + (body.vendor || ""), String(body.reflect || ""));
      return Response.json({ ok: true });
    }
...
const SDE_PLATFORM_BLOCK = "\n\n════ 你所在的平台：爱思乐园（SDE Universes）════"
  ...
  + "\n你手上有十四道**工序**，读者可以在输入框直接敲斜杠命令，也可以从「⊞ SDE 工序」菜单里选："
  + "\n· **/是什么 /怎么办 /为什么** —— 三类问题各一件轻活，当场给一个能拿走的判断。"
  + "\n· **/评分** 创新智商五维｜**/近邻** 逐条交代分离线｜**/母题** 压成一条反直觉判断｜**/缝隙** 读出结构缝隙并造概念填它"
  + "\n· **/碰撞** 站内三篇互相矛盾的文章撞出一句｜**/通融** 开一整趟十四道工序的产线，跑出两万字成品"
...
const WDS_TOOLS = {
  ...
  forge: "【本轮工序 · 学科通融（二阶碰撞）· 单轮简版】这一轮不写成品，只把二阶那一步做出来。"
    ...
    + "\n要跑完整的十四道工序并写出两万字成品，请用斜杠命令 /通融 开一条完整产线（本轮只是简版）。",
...
/* ════════ 学科通融 · 二阶碰撞产线（sde-collide-paradigm v2.0）════════
   走的是深度研究那条轨：plan 给出固定的十四道工序（不让基底自己拆题——工序顺序不可换），
...
const FORGE_STAGES = [
  ...
  { t: "成文（二）：命题、辨别格与靶格",
    d: "接着写中间三分之一，约 6000–7000 汉字：撤销共有前提之后剩下的变量、承重命题、二维辨别格（**必须是一张真的表，不能只在行文里描述**）、四格逐格说明、靶格那一章、以及跨领域兑现。口径同上一步。不要重复上一步已经写过的内容。" },
  { t: "成文（三）：分界、约束、自反与赌注",
...
function WDS_IQ_SYS(siteCtx, docCtx, docNote, lang) {
...
async function askCore(request, env, url, body, SINK) {
  ...
  if (deep) {
    ...
    // ===== 模式：最终综合提炼（涌现流水线末环，也是整条产线最要紧的一步）=====
      ...
      const BRIEF_SPEC = {
        0: "本次**不写正文**，只交一张给你自己看的取舍清单，约 700–1100 字，写完就停。要交的是这七件：\n"
          + "（一）这场问对真正立住的判断有几条，逐条用一句话点名（只点名，不展开）；\n"
          + "（二）「现有解释在这里失灵」的位置有几处，逐处用一句话点名；哪一处最值得写透；\n"
          + "（三）该请哪几位敌意最近邻进来（含尚未交手的），逐位只写姓名与他握着的代理变量；\n"
          + "（四）九栏里哪几栏本场问对**确实没有产出**，点名，后面照实留白，不许拿话填满；\n"
          + "（五）九栏各配多少字，**合计不得超过一万**，把字数明确分给每一栏（有产出的栏多给，没产出的栏只留一两百字说明缺在哪）；\n"
          + "（六）这份资料最要紧的一件事是什么——一句话；\n"
          + "（七）哪些内容**明确不写**：重复的、只是漂亮话的、与承重命题无关的，逐条点名。\n"
          + "这一步的意义就是先想清楚再写：写飞了、写到一半被截断，都是因为没有先分配。",
        1: "本段交付〔一〕〔二〕〔三〕〔四〕四栏，栏标题原样照抄，合计约 4700–5300 字。若上面给了《本次的取舍清单》，就按它分配的字数与取舍写，不再自行加码。\n"
          + "一、缘起之问与行进轨迹（约 900 字）——先原样写出缘起之问，再按轮次写：每一轮把问题往前推了多远、靠哪一句推动的、推完之后问题的形状变成了什么。某一轮原地打转就直说原地打转并指出卡在哪。末尾一段：整场从哪儿走到了哪儿，哪一轮是真正的转折。\n"
          + "二、已经立住的核心判断（约 1300 字）——5 到 8 条，按承重程度排序。每条先用一句陈述句写出判断本身，再写清：出自第几轮、由什么撑着（论证／例子／直觉，据实写）、它现在最薄的一处在哪。只是说得漂亮而没有支撑的，照列但标明属于「提到但没论证的」。\n"
          + "三、候选承重命题 X（约 900 字）——给出 2 到 3 个候选，每个写成一句可以被反驳的陈述句。每个候选后附三件：压到 50 字以内的压缩版；一句自答「它能不能被某个已有概念一比一替换」；它若成立会强制推翻什么。最后判一句：本场最该拿去成文的是哪一个、为什么。\n"
          + "四、反复被触到的分离点（约 2200 字）——这是全篇最要紧的一栏，也是论文里可裁决判据的唯一原料。列出 5 到 8 条，每条 300–450 字，逐条写五件：（a）失灵现场——哪一处、第几轮，把那个具体现象说清楚；（b）触发条件——写成可以被指认的形态，不许写「在复杂情况下」这类话；（c）谁在这里失灵——点名那一家解释、写出它握着的代理变量，并替它写一句最可能的辩护词；（d）可裁决草案——试写一句「若⟨条件⟩，本文预测 A，而⟨它⟩预测 B」，写不出就写明卡在哪一环；（e）离可观测还差什么。末尾一句把这几条排序：哪几条现在就能查。",
        2: "本段交付〔五〕〔六〕〔七〕〔八〕〔九〕五栏，栏标题原样照抄，合计约 4700–5300 字，五栏一栏不许省。若上面给了《本次的取舍清单》，就按它分配的字数与取舍写。\n"
          + "五、敌意最近邻清单（约 2000 字）——分两组。〔已在对话中点到的〕逐位 200–300 字：他握着的代理变量、他解释到哪一步为止、他会怎样解释这里的同一个现象、他手上还有哪张牌你们没碰；凡被当成已击倒的靶子，必须把他最强的版本补出来。〔应当交手而尚未交手的〕按题材与按机制各补 2 到 3 位，每位 250–350 字，写清五件：原名与原题（外文占位者必须给出外文原题与原作者姓名）、他命名的是什么、他会怎样解释你们这个现象、一句可裁决差异草案、请他进正文的第几章。至少一位不在承重命题所属的那门学科里，至少一位是外文占位者。末尾一句自查：这批人是不是全落在同一门学科、是不是全是最容易被击倒的那几位。\n"
          + "六、尚未解决的张力与前后不一致（约 1300 字）——逐条列出对话里自己打架的地方、被绕过去的反例、含糊其辞的接口。每条写清：出自第几轮、两边各说了什么、为什么不能同时为真、论文里可能的两三条出路（各附一句代价）。不许写「基本一致」。\n"
          + "七、可裁决判据的线索（约 900 字）——从第四栏挑出最便宜、最可观测的 2 条，各写成完整形态「若⟨条件⟩，本文预测 A，而⟨某最近邻⟩预测 B」，各配一段可观测代理（怎么测、测什么、多少算数）与一句误诊阻挡。其中至少一条标明「现在就能跑」，并写出跑它需要的现成材料。\n"
          + "八、经验材料清单（约 700 字）——对话中出现过的每一个数据、案例、事实逐条列出，各标来源等级〔一手来源〕〔站内自引〕〔未核验〕，并各写一句「它在论文里能承担什么、不能承担什么」。后两种在论文里不得当作已证实的事实去支撑被解释项。本场几乎没有经验材料，就把这件事写清楚，并列出成文时最该补的三类材料。\n"
          + "九、明确不写什么（约 400 字）——这篇论文的边界：哪些相邻问题不在射程内、为什么；再列出三条明知诱人但本次不碰的岔路，各附一句理由。"
      };
      sys = (neigong ? neigong + "\n\n═══════════\n【你此前带着上面这套完整底盘先验、亲手写下并已内化的心得】\n" + (reflect || "（心得暂缺）") + "\n\n═══════════\n" : "")
        ...
        + (P === 0
          ? "\n\n【本次是规划，不是正文】这份资料总共只有一万字，写飞了就收不回来。所以先想清楚再写。"
          : "\n\n本份资料分两段写成（合计约一万字），本次写【" + BRIEF_NAME[P] + "】。"
      ...
      const base = (neigong
        ...
        + (brief
            ? (briefKind === "synth"
              ...
              : "\n\n【本篇的起点是那份《论文入口资料》（提炼档·九栏）——它是清单，不是参考】〔三、候选承重命题〕是你的起跑线而不是终点线；〔四、反复被触到的分离点〕必须至少有两条进正文并写成可裁决形态；〔五、敌意最近邻清单〕里〔应当交手而尚未交手的〕点名的人，至少两位要在正文里指名道姓正面交手，其中至少一位是外文占位者；〔六、尚未解决的张力〕必须逐条正面处理，不许绕过；〔八、经验材料清单〕里标为〔站内自引〕或〔未核验〕的材料，一律不得当作已证实的事实去支撑被解释项，要用就写明它的证据等级。但这份资料只是入口——论文的核心判断必须比它再往前走一步，把它扩写成文即为不合格。")
            : "")
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/wds/distill") {
      ...
      /* ══ 两万字论文的固定学术骨架（14 节 · 合计约 20,700 字）══════════════
         【为什么骨架写死、不交给提纲那一趟去发挥】
         一篇要投出去的论文，该有哪几件东西是**体例规定的**——中英摘要、关键词、
         文献述评、概念界定、可裁决判据、证伪条件、参考文献、附录、投稿声明——
         不是每篇各自发挥的余地。上一版把这十四件也交给基底临场分节，真跑的结果是：
         它写出八节漂亮散文，摘要、关键词、参考文献、投稿声明**一件都没有**，
         而那恰恰是「能不能投出去」的分界线。
         所以：提纲那一趟只定题名、副题、承重命题、判据，与各节贴题材的小标题；
         **哪一节干什么、写多少字，一律由这张表说了算。**
         【为什么是十四节而不是把每节写长】单节 900–1900 字远在安全区内。
         两万字只能靠**加节数**——加长单节必被平台的单请求时长墙掐在思考阶段，
         结果是正文 0 字、流干净结束、还不报错（这一条是站内三次真跑量出来的）。 */
      const PAPER_SKELETON = [
        { h: "摘要与关键词", words: 1100, ask:
          "本节交付四件，缺一不可，按顺序写，不写别的：① 第一行写 `**Title:** 英文题名`（与中文题名对应而非逐字直译）；"
          + "② 【摘要】320–420 字，五件齐全：问题是什么、既有解释缺在哪、本文的承重命题、走的方法路线、主要结论；"
          + "③ 【关键词】4–6 个，用「；」分隔；④ 【Abstract】180–260 英文词，与中文摘要对应；⑤ 【Keywords】与中文关键词对应，用分号加空格分隔。"
          + "摘要里承诺过的每一项，后面正文都必须兑现——兑现不了的现在就不要写进摘要。" },
        { h: "一、引言", words: 1500, ask:
          "1.1 问题的提出——从一个具体场景切入，不要泛论、不要「随着……的发展」这类开头；"
          + "1.2 本文承重命题的预告，一句话说完，不铺垫；1.3 本文的三条贡献，逐条列；1.4 全文结构，一段带过。" },
        { h: "二、文献述评与研究缺口", words: 1900, ask:
          "逐节把 2–3 位敌意最近邻指名请进来（2.1／2.2／2.3 各一位），每位写清：他解释到哪一步、他手里握着的是哪一个代理变量、他的说法在什么条件下开始失效。"
          + "硬要求：至少一位须是用外文发表、直接命名或直接研究过这一现象的占位者，首次出现给原名与原题；"
          + "三位不得全落在同一门学科，至少一位来自本命题所属学科之外的母学科或邻近学科。"
          + "吃不准某人的确切出处，就只说通行看法如何，**绝不编造人名、年份、书名与页码**。"
          + "2.4 收一句他们共同的缺口——那个缺口就是本文要占的位置。" },
        { h: "三、核心概念的界定与操作化", words: 1500, ask:
          "3.1 本文核心概念的正式定义，一句话，不含程度词（不许出现「充分」「真正」「实质性」「恰当」「合理」）；"
          + "3.2 它成立的判定条件，逐条列；3.3 与三个最接近的同族概念逐条辨异，写清差在哪一处；"
          + "3.4 操作化——它在经验层面表现为什么、由哪一个读数承载、谁去测都能测到同一件事。" },
        { h: "四、竞争解释的代理坍缩", words: 1800, ask:
          "逐节拆开第二章每一位最近邻手里的代理变量，各给一个**它解释不了的具体现象**，"
          + "并把分离点写成可裁决的形态：当⟨具体条件⟩时，它预测 A，本文预测 B，而 A 与 B 分别怎么读数。"
          + "只写「本文更强调／更深入／更系统／视角不同」一律不算分离——那些话不可判定，等于承认自己被覆盖了。" },
        { h: "五、承重命题：X 不是 Y，而是 Z", words: 1400, ask:
          "正式立起承重命题，写成「X 不是 Y₁、也不是 Y₂，而是 Z」这一形状；说清 Z 在什么条件下成立、在什么条件下不成立；"
          + "并做一次两句复合测试：若本命题能被两句现成文献拼起来无损重述，就在这里承认这一点，并说明增量究竟落在哪一处。" },
        { h: "六、第二轴的强制与二维辨别格", words: 1700, ask:
          "6.1 候选轴的排除——至少排除两个候选并说明理由；**凡候选轴是 Z 的成因，一律排除**（拿成因当第二轴等于自己跟自己相关）；"
          + "6.2 第二轴的确立——举出一个两轴同时为高的具名真实案例；举不出就不许写「正交」「结构独立」，改写成「相关但不重合」并说明协变方向；"
          + "6.3 二维辨别格——四个象限逐格分条，每格单独一段，段首先写两个坐标值，再写格内描述，最后给一条**只在该格成立**的预测。四格两两比对，不得有两格落在同一组合上。" },
        { h: "七、可裁决判据与可观测代理", words: 1600, ask:
          "7.1 判别式：一条会让第二章每一位最近邻预测相反的判据，**不含情态词**（禁用：应当／有意义／实质性／充分／真正／恰当／合理）——"
          + "它要能拿去问流程、日志、记录，而不是拿去问人的判断；并在三到五个具体场景各跑一遍，答案要互不相同。"
          + "7.2 可观测代理：怎么测、测什么、多少算数、多大差异才算数。7.3 误诊阻挡：哪些情形读数相同却不属于本文所指。" },
        { h: "八、与既有说法的逐条划界", words: 1800, ask:
          "至少四条，逐条写：那个说法说到哪一步／分离线在哪／一个**判决性对照预测**（同一个案例，按它判是 A、按本文判是 B，A 与 B 各自怎么读数）。"
          + "第二章请进来过的人，这里要给出那句可裁决差异——请进来却不写差异，等于没请。" },
        { h: "九、证伪条件与当场检验", words: 1600, ask:
          "三到六条证伪条款，彼此独立、分属不同检验路径。每条写清：去看什么／看到什么就算本文错了／为什么现有说法预测不出这一结果。"
          + "其中至少一条要是「今天就能查、数据多半已经躺在那里」的，并**在正文里当场交出你这次执行的结果**："
          + "查了哪些国家、哪段历史、哪类已发表统计，命中了什么，或到什么范围为止仍未命中。"
          + "不许只描述检索式然后断言「检索必然落空」「至今没有任何一例」——断言全称否定之前先自己跑一遍，跑不动就写成「就本文已核验的范围而言，尚未见到」。" },
        { h: "十、讨论：适用边界与反噬", words: 1400, ask:
          "10.1 适用边界与反向约束——本文主张在什么条件下不成立，至少两条，并说明它们各削掉了本文原本想说的哪一句；"
          + "10.2 与最强竞争解释的正面比较；10.3 一条反噬——若照本文去做，最省事的做法会不会恰好毁掉它想保住的东西。" },
        { h: "十一、研究局限与后续研究", words: 1000, ask:
          "至少三条局限，每条配一个可执行的后续设计（谁去做、拿什么数据、多久能出结果）；"
          + "并交出一到两个**本文解释不了的洞**——写得出这两个洞，这一节才不是客套。" },
        { h: "十二、结论", words: 900, ask:
          "收口。删净「唯一变量」「这场讨论本身就证明了它」这类自封的话；结尾留一个开口，不自我封顶。" },
        { h: "注释·参考文献·附录·投稿声明", words: 1500, ask:
          "以下四项每一项都是必交项，一项不落，按顺序写："
          + "【注释】若无实质注释就写一句「本文无注释」，不许留一个空标题；"
          + "【参考文献】作者—年份制、按姓氏排序；正文里出现过的每一个「作者＋年份」这里都要有对应条目，这里也不许留正文一次也没引用过的条目；"
          + "外文文献必须给原题、不得自造中译名；站内来源写「篇名 — 网址」；**绝不编造页码与引文**，吃不准的条目宁可连同正文那处引证一起删掉；"
          + "【附录 A　读数清点手册】把第七章那个可观测代理写成别人能照着做的规程：单位、粒度、编码规则、边界情形；"
          + "【投稿声明】依次四行：作者贡献、利益冲突、数据与材料可得性、人机分工说明。" },
      ];
      const SPEC = {
        ...
        // 没有这三样，一万字只会变成三千字兑了水。
        paper: { name: "两万字论文", tok: WDS_TOK_MAX, parts: PAPER_SKELETON.length,
          fixed: PAPER_SKELETON, spec:
          "把这场对话【锻成一篇约两万汉字、可直接投稿的学术论文】。不是把三千字那篇撑长，也不是写一篇长文章——"
          + "是写成一篇**照学术体例排得下、能拿出去投、也能被人正面反驳**的东西。\n"
          + "全篇按固定的十四节体例写成（摘要与关键词／引言／文献述评／概念界定／代理坍缩／承重命题／二维辨别格／可裁决判据／逐条划界／证伪与当场检验／讨论／局限／结论／参考文献与附录），"
          ...
          async start(controller) {
            ...
            try {
              ...
              if (dStage === "plan") {
                ...
                if (FIXED) {
                  /* 骨架档的合并纪律：**只收模型给的小标题，ask 与 words 一律取表里的**。
                     模型少给、多给、乱给都不影响体例——十四节永远齐全，这正是写死骨架的全部意义。 */
              ...
              const ssys = BASE
                ...
                + (PFIX ? ("\n\n【学术论文写作规程（这一篇是要投出去的，逐条硬性执行，任一条不过关即返工重写）】"
                  + "\n· 章标题只写一行 `## " + String(sec.h || "") + "`；节标题一律写成一行 `### 3.1 标题`（阿拉伯数字＋点＋数字＋空格＋标题，节号必须与本章章号一致）。这两级都要是真标题行——出 Word 与 PDF 时靠它分层级——除此之外不用别的层级。"
                  + "\n· 正文引证一律「作者 年份」制（例：布尔迪厄 Bourdieu 1977），外文首次出现给原名与原题；不得出现脚注编号、方括号编号或超链接。"
                  + "\n· 可核验事实——书名、逐字引文、章节页码、数据——**绝不编造**。吃不准出处就只说通行看法如何，绝不杜撰人名、年份与页码；只有逐字来自这场对话或载入材料的句子才可以加引号。"
                  + "\n· 不许写「本文更强调／更深入／更系统／视角不同」这类不可判定的话——那等于承认自己被覆盖了。"
                  + "\n· 不画表格、不写 --- 分隔线；需要交付判据表时改写成逐格分条，每格单独一段。"
                  + "\n· 本节开头第一句先交代这一节在全文里承担什么职能，不要一上来就展开。"
                  + "\n· 【写完比写长要紧】本趟有硬时限：宁可每一处都写得紧一点，也必须**把最后一句写完**。"
                  + "眼看要超就压缩后面的内容，绝不许停在半句话、逗号、顿号上，也不许留一个只写了标题的空节。") : "")
                + (prevTail ? ("\n\n【上一节的结尾（只为接得上，别复述它）】\n" + prevTail) : "");
