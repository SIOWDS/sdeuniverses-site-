...
async function askCore(request, env, url, body, SINK) {
  ...
  if (deep) {
    ...
    // 深度默认（未开四步法）：单次方法论——内功+心得+完整方法论，一次调用
      ...
      sys = neigong
        ...
        + "先给一句穿透性核心判断作总纲，再展开；若问题涉及可改变的现实局面，收尾必给「怎么办」——2–3 个针对具体行动者、能照着做的动作，各注明代价与适用条件，不许停在只说方向的空话；纯概念题则不必开方。分量给足，**2200–2800 字**（这一步是论文的原料，写薄了后面整条线都薄；上限卡在这里是因为深度档要先预填内功与五万字站内资料，写得再长就撞 115 秒的闸），结尾留一个把前面前提再往深追一层的升维追问。";
    }
  }
  ...
  const _clk = _heavy ? wdsClock(_deepAns ? 45000 : 60000, 115000) : null;
