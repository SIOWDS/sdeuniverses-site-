    ...
    if (url.pathname === "/api/wds/distill") {
      ...
      const SPEC = {
        ...
        paper1: { name: "学术论文（一趟出全篇）", tok: 32000, spec:
          ...
          + "篇幅：18000–22000 汉字。**字数服从内容**——不要为了凑数把一段拉成三段，也不要把该写透的压成一句。\n\n"
          + "【体例·十六节，缺一件形式审查就退回】\n"
          + PAPER_SKELETON.map((x, i) => (i + 1) + "、" + x.h + "（约 " + x.words + " 字）").join("\n")
          + "\n\n【逐节要交的东西】\n"
          + PAPER_SKELETON.map((x, i) => "◆ " + (i + 1) + "、" + x.h + "\n" + x.ask).join("\n\n")
          + "\n\n【全篇纪律】\n"
          + "· 读者没参与过这场谈话，也没读过任何前情。全程不出现「读者问」「本次对话」之类痕迹。\n"
          + "· 章标题写成一行 `## 三、代理坍缩`，节标题写成一行 `### 3.2 核心概念的名义定义`——两级都要是**真标题行**。\n"
          + "· 不画表格、不写 --- 分隔线；需要象限表时逐格分条，每格单独一段。\n"
          + "· 正文一律「作者 年份」制；不得出现脚注编号、方括号编号或超链接。\n"
          + "· 可核验事实（书名、逐字引文、页码、数据、机构名）**绝不编造**；吃不准出处就只说通行看法，不杜撰人名年份。\n"
          + "· **从第 1 节一路写到第 16 节，中间不要停下来问、不要写「以下继续」、不要在中途总结全篇。**\n"
