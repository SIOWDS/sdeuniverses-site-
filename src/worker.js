    ...
    if (url.pathname === "/api/wds/distill") {
      ...
      const SPEC = {
        ...
        poem: { name: "诗歌（500字）", tok: 6000, spec:
          ...
          + "① 可以是一首长诗，也可以是三到五首短章成组（成组时各有小题）。**先定形式再动笔**，并让形式服从内容。\n"
          + "② **具体胜过抽象**：写得出的东西才写，写不出就换一个能看见的物。抽象名词（时间、意义、存在、命运）每首至多一个。\n"
          + "③ 意象要自己长出来，不要用现成的比喻（岁月如歌、心如止水这类一律不许）。\n"
          + "④ 分行与断句是意思的一部分：让停顿落在该停的地方，不要把散文按逗号切开当诗。\n"
          + "⑤ **不许解释自己**：没有题记、没有注释、没有结尾升华的那一句。\n"
          + "⑥ 第一行是诗题。除诗本身外，前后不要任何说明。" },
        /* ══ 应用文五档（2026-08-23）══════════════════════════════════
