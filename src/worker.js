...
const JOHN_COMPOSE_SPEC = {
  paper:
    ...
    "承重命题 → 用「形式—用法—场合」与三条件把机制拆开（**只用这套本地说法，不许出现 S／D／E、Form-D-Meaning、发生学这类外来术语**）→ 与已有说法的划界（指名道姓，说清两者预测在哪里相反）→ " +
      ...
      if (dStage === "plan" || dStage === "part") {
        ...
        const cstream = new ReadableStream({
          async start(controller) {
            ...
            try {
              let reflect = ""; try { reflect = await ensureReflect(env, url, rvendor, VC, KEY); } catch (e) {}
              const BASE = (prof ? (prof.sys + prof.guard + (prof.term || "")) : "你是 SDE 本体论的老师（SDE 由王德生创立）。")
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          try {
            let reflect = ""; try { reflect = await ensureReflect(env, url, rvendor, VC, KEY); } catch (e) {}
            _st.stage = SPEC.name;
            const sys = (prof ? (prof.sys + prof.guard + (prof.term || "") + "\n\n现在要把一场你与读者的谈话，锻成一件能留下来的东西。")
