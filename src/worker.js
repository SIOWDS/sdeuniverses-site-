...
async function askCore(request, env, url, body, SINK) {
  ...
  if (deep) {
    ...
    // ===== 模式：最终综合提炼（涌现流水线末环，也是整条产线最要紧的一步）=====
      ...
      const P = (part >= 0 && part <= 3) ? part : 1;
      MAXTOK = P === 0 ? 12000 : 32000;
      const BRIEF_NAME = { 0: "规划段", 1: "第一段", 2: "第二段", 3: "第三段" };
      const BRIEF_END = { 0: "〔规划完〕", 1: "〔第一段完·待续〕", 2: "〔第二段完·待续〕", 3: "〔全文完〕" };
      const BRIEF_SPEC = {
        0: "本次**不写正文**，只交两样东西：一棵树的骨架 ＋ 一张取舍清单，合计约 700–1100 字，写完就停。\n"
          ...
          + "（六）九栏里哪几栏本场问对**确实没有产出**，点名，后面照实留白，不许拿话填满；九栏各配多少字，**合计不得超过一万**，把字数明确分给每一栏（第十栏〔论文观点与分章大纲〕另占约四千五百字，由第三段单独写，不占这一万）；\n"
          + "（七）哪些内容**明确不写**：与《缘起之问》接不上的、重复的、只是漂亮话的，逐条点名——**偏题的漂亮话是这一步最该删的东西**。\n"
          + "这一步的意义就是先想清楚再写：写飞了、写到一半被截断、写着写着离开了原题，都是因为没有先把树画出来。",
        1: "本段交付〔一〕〔二〕〔三〕〔四〕四栏，栏标题原样照抄，合计约 4700–5300 字。若上面给了《本次的取舍清单》，就按它给的树骨架、字数与取舍写，不再自行加码；那张清单里的节点编号原样沿用，不许重编。\n"
          + "【本段的形态】第二、三栏一律按 M1 ／ M1.1 ／ M1.1.1 三级写：M 级是判断本身，.1 级是三个着力面各一句，.1.1 级是推到可指认的那一样东西。第四栏每条分离点开头先标它挂在哪个节点（如「挂 M2.3.1」）。不写目录、不复述编号规则，直接用。\n"
        ...
        2: "本段交付〔五〕〔六〕〔七〕〔八〕〔九〕五栏，栏标题原样照抄，合计约 4700–5300 字，五栏一栏不许省。若上面给了《本次的取舍清单》，就按它分配的字数与取舍写。\n"
          + "【本段的形态】沿用第一段已经立好的节点编号，不另起一套：第五栏每位最近邻写明他站在哪个节点的对面；第七栏两条判据各写明出自哪个叶节点；第六栏每条张力写明它横在哪两个节点之间。第九栏专收一件事——本场岔出、与《缘起之问》接不上的那些内容，逐条记明「岔在第几轮、为什么不进论文」。\n"
        ...
        /* 【第三段 —— 2026-08-13 用户令】原话：「提炼精华，处理输入的文档，进行论文的观点和大纲打造，
           然后输出一个文档，交给下面的论文 2 万字打造。提炼精华要写很详细，不仅仅是个大纲。」
           查出来的缺口是实的：前九栏是一张**作业单**（判断、分离点、最近邻、判据、材料），
           里面没有任何一栏是「这篇论文写什么、怎么分章」。而下游成文的八章骨架是**写死在成文那一侧**的，
           跟这份报告没有关系 ⇒ 两头对不上：报告不知道自己在为哪一章供料，成文也不知道该把哪条判断放进哪一节。
           所以补第十栏，并按本文件那条硬规矩**另起一段写**（字数只能靠加段数，不能靠加长单段：
           第二段已经排了五栏 4700–5300 字，硬塞进去必然写不完）。
           它是「观点 ＋ 施工图」：先把全篇论点定死，再逐章交代这一章由前九栏的哪几条供料、
           请谁进来、写多少字——**每一章都要有实质内容，不是一行小标题**。 */
        3: "本段交付〔十〕这一栏，栏标题原样照抄，约 4200–4800 字。这是整份报告的收口，也是交给下一步「成文一篇」的**施工图**。\n"
          + "【本段的形态】沿用前两段的节点编号（M1／M1.1／M1.1.1），不另起一套。每一章都要写实质内容，**不许只写一行小标题**——一行小标题的大纲对成文毫无用处，它已经有固定骨架了，缺的正是「这一章具体写什么、由哪几条供料」。\n"
          + "十、论文观点与分章大纲（约 4200–4800 字）——分三层写：\n"
          + "（甲）全篇论点（约 500 字）：先用一句可被反驳的陈述句写死这篇论文要证成的那一条（形态：「X 不是 Y₁、也不是 Y₂，而是 Z」）；再写三句——它与《缘起之问》的对应关系（答的是这一问的哪一面）、它比问对里最接近的说法多走了哪一步、它若被推翻需要出现什么。这三句是全文的准星，后面每一章都要能挂回来。\n"
          + "（乙）三条贡献（约 400 字）：逐条写成「本文提出/区分/建立了⋯⋯」的完整句，各附一句「它在哪一章兑现」。凡是问对里没有材料兑现的，不许写进来。\n"
          + "（丙）分章大纲（约 3300–3900 字）：论文共八章、约两万字，逐章写清六件——① 本章要证成的那一小步（一句陈述句）；② 由前九栏的哪几条供料（**写出栏号与节点编号**，如「〔四〕第 3 条·挂 M2.3.1」）；③ 本章请谁进来正面交手（点名，写出他握着的代理变量）；④ 本章最难的一处在哪、准备怎么过；⑤ 预计字数；⑥ 与上一章的接口（上一章交出什么，本章才能起步）。\n"
          + "八章依次是：一、引言（问题的提出·承重命题预告·三条贡献·全文结构）；二、文献述评与研究缺口；三、代理坍缩；四、控制变量 Z 的提取与界定；五、第二轴的强制与二维辨别格；六、可裁决判据与可观测代理；七、证伪条件与当场检验；八、结论·局限·延伸。\n"
          + "末尾另起一段〔供料自查〕（约 200 字）：逐条点出前九栏里**还没有被任何一章领走**的内容，并各写一句判断——是这一条其实不该进论文，还是大纲漏了它。两者都要明说，不许含糊过去。"
      };
      ...
      sys = (neigong ? neigong + "\n\n═══════════\n【你此前带着上面这套完整底盘先验、亲手写下并已内化的心得】\n" + (reflect || "（心得暂缺）") + "\n\n═══════════\n" : "")
        ...
        + (P === 0
          ? "\n\n【本次是规划，不是正文】这份报告的正文分三段写成，写飞了就收不回来。所以先想清楚再写。"
          : "\n\n本份《精华报告》分三段写成（合计约一万四千字：九栏作业单约一万字 ＋ 第十栏的论文观点与分章大纲约四千五百字），本次写【" + BRIEF_NAME[P] + "】。"
