...
# 追记 · 2026-09-26　① 把 R2 上的 PDF 误报成「本地文件不存在」

**报**：`check_bookshelf.py` 反复报 #165《普通人都能明白的罗素》、#195《AI 时代为何发生学哲学必须成为每个人的主科》的 `pdfUrl` 指向不存在的文件。

**实情**：两本的 PDF 都在 R2（`/students/wang-desheng/…/m165-v1/`、`…/m195-v1/`；165 于 `be00bf6` 迁桶，195 发书时即上桶）。线上取回 206、`application/pdf`，文件尾有 `%%EOF`（2,688,074 与 2,376,973 字节）；书目页与翻页阅读器都正常。**链接没坏，是体检误报。**

**因**：① 对 `/students/**.pdf` 的豁免写成 `u.endswith('.pdf')`，这两条 `pdfUrl` 带缓存参数 `?v=20260925`，豁免不成立，落进本地存在性检查——本地当然没有。

**修**：新增 `path_only()`／`is_r2_pdf()`，先去掉查询串与锚点再判（与 `src/worker.js` 的拦截同口径）；① 对 R2 上的 PDF 不再直接跳过，改为线上真取前 1KB（`SKIP_R2=1` 可跳过）；③ 也改用这两个函数。
反证：把 #165 的 `pdfUrl` 临时改成桶里没有的文件名，① 报「R2 取不到（404）」，退出码 1。

**读数**：197 条，缺失 0；R2 上的 PDF 15 个已线上真取，取不到 0。
