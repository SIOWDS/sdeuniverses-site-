...
(function () {
  ...
  var PROFILES = {
    ...
    /* ── 第三个分身：edu（ChatYang，2026-08-23）───────────────────────
       同 liter：只覆盖带产品口音的那几条，其余落回 ChatSDE 的通用文案。
       人格、语料白名单、题域闸与术语闸都在服务端 WDS_PROFILES.edu 里。 */
    edu: {
      id: "edu",
      brand: "ChatYang",
      url: "/chatyang/",
      sig: "ChatYang \u00b7 edu.sdeuniverses.com",
      who: "阳涌",
      kicker: "EDU.SDEUNIVERSES.COM",
      home: "/",
      tools: ["iq", "three", "motif", "nbr", "rename", "gap", "collide", "forge", "what", "how", "why"],
      seeds: [
        "这节课评委给了 96 分，可我总觉得什么也没长出来——问题可能在哪？",
        "两个孩子都做对了这道题，怎么看出哪一份正确里还留着能生长的东西？",
        "班上有个学生一提示就会、不提示就不动，我该怎么撤这只手？",
        "我们的评价表还缺一栏，该加什么？"
      ],
      cmd: {
        iq: ["有多新"], three: ["三个角度"], motif: ["主绳"], nbr: ["说过没有", "查一查"],
        rename: ["同行的话", "本地话"], gap: ["没名字", "空位"], collide: ["顶一顶"],
        forge: ["撞一条"], what: ["到底是什么"], how: ["怎么上"], why: ["为什么会这样"]
      },
      hero: {
        zh: {
          title: "ChatYang",
          sub: "教育发生学 · 阳涌。课堂、教学、学习、评价与课程——从你手上那一节具体的课开始。",
          foot: "答案会带教育分站的出处。用你自己的大模型 Key 运行，Key 只存在这台设备的浏览器里。"
        },
        en: {
          title: "ChatYang",
          sub: "Education genesis, with Yang Yong. Classrooms, teaching, learning, assessment and curriculum — starting from one real lesson you have in hand.",
          foot: "Answers cite the education subsite. Runs on your own model key, kept only in this browser."
        }
      },
      copy: {
        zh: {
          ph: "说一节具体的课——一道题、一份评语、一个学生的反应…",
          note: "ChatYang 只答教育、教学、学习、课堂、评价与课程，答案扣着教育分站的原文，可核验的篇名与引文请以原文为准。用你自己的大模型 Key 运行：Key 存在你这台机器上，调用时经本站边缘服务内存转发给你选的厂商，本站不写入数据库、不写进日志、不做分析。",
          setKeyP: "ChatYang 用你自己的大模型 Key 运行。<b style=\"color:#C9A227\">Key 保存在你这台机器上；调用时会经本站边缘服务内存转发给你选的厂商，本站不写入数据库、不写进日志、不做分析</b>，随时可清除。联网搜索走智谱通道，填一把智谱 Key 即可同时用于对话与联网。",
          tlBtn: "⊞ 教学工序", tlTitle: "这一轮走哪道工序", tlNone: "不用工序（普通对话）",
          tipDeep: "满血基底＋满功率思考＋整套教学工序，慢但深",
          srcSite: "教育分站文献",
          tlIq: "这一篇有多新", tlIqS: "五维打分＋它在已有说法里站在哪＋三条能提上去的路",
          tlWhat: "它到底是什么", tlHow: "这一节课该怎么上", tlWhy: "为什么会这样"
        },
        en: {
          ph: "Describe one real lesson — a problem, a comment you wrote, a student's reaction…",
          srcSite: "From the education subsite"
        }
      }
    },
  ...
  var TXT = {
    zh: {
      ...
      mtShowT: "展开档位条（现在开着的）",
      tipStd: "快答档，不烧思考、8000 预算全归正文", tipDeep: "难度条：按这一问在站内检索到的核心词材料多少与准确度自动定档——1 轻 2 常 3 深 4 满 5 极；1–2 档不烧思考（预算全归正文），思考从第 3 档起；落到核心律（如幸福律）且材料厚才到 4–5：顶配基底＋满功率思考＋完整工序，5 档再装完整内功。再点一次「深度思考」展开 1–5，可钉死一档。", tipWeb: " · 已开联网（需智谱 Key）",
    ...
    en: {
      ...
      tipStd: "Fast tier \u2014 no reasoning pass; the whole 8000-token budget goes to the answer",
      tipDeep: "Difficulty bar: the level is set automatically from how much accurate site material the retrieval finds on your question's core terms — 1 light … 5 max. Levels 1–2 spend nothing on reasoning (the whole budget goes to the answer); reasoning starts at level 3. Only a question that lands on a core law with thick material reaches 4–5: top model, full reasoning, full method stages; level 5 also loads the complete SDE groundwork. Click Deep again to unfold 1–5 and pin a level.",
...
「⇄ 改了什么」能看到两版之间动了哪几处。",
      ...
      triTip: "同一问三家接力：第一家用平常话给判断，第二家读到原文专门拆它，第三家找出他们俩都默认、却谁也没提的那样东西。三家各是不同厂商——别处不会请对手来拆自己的台。\n\n座次规矩：一吃两不吃。前两席既不读本站语料、也不用这套术语；只有结算那一席吃 SDE，而且只用它来读前两家，不替他们重写。三个人戴同一副眼镜看同一处，看见的当然一样——那不是共识，是同源共振。",
  ...
  layer.innerHTML =
    "<div class='wdsm-side'>" +
      ...
      "<div class='wdsm-sbot'>" +
        /* 分身页（读书馆等分站）的回首页入口：指向 PROFILE.home。ChatSDE 本身不画（它就在主站）。 */
        (PROF_ID ? "<a class='wdsm-sb wdsm-sbhome' href='" + ((PROFILE && PROFILE.home) ? PROFILE.home : "/") + "'></a>" : "") +
  ...
  function applyLang() {
    ...
    var _sbh = layer.querySelector(".wdsm-sbhome"); if (_sbh) _sbh.textContent = t("sbSite");
    paintTool();
    paintMp(); sbRender();
  ...
  function leave() { if (PROFILE && PROFILE.home) { window.location.href = PROFILE.home; return; } if (window.history.length > 1) { window.history.back(); } else { window.location.href = "/browse/"; } }
  ...
  var KIND_DEF = [
    ...
    { k: "novel", t: "kNovel", sty: 1, doc: 1, w: 120000, c: 1, seq: 1, wo: [100000, 120000, 200000] },
  ...
  var TOOLS = [
    ...
    /* 三道问对（2026-09-08 王德生令）：与上面 what/how/why 三道**轻松版**的分别是
       它们一场接一场——每一轮沿同一件工具多走一格，靠答末那行〔落位〕接力（下一格在服务端算）。 */
