...
这个场景的关键不在两位徒步者临时共用的折叠地图好不好，而在它与现场之间是否还保留来回校正。只要一次失败仍能改变下一步，工具就在服务生活；当失败只能被解释成“使用者不够标准”，方向就反了。母文把理解的临时结盟放在这里观察，也就是说，理解不是我在头脑里把你猜得更准，而是我们暂时同意使用一张可共同修改的地图，并让后续行动受它约束。
...
先承认旧办法的功劳，才能找到它真正反转的位置。两位徒步者临时共用的折叠地图让大量陌生人能够合作，也降低了每次从头摸索的成本。若把一切问题都归咎于规则、模型或标准，就解释不了为何有些高度标准化的现场仍能保持活判断。真正的分水岭不是工具多少，而是工具有没有一个自己无法收买的校准者。
...
== 转折发生在工具不再接受外部纠正时
如果理解只停在我对你的模型更准，双方仍可能各走各路；准确表征没有自动产生共同约束。

反转并非某天突然宣布，通常是许多合理小动作累积的结果：为了公平而统一，为了效率而缩短，为了可比而编码，最后现场只能用编码证明自己。此时两位徒步者临时共用的折叠地图仍沿用原来的名称，功能却从指路变成终点。判断反转应看错误能否穿过记录系统，回头改动规则，而不是看规则写得多精细。
...
== 第二处变化出现在动作顺序
双方先悬置部分预装路线，建立共同参照，约定修订权，再把后续行动交给联盟接受检验。
...
== 第三处变化让旧办法开始自我维持
只要一方保留最终解释权，所谓结盟就会退化成对另一方的收编，表面一致反而遮住关系失败。
...
== 看似相同的结果可能来自相反过程
同意同一句话可能来自顺从，也可能来自结盟；能否安全纠错、改图和退出，是更可靠的分界。
...
== 与最近的说法相邻但不相同
共同基础理论解释了交流者怎样建立共享知识、指称与背景假设以协调语言行动，但临时联盟还要求共享参照对后续行为有约束力，并把可修正幅度作为理解深度。

最近的既有说法是共同基础理论。它已经能解释交流者怎样建立共享知识、指称与背景假设以协调语言行动，但本文另切一层：临时联盟还要求共享参照对后续行为有约束力，并把可修正幅度作为理解深度。可分离判据是，若改变旧理论指出的因素后现象立即消失，本文无需另立；若那些因素已改善，而复述准确度相近时，拥有共同参照、双向修订权和退出权的对话会产生更一致的后续协作仍稳定出现，就说明还有一个独立环节需要命名。
...
再看两位徒步者各有一条熟悉路线，进山后先把地图铺在石头上，约定北方、危险点和谁发现新路就能改线，随后才真正成为同行者，新的分辨是“完成任务”与“接受现场审判”并非一回事。前者可以靠熟练路线抵达，后者要求路线随地面变化而改变。两位徒步者临时共用的折叠地图仍可保留，但必须允许一次真实碰撞把它改写。这样，类比带回的不是反工具口号，而是一条朴素判据：谁能在结果不合时迫使规则退让。
...
== 再划一条能被观察的分界线
布兰顿的规范记分关注参与者怎样在理由实践中相互赋予承诺、资格与修正责任，母文另指出母文关注联盟在注意稀缺和关系风险中能否实际形成，而不仅是断言进入规范空间后的逻辑地位。

另一个相邻说法是布兰顿的规范记分。它关注参与者怎样在理由实践中相互赋予承诺、资格与修正责任，本文则关注母文关注联盟在注意稀缺和关系风险中能否实际形成，而不仅是断言进入规范空间后的逻辑地位。两者可以同时成立，却会给出不同预测：前者改善后若复述准确度相近时，拥有共同参照、双向修订权和退出权的对话会产生更一致的后续协作随之下降，可用旧说法解释；若它在相同条件下持续，甚至因工具更精细而增强，母文的分界才真正站住。划界的目的不是争夺新词，而是防止用错修复方向。
...
这项判断只适用于平等或可安全协商的日常对话、团队协作、共同阅读与人机辅助反思，不能扩张成对一切规则和中介的否定。胁迫、审讯、严重权力不对等、紧急指令、对方只需要被倾听而不愿建立共同任务的场景就是重要反例：那里即使工具密集，参与者仍能被共同后果校正，因此工具与能力可以一起成长。边界要求我们同时看后果硬度、反馈是否有信息、参与者能否承担并修正规则，缺少其中一项都不能草率下结论。
...
== 它不能被这样误用
不能用我们已经结盟压制分歧；地图必须可见、可改、可退出，事实与权益仍有独立约束。

它不主张要求达成一致、把关系置于事实之上、迫使弱者承诺，或把机器的语言配合等同于对称承诺，也不能被权力更大的一方拿去要求别人“多承担真实后果”。合理保护、公平程序和专业标准仍然必要；真正要防的是保护与标准把修正通道一并封死。若长期观察发现共同地图、双向修订和退出权不影响后续协作，理解效果完全由个体表征准确度决定，核心判断就应被削弱或撤回。一个只会把反例重新解释成支持材料的理论，与它所批评的封闭工具没有区别。
