...
ABS: 文章以司机和导航为主类比，区分合理代办与判断接管，并把是否保留自生疑问作为教师主体仍在场的可观察线索。

== 从一个日常场景说起
司机的手一直放在方向盘上，导航替他决定每个路口、提醒每次变道；车开得平稳，某天导航断线，他才发现自己没有留下沿途地形和方向的内部地图

这个场景的关键不在手还握着却失去路感的方向盘好不好，而在它与现场之间是否还保留来回校正。只要一次失败仍能改变下一步，工具就在服务生活；当失败只能被解释成“使用者不够标准”，方向就反了。母文把核态与恍态放在这里观察，也就是说，教师外表仍能完成备课、反馈和答疑，内部却可能不再由自己的疑问、判断和关系记忆组织行动，而被外部流程逐步接管。
...
先承认旧办法的功劳，才能找到它真正反转的位置。手还握着却失去路感的方向盘让大量陌生人能够合作，也降低了每次从头摸索的成本。若把一切问题都归咎于规则、模型或标准，就解释不了为何有些高度标准化的现场仍能保持活判断。真正的分水岭不是工具多少，而是工具有没有一个自己无法收买的校准者。
...
反转并非某天突然宣布，通常是许多合理小动作累积的结果：为了公平而统一，为了效率而缩短，为了可比而编码，最后现场只能用编码证明自己。此时手还握着却失去路感的方向盘仍沿用原来的名称，功能却从指路变成终点。判断反转应看错误能否穿过记录系统，回头改动规则，而不是看规则写得多精细。
...
== 第二处变化出现在动作顺序
外部方案先替代准备，随后替代现场解释，再替代复盘提问；表面任务完整，内部组织逐步停止转动。
...
== 第三处变化让旧办法开始自我维持
流畅产出得到认可，教师更愿交出下一层判断；交出越多，自主追问越少，工具接管显得越必要。
...
== 看似相同的结果可能来自相反过程
合理委托节省操作，恍态式接管则让使用者无法解释为何采纳、何时反对以及下次怎样自行改动。
...
== 最容易误判的是把熟练当成真正拥有
签名和在场不等于主体仍在，真正读数是断开工具后能否指出未决问题、证据冲突和自己的下一步。
...
== 与最近的说法相邻但不相同
自动化偏误研究解释了人为何过度依赖自动建议并忽视相反证据，但核态与恍态关注长期内部组织是否停止生成，不只分析一次错误采纳。

最近的既有说法是自动化偏误研究。它已经能解释人为何过度依赖自动建议并忽视相反证据，但本文另切一层：核态与恍态关注长期内部组织是否停止生成，不只分析一次错误采纳。可分离判据是，若改变旧理论指出的因素后现象立即消失，本文无需另立；若那些因素已改善，而保持工具总量不变，只要求教师保留一个自生疑问并记录其回写，会提高断开工具后的独立判断和情境迁移仍稳定出现，就说明还有一个独立环节需要命名。
...
再看司机的手一直放在方向盘上，导航替他决定每个路口、提醒每次变道；车开得平稳，某天导航断线，他才发现自己没有留下沿途地形和方向的内部地图，新的分辨是“完成任务”与“接受现场审判”并非一回事。前者可以靠熟练路线抵达，后者要求路线随地面变化而改变。手还握着却失去路感的方向盘仍可保留，但必须允许一次真实碰撞把它改写。这样，类比带回的不是反工具口号，而是一条朴素判据：谁能在结果不合时迫使规则退让。
...
== 再划一条能被观察的分界线
教师能动性研究关注教师怎样在制度条件中作出专业选择并塑造实践，母文另指出母文提出表面行动仍完整时能动性也可能已空转，需从疑问与判断的来源辨认。

另一个相邻说法是教师能动性研究。它关注教师怎样在制度条件中作出专业选择并塑造实践，本文则关注母文提出表面行动仍完整时能动性也可能已空转，需从疑问与判断的来源辨认。两者可以同时成立，却会给出不同预测：前者改善后若保持工具总量不变，只要求教师保留一个自生疑问并记录其回写，会提高断开工具后的独立判断和情境迁移随之下降，可用旧说法解释；若它在相同条件下持续，甚至因工具更精细而增强，母文的分界才真正站住。划界的目的不是争夺新词，而是防止用错修复方向。
...
这项判断只适用于教师和一般知识工作者在低风险使用生成工具时，辨认流程代办与内部组织接管的边界，不能扩张成对一切规则和中介的否定。紧急安全、标准答案核验、无障碍辅助、行政自动化，以及把不用工具当成道德优越的场景就是重要反例：那里即使工具密集，参与者仍能被共同后果校正，因此工具与能力可以一起成长。边界要求我们同时看后果硬度、反馈是否有信息、参与者能否承担并修正规则，缺少其中一项都不能草率下结论。
...
== 它不能被这样误用
不能要求教师为证明主体性而拒绝无障碍或效率工具，也不能把疲惫、病痛和资源不足道德化。

它不主张拒绝技术、手工劳动更真实、所有高效都是空心化，或教师必须独自承担全部工作，也不能被权力更大的一方拿去要求别人“多承担真实后果”。合理保护、公平程序和专业标准仍然必要；真正要防的是保护与标准把修正通道一并封死。若长期观察发现保留自生疑问不改善独立判断与迁移，工具断开后表现完全不受长期接管程度影响，核心判断就应被削弱或撤回。一个只会把反例重新解释成支持材料的理论，与它所批评的封闭工具没有区别。
