...
这个场景的关键不在两个人共用的拼图桌布好不好，而在它与现场之间是否还保留来回校正。只要一次失败仍能改变下一步，工具就在服务生活；当失败只能被解释成“使用者不够标准”，方向就反了。母文把交互认知共生体放在这里观察，也就是说，在人与模型共同完成复杂任务时，有效认知未必完整地位于任一方，而可能暂时形成在提示、回写、工具和界面接成的交互系统里。
...
先承认旧办法的功劳，才能找到它真正反转的位置。两个人共用的拼图桌布让大量陌生人能够合作，也降低了每次从头摸索的成本。若把一切问题都归咎于规则、模型或标准，就解释不了为何有些高度标准化的现场仍能保持活判断。真正的分水岭不是工具多少，而是工具有没有一个自己无法收买的校准者。
...
反转并非某天突然宣布，通常是许多合理小动作累积的结果：为了公平而统一，为了效率而缩短，为了可比而编码，最后现场只能用编码证明自己。此时两个人共用的拼图桌布仍沿用原来的名称，功能却从指路变成终点。判断反转应看错误能否穿过记录系统，回头改动规则，而不是看规则写得多精细。
...
== 第二处变化出现在动作顺序
人提出暂定结构，模型生成差异，界面保存分歧，外部工具核验，结果回写下一轮，临时系统由此形成。
...
== 第三处变化让旧办法开始自我维持
平台只按单轮答案评分，团队便优化更长提示；提示变长掩盖接口断点，又把失败归给参与者不会使用。
...
== 看似相同的结果可能来自相反过程
工具相加只是多了部件，共生回路则要求某一方的输出能改变其他部件下一轮的选择规则。
...
== 与最近的说法相邻但不相同
分布式认知解释了认知过程怎样跨个体、人工物和环境共同实现，但交互认知共生体强调可工程化的回写接口与临时边界，并用开关实验检验系统是否真的形成。

最近的既有说法是分布式认知。它已经能解释认知过程怎样跨个体、人工物和环境共同实现，但本文另切一层：交互认知共生体强调可工程化的回写接口与临时边界，并用开关实验检验系统是否真的形成。可分离判据是，若改变旧理论指出的因素后现象立即消失，本文无需另立；若那些因素已改善，而人员、模型和任务都不变时，打开保存分歧与外部核验的接口，会独立提高错误发现和变式保持；关闭后效应消失仍稳定出现，就说明还有一个独立环节需要命名。
...
再看同样两个人和同一盒拼图，换成印有分区、待定区与回退标记的桌布后，他们开始保存冲突碎片、轮流校正并更快发现拼错；变化不只属于任何一双手，新的分辨是“完成任务”与“接受现场审判”并非一回事。前者可以靠熟练路线抵达，后者要求路线随地面变化而改变。两个人共用的拼图桌布仍可保留，但必须允许一次真实碰撞把它改写。这样，类比带回的不是反工具口号，而是一条朴素判据：谁能在结果不合时迫使规则退让。
...
== 再划一条能被观察的分界线
人机协同研究关注人和自动化系统怎样分工、互补并提高任务绩效，母文另指出母文不以角色互补为终点，而把认知位置放在持续相互改写的整个回路中。

另一个相邻说法是人机协同研究。它关注人和自动化系统怎样分工、互补并提高任务绩效，本文则关注母文不以角色互补为终点，而把认知位置放在持续相互改写的整个回路中。两者可以同时成立，却会给出不同预测：前者改善后若人员、模型和任务都不变时，打开保存分歧与外部核验的接口，会独立提高错误发现和变式保持；关闭后效应消失随之下降，可用旧说法解释；若它在相同条件下持续，甚至因工具更精细而增强，母文的分界才真正站住。划界的目的不是争夺新词，而是防止用错修复方向。
...
这项判断只适用于可撤回、低风险的人机写作、研究整理与设计任务中的界面和流程实验，不能扩张成对一切规则和中介的否定。医疗法律裁决、自动武器、隐私资料、责任不可分配的关键任务，以及由流畅对话推断机器人格的场景就是重要反例：那里即使工具密集，参与者仍能被共同后果校正，因此工具与能力可以一起成长。边界要求我们同时看后果硬度、反馈是否有信息、参与者能否承担并修正规则，缺少其中一项都不能草率下结论。
...
== 它不能被这样误用
不能因结果来自回路就让任何参与方免责，也不能用共生称呼诱导依赖、拟人化或取消独立复核。

它不主张机器具有人格、责任由系统平均分担、任何协作都是共生体，或单体能力不再需要测量，也不能被权力更大的一方拿去要求别人“多承担真实后果”。合理保护、公平程序和专业标准仍然必要；真正要防的是保护与标准把修正通道一并封死。若长期观察发现界面与回写条件变化不产生独立影响，全部表现都可由人和模型各自固定能力相加解释，核心判断就应被削弱或撤回。一个只会把反例重新解释成支持材料的理论，与它所批评的封闭工具没有区别。
