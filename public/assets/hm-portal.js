/* 健康大世界 入口页（hm-portal.js，克隆自 sde-portal.js）—— 胡敏健康分站的总入口三角。
 * 只改了配置：三节点文案/链接、正中名、门规常量；三角/花园/火/动画机器一律照搬。
 * ── 原注保留 ──
 * SDE 入口页 —— 打开网站先见到的那一层：一个三角形，三个顶端是三个入口。
 *
 * 三元素（用户定）：SDE 浏览 · SDE 对话 · SDE 社区。三角形不是装饰——
...
(function () {
  "use strict";
  if (window.__hmPortalMounted) return;
  window.__hmPortalMounted = true;
  ...
  var NODES = [
    { k: "browse", x: 50, y: 15, c: "#4FB6B2", icon: "\u25a4", zh: "健康浏览", en: "Health Browse",
      zhS: "专栏 · 专著 · 账册 · 全站检索", enS: "Columns · Books · Ledgers · Search" },
    { k: "wds", x: 88, y: 82, c: "#E0B65C", icon: "\u2726", zh: "ChatHM", en: "ChatHM",
      zhS: "健康问答 · 疾病 · 照料 · 医疗制度", enS: "Health · illness · care · medicine" },
    { k: "im", x: 12, y: 82, c: "#A981C4", icon: "\ud83d\udcac", zh: "HM 社区", en: "HM Community",
      zhS: "群聊 · 私聊 · 社区动态 · 通讯录", enS: "Groups · DMs · Feed · Contacts" },
  ];
  ...
  var GO = { browse: "/site/", im: "/community/", wds: "/chathm/" };
  ...
  /* 烧 TOKEN 的火色（用户定）：浏览烧绿 · 对话烧红 · 社区烧蓝。
...
".sdep-sp{position:absolute;left:50%;top:50%;width:var(--d,16px);height:var(--d,16px);margin:calc(var(--d,16px) / -2) 0 0 calc(var(--d,16px) / -2);border-radius:50%;opacity:0;box-shadow:0 0 22px currentColor;animation-name:none;animation-timing-function:ease-out;animation-iteration-count:1}" +
  ...
  function mount() {
    ...
    mid2.textContent = "HEALTH UNIVERSE";
    ...
    window.HMPortal = { close: close, nodes: NODES };
  }
