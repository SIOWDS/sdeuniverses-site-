# -*- coding: utf-8 -*-
"""《越顺利，越没有你的位置》上站为专著第 84 号"""
import os, re, shutil, html as H

SITE = '/home/claude/site/public'
SRC = '/home/claude/b84'
NO, TITLE = '84', '越顺利，越没有你的位置'
PDFNAME = '越顺利越没有你的位置.pdf'
DST = os.path.join(SITE, 'books/m/' + NO)
SUB = 'AI 时代普通人的困境与出路'
PAGES_N, WAN = '343 页', '约 26.2 万汉字'

os.makedirs(DST, exist_ok=True)
shutil.copy(f'{SRC}/out/{PDFNAME}', f'{DST}/{PDFNAME}')
for f in ('cover.jpg', 'cover-full.jpg'):
    shutil.copy(f'{SRC}/out/{f}', f'{DST}/{f}')

# read.html 抄 83 号
r = open(f'{SITE}/books/m/83/read.html', encoding='utf-8').read()
old_pdf = [x for x in re.findall(r'[^"\'/>]+\.pdf', r)]
r = r.replace('/books/m/83/', f'/books/m/{NO}/').replace('第 83 号', f'第 {NO} 号')
for op in set(old_pdf):
    r = r.replace(op, PDFNAME)
r = r.replace('子宫癌探幽', TITLE)
...
SLUG = {'出版信息': 'pub', '作者介绍': 'au', '前言': 'qy', '导读': 'dd', '导论': 'dl',
        '枢纽章': 'sn', '合章': 'hz', '参考书目': 'ref', '结语': 'jy', '封底': 'fd'}
...
for b in blocks:
    if b.startswith('# '):
        ...
        if re.match(r'^第[一二三四]编', flat):
            ...
            slug = None
            for k, v in SLUG.items():          # ⚠ 表意空格：用前缀匹配，不用整串 get
                if flat.startswith(k): slug = v; break
            if slug is None: slug = 'x%d' % len(pages)
        cur = (slug, t, []); pages.append(cur)
    elif cur is not None:
        cur[2].append(b)

CSS = """<style>
:root{--bg:#0B0E14;--fg:#E6E8EE;--dim:#8A90A0;--gold:#D6AC60;--line:#222836;--card:#121722}
...
h3{font-size:15.5px;color:#B5BECC;margin:22px 0 8px;font-weight:700}
...
for i, (slug, title, bs) in enumerate(pages):
    ...
    for b in bs:
        if b.startswith('### '):
            body.append('<h3>%s</h3>' % H.escape(b[4:].strip()))
        elif b.startswith('## '):
            body.append('<h2>%s</h2>' % H.escape(b[3:].strip()))
        elif b.startswith('> '):
...
idx = (f'<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8">'
       ...
       f'<meta name="description" content="{SUB}。王德生＋Claude 编，据学员专栏十一位作者的十一篇论文编成。网页版全书目录。">{CSS}</head><body>'
       f'{BAR}<div class="wrap"><h1>{TITLE}</h1>'
       f'<div class="meta">{SUB} · 王德生 ＋ Claude 编 · 前言·导读·导论·四编十一章·枢纽章·合章·结语·参考书目·三附录 · {WAN} · {PAGES_N} · 德麦国际专著第 {NO} 号</div>'
