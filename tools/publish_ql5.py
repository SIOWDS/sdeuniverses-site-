...
from ql5_meta import PAPERS, STUDENT, PUBDATE_CN
...
RENAME = [
    ...
    ("这正是发现学的预设", "这正是既成属性论的预设"),
    ("发生学会问另一个问题", "生成机制的追问会换一个问题"),
    ("在本文的论域中，“发生学”不是在描述一个东西“如何产生”，而是在追问",
     "在本文的论域中，所谓生成机制的追问，不是在描述一个东西“如何产生”，而是在追问"),
    ("决断/代偿二分法是一种发现学的操作", "决断/代偿二分法是一种贴标签式的操作"),
    ("而盈余的发生学追问是", "而对盈余的生成机制追问是"),
    ("本文的论证将全程守住这一发生学纯度", "本文的论证将全程守住这一追问纯度"),
    ("发现学式的诊断", "定性判定式的诊断"),
    ("发现学姿势", "贴标签的姿势"),
    ("发现学意义上的包装", "换名意义上的包装"),
    ("发现学", "既成事实论"),
...
EXTRA_CSS = """
.scorebox{border:1px solid rgba(138,104,23,0.42);border-left:3px solid #8A6817;padding:16px 22px;margin:22px 0;font-size:15px;border-radius:8px}
.scorebox b{color:#8A6817}
.scorebox p{margin:6px 0 0;font-size:14px;opacity:.85}
.ref{font-size:14.5px;padding-left:2em;text-indent:-2em;opacity:.86}
.endbox{text-align:center;border-top:1px solid rgba(138,104,23,0.28);margin-top:48px;padding:38px 20px;opacity:.9}
"""
...
<header class="art">
  ...
  <div class="art-meta">作者 {STUDENT["name"]} · {STUDENT["role"]} · 约 {paper["wan"]} 万字 · 发表于{PUBDATE_CN}</div>
</header>
<div class="wrap">
<div class="abstract"><span class="lb"><b>摘 要</b></span><p>{strongify(abstract or paper["hook"])}</p>
{f'<p class="kw"><b>关键词：</b>{html.escape(keywords)}</p>' if keywords else ''}</div>
<div class="scorebox"><b>SDE 创新智商：{paper["score"]}</b>
<p>审稿评分依五维加权（结构精确度·差异锐度·交织深度·不可还原性·可证伪性）。此为编辑评分，待独立复评。</p></div>
{''.join(body)}
...
PRINT_CSS = """
@page{size:A4}
body{font-family:"Noto Serif CJK SC","Noto Serif SC",serif;color:#2A2315;font-size:10.5pt;line-height:1.85;margin:0}
.cover{text-align:center;padding-bottom:13pt;border-bottom:1.2pt solid #8A6817;margin-bottom:16pt}
.eyebrow{color:#8A6817;letter-spacing:.3em;font-size:7.8pt;margin-bottom:10pt}
h1{font-size:18.5pt;line-height:1.44;margin:0 0 8pt;color:#2A2315}
.sub{font-size:10pt;color:#6B5E44;margin:0 auto 10pt;max-width:34em;line-height:1.7}
.by{font-size:9pt;color:#6B5E44}.by b{color:#8A6817}
.abs{background:#F5EFE0;border-left:3pt solid #8A6817;padding:11pt 13pt;margin:0 0 12pt;font-size:9.4pt;line-height:1.75;text-align:justify}
.abs .lb{letter-spacing:.32em;color:#2A2315;font-weight:700}
.kw{font-size:9pt;color:#6B5E44;margin:0 0 16pt}
h2{font-size:13pt;color:#2A2315;padding-left:8pt;border-left:3.5pt solid #8A6817;margin:19pt 0 9pt;page-break-after:avoid}
p{text-indent:2em;text-align:justify;margin:0 0 8pt}
.ref{text-indent:-1.6em;padding-left:1.6em;font-size:8.8pt;color:#6B5E44;margin:0 0 4pt;text-align:left}
"""
...
<style>html,body{{margin:0;height:100%;background:#2A2315}}
header{{height:56px;background:#2A2315;display:flex;align-items:center;justify-content:space-between;padding:0 18px;font-family:"Songti SC","Noto Serif SC",serif;font-size:14px;border-bottom:1px solid rgba(138,104,23,0.4);color:#EFE7D3}}
header a{{color:#C89117;text-decoration:none}}
...
def main():
    ...
    for paper in PAPERS:
        ...
        print(f'  {sid} → {paper["slug"]:24s} {chars:>6d}字 {pages:>3d}页 '
              f'章节{heads:>3d} 文献{refs:>3d}条 第{paper["order"]}篇 IQ{paper["score"]}')

    (ROOT / "tools" / "ql5_report.json").write_text(json.dumps(
        {"papers": [{k: p[k] for k in ("src", "order", "slug", "title", "kind",
                                       "hook", "wan", "pages", "score")}
