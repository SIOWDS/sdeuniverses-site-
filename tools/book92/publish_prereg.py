...
TITLE = '谁来陪伴我？'
PDF = '谁来陪伴我.pdf'
DST = os.path.join(SITE, 'books/m', NO, 'prereg')
os.makedirs(DST, exist_ok=True)

raw = open('prereg.md', encoding='utf-8').read()
...
if MARK not in h:
    ...
    ins = (MARK + '\n<h2>一份已经写好的预注册</h2>\n'
           ...
           f'<p>这一条的<b>完整预注册已经写好并公开</b>：四条并存假设、变量操作化、样本量与功效、'
           '盲法与停止规则、六道可行性门、<b>八种结果分别改写本书哪一节的路由表</b>，以及伦理条款。'
           '它由本书的编写者所写，因而把每一个自由度提前钉死——'
           f'<b>任何被改紧的版本都优先于本版。</b></p>\n'
...
if 'prereg' not in t:
    ...
    assert anchor in t
    ins = (f'<p style="margin-top:26px;font-size:14px;color:#8CA0B4">附：'
           f'<a href="/books/m/{NO}/prereg/">《同成品—同接受项—异删除分支》· '
           f'附录二第九条的可执行预注册</a></p>')
