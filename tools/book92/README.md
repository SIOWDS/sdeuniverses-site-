...
AI 时代的婚姻困境 · 王德生 ＋ Claude 编著 · ISBN 979-8-90690-040-1 · 141 页 · 8.1 万汉字
...
## 2026-08-17 件次补齐（第二版）
按标准件次逐条体检后补的四处：
1. **封面图页**：`build_docx.py` 在扉页前插整幅 `cover.jpg`（宽＝版心宽）——第一版 PDF 里没有封面图页
2. **前言 1,631 → 4,109 字**：新增第五节（十把刀各自是什么、在婚姻里砍出了什么）、第六节（为什么是婚姻·三个条件）、第七节（三种误用）、第九节（怎么被检验的与三处未验），并在第三节补第四处"不算改主意但更不安的地方"
3. **导读 619 → 1,435 字**：补十二个量一览、四道门逐门一句、为什么不像一本关于婚姻的书、对 AI 的立场一句话、篇幅提醒
4. **目录位置**：由扉页后移到导读之后（`asm.py` 插 `## 目　录` 占位，`build_docx.py` 遇此标记调 `render_toc()`；`publish92.py` 跳过该部件不生成网页）
另加厚：作者介绍 226→531（补合作分工与自陈）、结语 495→约 1,100（补"为什么最后只剩四个问题"）、后记 477→约 1,500（补第四、五件）

⚠ **`mkpagemap.py` 的目录页误命中已修**：第一版把「出版信息」匹配到了目录页（p2）而非正文页（p4）——目录页第一行正好是第一个标题。现在先扫出全部目录页放进 `TOCPAGES` 跳过。
终版：**141 页 · 80,687 汉字（PDF 实测）· 豆腐块 0 · 目录 28/28 全命中 · 两遍构建页码漂移 0**
