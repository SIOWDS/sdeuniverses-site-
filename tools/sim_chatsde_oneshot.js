...
/* ⚠ 落点搬家：第一版把十六节写成"逐节要交的东西"＋每节字数目标 ＝ 一张表，
   实测只出 2958 字。现在十六节仍在（要素不许缺），但降格为**要素清单**，
   并明写"不要按固定格子去填"。用意变了，断言跟着变。 */
ok("★ 十六节要素仍然写进提示语（一件都不许缺）", /PAPER_SKELETON\.map/.test(seg));
ok("★★★ 但明写「不要按固定格子去填」「结构与篇幅由内容决定」",
  /不要按固定格子去填/.test(seg) && /全部由内容决定/.test(seg));
ok("★★★ 立了一个心脏：核心论证最该充分展开、每个判断都要给论证",
  /核心论证是这篇文章的心脏/.test(seg) && /不能只断言/.test(seg));
ok("★★★ 长度写成结果不是指标，并配反注水条款",
  /长度是论证充分的自然结果，不是先定的指标/.test(seg)
  && /绝不为了凑字数而注水/.test(seg) && /不要为每一节凑一个预设字数/.test(seg));
ok("★ 承重命题的形状写死（可被反驳）", /X 不是 Y，而是 Z/.test(seg));
ok("★ 真实优先于完整（编造引证是最重的错）", /真实 > 完整/.test(seg));
ok("注释算清了 2958 字是怎么来的", /把表填完/.test(W));
ok("★★ 明写从头写到尾、中途不要停下来问", /中途不要停下来问/.test(seg) && /必须把最后一节写完/.test(seg));
