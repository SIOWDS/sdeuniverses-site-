...
console.log("── 三点八八 · 第四份产物抓到的两处（引文出处双破折号／末页没走收尾）");
{
  const t = (title, bs, i, n) => X.pickLayout({ title: title, bullets: bs, notes: "", kind: "content" }, i, n);
  ok(t("明天就可以开始的两个练习", ["练习一 ｜ 写一份发生史", "练习二 ｜ 追问这是怎么来的"], 10, 10) === "closing",
     "末页即使标题没写「下一步」也走收尾（实测那份叫「明天就可以开始的两个练习」，落回了要点页）");
  ok(t("他这样说", ["「一句原话。」", "某人"], 10, 10) === "quote", "末页是引文时不被收尾抢走");
  ok(t("三条要点", ["甲", "乙", "丙"], 5, 10) === "bullets", "中间页不受影响");
  // ⚠️顺序：兜底必须排在所有形状判据之后，否则会把末页的对照/2×2/大数字全抢走
  ok(t("新旧对比", ["旧 ｜ 新", "甲 ｜ 乙", "丙 ｜ 丁"], 10, 10) === "compare", "末页若是对照，不被收尾抢走");
  ok(t("四格辨别", ["a ｜ 1", "b ｜ 2", "c ｜ 3", "d ｜ 4"], 10, 10) === "matrix", "末页若是 2×2，不被抢走");
  ok(t("三个数", ["3 ｜ 甲", "6 ｜ 乙", "9 ｜ 丙"], 10, 10) === "kpi", "末页若是大数字，不被抢走");
  ok(MOD.indexOf('idx === total && n >= 2 && n <= 4') > MOD.indexOf('return "matrix"'), "兜底在源码里确实排在形状判据之后");
  ok(/replace\(\/\^\[\\s—–\\-－\]\+\/, ""\)/.test(MOD), "引文出处先剥掉模型自带的破折号再补前缀");
  const zq = unzip(Buffer.from(X.build(X.parse("# a\n## b\n---\n## 他这样说\n- 「一句原话。」\n- ——王德生\n"))));
  const sq = zq["ppt/slides/slide2.xml"].toString("utf8");
  ok(/— 王德生/.test(sq) && !/— ——王德生/.test(sq), "实测：出处渲染成「— 王德生」而不是「— ——王德生」");
}
