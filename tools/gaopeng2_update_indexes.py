...
from gaopeng2_meta import PUBDATE_CN

ROOT = Path(__file__).resolve().parents[1]
STU = ROOT / "public" / "students" / "gao-peng"
IDX, PROF = STU / "works" / "index.html", STU / "index.html"
REPORT = json.loads((ROOT / "tools" / "gaopeng2_report.json").read_text(encoding="utf-8"))
BY = {p["slug"]: p for p in REPORT["papers"]}
ORDER = ["grammar-of-good", "flesh-of-prohibition",
         "rules-that-make-subjects", "ethical-autophagy"]
...
def do_index():
    ...
    assert t.count('<div class="work">') == 62, "卡片数应为 62"
    assert t.count("<html") == 1 and t.count("</html>") == 1
    IDX.write_text(t, encoding="utf-8")
    print(f"works 索引：新增 4 张卡片置顶（58 → 62），退役旧「新作」标记 {n_old} 处")
...
def do_profile():
    ...
    t = t[:old_p.start()] + (
        ...
        f'从翻译史、制度演化、语言发育与主体建构四个入口进入同一个问题。另附三种读法：网页长文、在线 PDF 与下载 PDF。</p>'
    ) + t[old_p.end():]

    old_n = re.search(r'浏览全部(\d+)篇', t)
    assert old_n, "找不到浏览全部 N 篇"
    t = t[:old_n.start()] + "浏览全部62篇" + t[old_n.end():]

    assert t.count("<html") == 1 and t.count("</html>") == 1
    PROF.write_text(t, encoding="utf-8")
    print(f"主页：新作清单置顶 4 条，页脚 {old_n.group(1)} → 62 篇")
