...
PAPERS = [{
    ...
    "deck": ("符号学、逻辑学、数学通常被当作三门各有地基的独立学科。本文指出，三门学科当今各自最前沿、"
             ...
             "而是对同一场个体化生成行使的三种权能——命名、判错、计算。三门学科各自最硬的难题"
             "（贝纳塞拉夫非唯一性、解释项难题、多元论坍缩）被统一为同一处症结的三种发作，打开一门新交叉学科：**个体化生成论**。"
             "本文并把它**形式坐实做满三门**、以分区锚定与守恒两重原则加固，最后让它**向前工作**："
             "复原经典、直觉主义、相干三种已知逻辑，并从守恒导出一条被单价基础证实的跨门结论"
             "——同一性被完全结构化的数学基础，其逻辑必为直觉主义——从而由解释框架推进为**可向前预言的研究纲领**。"),
    ...
    "sources": [
        ...
        ("数学 · 单价基础 / 同伦类型论",
         ...
         "单价公理推出「同构即同一」；数学对象无结构之外的内在同一性，消解贝纳塞拉夫困境。且原书明载：排中律的朴素解释与单价公理不相容，单价基础在本性上是直觉主义的、经典逻辑只能受限另加——本文预言的跨门结论的对账靶。"),
    ],
}]
...
def build_page(p, body, toc, pages):
    ...
    t = re.sub(r'<div class="art-meta">.*?</div>',
               f'<div class="art-meta">王德生 ＋ Claude · 约 2.6 万字 · {pages} 页 · '
