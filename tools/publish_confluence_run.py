...
PAPERS = [{
    ...
    "deck": ("符号学、逻辑学、数学通常被当作三门各有地基的独立学科。本文指出，三门学科当今各自最前沿、"
             ...
             "三门学科各自最硬的难题——数学的贝纳塞拉夫非唯一性、符号学的解释项难题、逻辑多元论的坍缩问题——被统一为同一处症结的三种发作，"
             "由此打开一门此前不存在的交叉学科：**个体化生成论**。"
             "本文并把这一断言**形式坐实做满三门**（同构即同一由公理降格为定理／构造本地判准化解坍缩／证明解释项不可实体化），"
             "再以**分区锚定**与**个体化守恒**两重正交原则加固，合成一道三重锁。"),
...
def build_page(p, body, toc, pages):
    ...
    t = re.sub(r'<div class="art-meta">.*?</div>',
               f'<div class="art-meta">王德生 ＋ Claude · 约 2.3 万字 · {pages} 页 · '
