...
PAPERS = [{
    ...
    "deck": ("符号学、逻辑学、数学通常被当作三门各有地基的独立学科。本文指出，三门学科当今各自最前沿、"
             ...
             "本文论证同一性是**一束关系经差异呈现、被指称接住的生成事件**，三门学科是对同一场个体化生成行使的三种权能——命名、判错、计算；"
             "三门各自最硬的难题被统一为同一处症结的三种发作，打开一门新交叉学科：**个体化生成论**。"
             "本文并把它**形式坐实做满三门**、以分区与守恒两重原则加固、再让它**向前工作**："
             "复原经典/直觉主义/相干三种已知逻辑，导出一条被单价基础证实的跨门结论（同一性完全结构化的数学基础其逻辑必为直觉主义），"
             "并最终**产出一条此前无人陈述的新守恒律**——结构同一性、可命名性、经典性支取同一份守恒之荷、不可同时满额，"
             "由此得一可核对的不可能性结论（经典集合论与单价基础恰为这份预算的两个已知落点，第四角被预言为不可一致地建成）。"),
    ...
    "sources": [
        ...
        ("数学 · 单价基础 / 同伦类型论",
         ...
         "单价公理推出「同构即同一」；数学对象无结构之外的内在同一性，消解贝纳塞拉夫困境。且原书明载：排中律的朴素解释与单价公理不相容，单价基础在本性上是直觉主义的、经典逻辑只能受限另加——本文预言与新守恒律的对账靶。"),
    ],
}]
...
def build_page(p, body, toc, pages):
    ...
    t = re.sub(r'<div class="art-meta">.*?</div>',
               f'<div class="art-meta">王德生 ＋ Claude · 约 2.9 万字 · {pages} 页 · '
