...
TYPES.forEach((t) => {
  ...
  ok(t + "：序号连续（v1.7.1：含 6 之二、7 之二、8 之二、8 之三、9 之二、9 之三、9 之四、11A、11B）", rows.map((r) => String(r.n).replace(/\s+/g, "")).join(",") === ["1","2","3","4","5","6","6之二","7之二","7","8","8之二","8之三","9","9之二","9之三","9之四","10","11A","11B"].join(","));
  ...
  ok(t + "：合计 " + total + " 汉字落在 28000–34500（v1.7.1 加 6 之二）", total >= 28000 && total <= 34500);
...
if (fs.existsSync(PAGE_P)) {
  ...
  ok("worker 含 v1.7 四个 mode（consensus／minmodel／install／newroute）与 ===CONS===／===MIN===／===INST===／===ROUTE=== 分隔符", ["checkup","consensus","minmodel","install","newroute"].every((m) => WSRC.indexOf('rmode === "' + m + '"') > 0) && ["===CONS===","===MIN===","===INST===","===ROUTE==="].every((k) => WSRC.indexOf(k) > 0));
const PAGE = fs.readFileSync(PAGE_P, "utf8");
ok("页面含 v1.7 四步（runConsensus／runMinModel／runInstall／runNewRoute）与四个 stage", ["runCheckup","runConsensus","runMinModel","runInstall","runNewRoute"].every((f) => PAGE.indexOf("async function " + f) > 0) && ["consensus","minmodel","install","newroute"].every((k) => PAGE.indexOf('data-s="' + k + '"') > 0));
ok("Skill 是 v1.7 且含⑦之二共识／⑧之三最小模型／⑨之三全量安装／⑨之四新路／R-48／§10 未来方向，并含⑤之二两道＋操作性饱和、⑧之二维度碰撞、典范判据、⑨之二新研究领地＋反土地四击、⑩之二 SDE 改姓、R-41", /version:\s*1\.7\.2\b/.test(SKILL) && SKILL.indexOf("R-60") > 0 && SKILL.indexOf("矛盾对闸") > 0 && SKILL.indexOf("场合闸") > 0 && SKILL.indexOf("α_eff") > 0 && SKILL.indexOf("工序⑤之三 体检闸") > 0 && SKILL.indexOf("R-55") > 0 && SKILL.indexOf("资历击") > 0 && SKILL.indexOf("工序⑦之二 共识闸") > 0 && SKILL.indexOf("工序⑧之三 最小模型算子") > 0 && SKILL.indexOf("工序⑨之三 全量安装") > 0 && SKILL.indexOf("工序⑨之四 新路发生") > 0 && SKILL.indexOf("R-48") > 0 && SKILL.indexOf("未来方向与分年目标") > 0 && SKILL.indexOf("工序⑩之二 SDE 改姓") > 0 && SKILL.indexOf("操作性饱和") > 0 && SKILL.indexOf("反土地四击闸") > 0 && SKILL.indexOf("R-41") > 0 && SKILL.indexOf("候选发生挑战与核心五条") > 0 && SKILL.indexOf("工序⑨之二 新研究领地发生") > 0 && SKILL.indexOf("典范判据") > 0 && SKILL.indexOf("工序⑤之二 敌拓闸") > 0 && SKILL.indexOf("第二道：按读数形状查对象词") > 0 && SKILL.indexOf("工序⑧之二 SDE 维度碰撞") > 0 && SKILL.indexOf("学科内") > 0);
  /* v1.4：⑧之二 学科内维度碰撞 */
  ...
  ok("页面自检 44 项（v1.7.2 加三项：矛盾对／新场合／挤格比区间）", PG.indexOf("自检 44 项") > 0 && (PG.match(/out\.push\(\{n:/g) || []).length === 44 && PG.indexOf("state.pairsN") > 0 && PG.indexOf("installAlphaEff") > 0 && PG.indexOf("crowdLo") > 0);
ok("worker v1.7.2：collide 出 pairs、minmodel 出 newSetting/subclaim、install 出 prop、checkup 认内卷（边缘）", WSRC.indexOf('\\"pairs\\":[{') > 0 && WSRC.indexOf("newSetting") > 0 && WSRC.indexOf('\\"prop\\"') > 0 && WSRC.indexOf("内卷（边缘）") > 0);
