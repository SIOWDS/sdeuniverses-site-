...
const art = { frame: "f".repeat(3000), map: "m".repeat(12000), verdict: "v".repeat(8000), surface: "s".repeat(4000), challenges: "c".repeat(12000), gaps: "g".repeat(8000), collide: "k".repeat(9000), collideRun: "r".repeat(5000), conjectures: "j".repeat(12000), occupants: "o".repeat(6000), territory: "t".repeat(10000), territoryCheck: "q".repeat(5000), checkup: "h".repeat(3000), consensus: "n".repeat(5000), minmodel: "i".repeat(6000), install: "e".repeat(20000), newroute: "w".repeat(5000) };
const refs = []; for (let i = 1; i <= 70; i++) refs.push("Author. 2000. Title " + i + ". Venue. doi:10.1/" + i);
let fail = 0, maxLen = 0;
for (const type of ["how", "what", "why"]) for (let sec = 0; sec < 17; sec++) {
...
const r2 = run({ sec: 16, art, cards: mk("how"), refs, prev: "p".repeat(5000) }, "how", { url: "https://openrouter.ai/api/v1", model: "nvidia/nemotron:free", name: "OR" });
...
console.log(fail ? fail + " FAILED" : "WRITE BRANCH OK (51 sections × 4 checks)");
