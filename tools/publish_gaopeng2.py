# -*- coding: utf-8 -*-
"""高鹏四篇（禁令与强制性义务 · B 批打磨版） · 建页 + PDF + 三种读法。

用法： python3 tools/publish_gaopeng.py --src /home/claude/gp6
...
from gaopeng2_meta import PAPERS, STUDENTS, PUBDATE_CN
from gaopeng2_supp import NOTES, REFERENCES
...
<p>本批四篇评分排名第 {paper["rank"]}。打磨走外科手术式定点修改，只动审稿记下的扣分点——一处实质论证错误的更正、被放跑的最近邻（动机排挤／福柯／默会知识与古德哈特定律）、承重材料的来源交代，以及虚构学术礼节与充门面文献的清除——作者的核心判断与章节结构一律不动。提升后分数为编辑自评，待独立复评。</p></div>
{''.join(body)}
...
def main():
    ...
    (ROOT / "tools" / "gaopeng2_report.json").write_text(json.dumps(
