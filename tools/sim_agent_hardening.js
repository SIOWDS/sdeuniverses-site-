...
/* ⚠ 窗口要按注释长度留余量：注释一变长，贴身窗口的断言就会红成假缺陷。锚点也不带尾括号。 */
const _lm = W.indexOf("【本轮收尾：交一个结账块");
const LEDGER_BLK = W.slice(_lm - 900, _lm + 1800);
ok(/if \(mode === "answer"\) \{[\s\S]{0,1200}【本轮收尾：交一个结账块/.test(W),
...
/* 🔴 真跑抓到的第二处：这一条注入了却没被执行——深度档 sys 先把结尾定死成「升维追问」，
   基底写完就收笔。两条收尾指令并存时，认的是先出现的那条结构规定。 */
ok(/它才是本轮的最后一行/.test(W) && /那\*\*不是\*\*最后一步/.test(W),
...
/* 🔴🔴🔴 前三次修（改措辞／显式改写旧收尾／搬到 user 末尾）全部无效。
   决定性对照（同一天线上真跑）：**轻档 system 约 1k 字 ⇒ 一次就交；深度档 76k 字 ⇒ 四次全无。**
   所以不是措辞、不是位置，是那副被反复校准过的强骨架（六路径→三维→三方程→误差互消→
   逮先验→怎么办→升维追问）压掉了一条挂在末尾的新纪律。
   💡 通则：**一条总被忽略的指令，先问它跟谁抢，再问它说得够不够狠。**
      治法是把它写进骨架、成为编号动作的一项，而不是继续在骨架外加码。 */
/* ⚠ 抠取要用「同一段之内」的两个锚点：「写完比写长要紧」在别处也出现，且排在前面，
   直接 indexOf 会得到一个倒过来的区间（空串），断言全红成假缺陷。 */
const _ds = W.indexOf("你现在是「SDE Universes」站内知识助手。请用 SDE 方法论");
const DEEPSYS = W.slice(_ds, W.indexOf("【写完比写长要紧】", _ds));
ok(/⑧ \*\*最后一步：交结账块\*\*/.test(DEEPSYS),
  "深度档骨架里，结账块是编号动作的第⑧项（与前七步同级，不是骨架外的附加要求）");
ok(/⑧ 与前面七步同级/.test(DEEPSYS), "并明说它与前七步同级");
ok(/升维追问不是最后一行，结账块才是/.test(DEEPSYS),
  "深度档那句写死结尾的话被就地改写（不改它，新纪律永远在跟它抢）");
