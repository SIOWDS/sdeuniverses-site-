# -*- coding: utf-8 -*-
"""秦莉四篇（《追忆似水年华》改编研究 · 补分离线版） · 建页 + PDF + 三种读法。

用法： python3 tools/publish_qinli6.py --src /home/claude/ql6
...
from qinli6_meta import PAPERS, STUDENTS, PUBDATE_CN
from qinli6_supp import NOTES, REFERENCES
...
RENAME = [
    # 招牌词 → 跨媒介／改编研究的本地说法（长词在前，避免被短词吃掉）
    # 注意：本批**不动「本体论」与「裂缝」**——前者是本学科本行话（巴赞《摄影影像的本体论》），
    # 后者在四篇里全是普通汉语用法（"暴露出一道裂缝"）。只有发生学／发现学是学派口音。
    ("在发生学层面上", "在生成层面上"),
    ("发生学与现象学", "生成分析与现象学"),
    ("发生学逻辑截然不同", "生成逻辑截然不同"),
    ("发生学上是两种完全", "在生成层面上是两种完全"),
    ("发生学定位", "生成层面的定位"),
    ("发生学判据", "生成判据"),
    ("发生学判断", "生成层面的判断"),
    ("发生学重写", "生成层面的重写"),
...
<p>本批四篇评分排名第 {paper["rank"]}。本轮打磨只补一样东西：与场域里最近那位邻居的分离线（查特曼／巴特／马克斯）。作者的核心判断与章节结构一律不动，正文其余部分一字未改。提升后分数为编辑自评，待独立复评。</p></div>
{''.join(body)}
...
def main():
    ...
    for paper in PAPERS:
        ...
        probe = page
        # 本批不查「本体论」与「裂缝」——前者是本学科本行话（巴赞《摄影影像的本体论》），
        # 后者在四篇里全是普通汉语用法。只查真正的学派口音。
        leaked = [w for w in ("发生学", "发现学", "发生论", "显露",
                              "纠缠", "差异序列", "金点子", "回写", "三界") if w in page]
    ...
    (ROOT / "tools" / "qinli6_report.json").write_text(json.dumps(
