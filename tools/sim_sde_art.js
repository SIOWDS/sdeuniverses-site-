...
ok("九步一律不自带上限（传 0），统一走阶梯——含开工仪式那一次",
  (js.match(/mmChat\([^;]*?, 0[,)]/g) || []).length === 9,
...
/* ═════ 十一之六点五、真跑漏网后补的占位（2026-07-31 第一份产物） ═════ */
group("十一之六点五、真跑补入的占位");
{
  ok("「喷发」族在库里（真跑那张画正撞在这里）", !!box.PLACE_SDE["喷发"]);
  ok("「放射光芒＝灵感迸发」写进通用陈词",
    box.PLACE_CLICHE.some(s => /放射光芒/.test(s) && /灵感迸发/.test(s)));
  ok("同族的漩涡/晚霞/孤树/剪影逆光也一并补上",
    ["漩涡","晚霞","孤树","剪影逆光"].every(k => box.PLACE_CLICHE.some(s => s.indexOf(k) >= 0)));
  ok("通用陈词已扩到 30 条以上", box.PLACE_CLICHE.length >= 30, String(box.PLACE_CLICHE.length));
  ok("补入处留了出处注释（为什么补、什么时候漏的）", /第一份真跑产物漏网后补入/.test(js));
  ok("闸门明令「动词也要查」——占位常藏在动作里而非名词里",
    /动词也要查/.test(js) && /喷发／迸发／涌出／绽放/.test(js));
  ok("并给出动作类默认图的对照（喷发→放射光芒等）", /喷发→满幅放射光芒/.test(js));
  ok("绘图指令写死「必须是一个场，不是拼贴」",
    /必须是一个场，不是拼贴/.test(js) && /one continuous surface/.test(js));
  ok("并写明这条是实测栽出来的（硬竖缝把连接分项拉塌）", /连接分项当场塌/.test(js));
}
...
/* ═════ 十一之八、错误码翻译（第四次真跑撞 402「Key 不对？」之后加的） ═════ */
group("十一之八、错误码翻译");
{
  ok("错误码表在场且覆盖八类", /var MM_ERRS = \[/.test(js)
    && ["1008","1004","1002","1039","1013","1027","2013","1001"].every(c => js.indexOf(c) >= 0));
  ok("402/1008 明写「不是 Key 的问题」（这正是读者会猜错的那一步）",
    /1008\|insufficient_balance/.test(js) && /不是 Key 的问题/.test(js));
  ok("401/1004 才是 Key 的问题，并提示国内外站 Key 不通用",
    /1004\|invalid_api_key/.test(js) && /不通用/.test(js));
  ok("两处 HTTP 报错都先给人话再附原文", (js.match(/mmExplain\(/g) || []).length === 3);
  ok("认不出的错误码不硬套解释（返回空串则退回原文）", /return "";\n}/.test(js));
  ok("注释写清做这张表的理由：报错不解释，读者只能猜", /读者就只能猜/.test(js));
}

/* ═════ 十一之九、开工仪式·心得（涌现流水线铁律的第三样） ═════ */
group("十一之九、心得");
{
  ok("铁律三样齐：内功 ＋ 心得 ＋ 方法论", /内功 ＋ \*\*心得\*\* ＋ 方法论/.test(js));
  ok("ensureReflect 在场且带 onStat（这一步要让读者看得见）", /async function ensureReflect\(core, onStat\)/.test(js));
  ok("按基底＋口径版本作缓存键，改口径自动作废", /REFLECT_KEY \+ ":" \+ MODEL/.test(js) && /心得口径版本/.test(js));
  ok("心得只装进 sysBase（生成类），闸门与评分类不走 sysBase",
    /你自己写下的绘画心得/.test(js) && /只装进\*\*生成类\*\*/.test(js));
  ok("心得明说术语放开、对内不对外", /术语放开/.test(js) && /对内不对外/.test(js));
  ok("心得六节里有「我最容易在哪里偷懒」（防基底滑向廉价做法）", /最容易在哪里偷懒/.test(js));
  ok("并把三种典型廉价做法点名（含满幅光芒表示灵感——真跑栽过的那条）",
    /把命题翻译成象征物/.test(js) && /用满幅光芒表示灵感/.test(js));
  ok("要「读完之后多出来的判断」，不许复述原文", /多出来的判断/.test(js) && /不要复述原文/.test(js));
  ok("写不出来不阻断开工，但要如实说", /心得没写成/.test(js) && /REFLECT = "";/.test(js));
  ok("页面把心得亮出来，并给重写与清掉两个按钮",
    /reflectCard/.test(html) && /btnReflectAgain/.test(html) && /btnReflectDrop/.test(html));
  ok("成本把心得那一次算进去，且已缓存时不重复计",
...
/* ═════ 十三、页面结构 ═════ */
group("十三、页面结构");
ok("六个步骤面板齐（连接/选档/入题/发生现场/心得/结果）",
  /① 连接基底/.test(html) && /② 选档/.test(html) && /③ 入题/.test(html)
  && /④ 发生现场/.test(html) && /⑤ 开工仪式/.test(html) && /⑥ 三视觉观点/.test(html));
