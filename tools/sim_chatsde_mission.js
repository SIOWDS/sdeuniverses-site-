...
try {
  ...
  const b = S.indexOf('\n\n\n/* ═══════════ 三类问题', a);
  B = new Function(S.slice(a, b) + '\n; return { MISSION: SDE_METHOD_MISSION, SETTLE: SDE_METHOD_SETTLE, DEEP: SDE_METHOD_BLOCK, LITE: SDE_METHOD_LITE };')();
...
if (B) {
  ...
  ok(B.LITE.length < 900, '精简版控制在 900 字以内', B.LITE.length + ' 字');
  ok(B.DEEP.indexOf('意义三律') >= 0 && B.LITE.indexOf('意义三律') < 0, '意义三律等只留深度档');
}

console.log('【六】装配：非分身档每一轮都注入');
