...
if (B) {
  ...
  /* 长度账（每抬一次都要写清理由——这是标准档每一轮都付的固定成本，不是可以随手放宽的数）：
       900 → 1300：装进「先清家底」(+504)
      1300 → 1800：装进「朝五维去写」(+509)
      1800 → 2200：装进「交账」(+408)——这一条买回来的是**可测量**：
                   前三刀走没走过，此前全靠猜；有了账才有得数。
     ⚠ 到此为止。再往里加提示词，边际收益已经很低，而成本是每轮实打实付的。 */
  ok(B.LITE.length < 2200, '精简版控制在 2200 字以内', B.LITE.length + ' 字');
