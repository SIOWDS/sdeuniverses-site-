...
(async function () {
  ...
  /* ⚠ 2026-08-21 契约翻面：旧文案写「请再点一次「成文一篇」补齐」——**那是假话**：
     doPaper 一进来就 paperAll=''、从第一段整篇重跑，不是接着补。读者照着做会把已写的字冲掉。
     新判据守两件：给得出重来的动作，且**说清楚那是重写不是补**，并提醒先把已写的复制走。 */
  ok(h.indexOf("再点一次「成文一篇」") > 0, "状态栏：给出重来的动作");
  ok(/从第一段整篇重写[\s\S]{0,40}不是接着补/.test(h), "状态栏：说明白那是整篇重写，不冒充「补齐」");
  ok(/复制走/.test(h), "状态栏：重写前提醒先把已写的字复制走");
