...
PAPERS = [{
    "slug": "isomorphic-labour",
    "src": Path("/home/claude/conf44.md"),
    "title": "认不出来，就得自己交代",
    "sub": "论核实这份工作如何从评价一方移到被评价一方，及它所产生的那类不入账的劳动",
    "cross": "经济学 × 艺术学 × 工程学",
    "no": "之四十八",
    "deck": ("生成一份看上去合格的产出，边际成本已接近于零；而评价一方分辨它们的能力没有同步提高。"
             "三门学科对由此产生的困境给出三条互相取消的解释：经济学说决定性的是稀缺搬到了哪里，"
             "艺术学说决定性的是制作路径被重写，工程学说决定性的是接口没有跟着能力重新切开。"
             "三条各自成立，却指定了三个不同的决定项。三家共有一个没说出口的前提——"
             "核实一份产出这件工作，有一个由领域性质给定的承担者；推翻它的材料来自工程学自己："
             "逆康威操作每天都在调换谁先动，可见承担者是可以被调换的。由此得到的判断是："
             "这类困境不是要素固定下的劳动内耗，不是为分离而追加的信号支出，也不是指标被博弈后的失真，"
             "而是核实的负担从评价一方向被评价一方的整体转嫁——什么都没有多出来，是同一份工作换了人做。"
             "文中给出二维辨别表（其中一格是关键对照：产出暴涨而分辨跟得上时，困境根本不发生）、"
             "一句不含判断词的问话、十处兑现（两处反过来迫使命题收窄）、与十种既有说法各自可分开的观察，"
             "以及一条用现成时间戳一周可跑完的检验。第九节另与本栏同日同取这三个学科的四篇逐条分清缺口位置，"
             "并据五处合观提出一条更强也更易被推翻的判断：五处不是五种病，是同一笔账在五个位置上的分录缺失。"),
    "clash": ("第一家把技术对劳动的作用按任务来读，主张决定分配的是稀缺搬到了哪里；"
              "第二家主张生成式工具改变的是作者与工具之间的分工位置，价值必须重新落到制作路径上；"
              "第三家主张系统的形态由造它的组织的沟通结构决定，能力变了而接口不变，"
              "所有人就会挤在同一个接口上堆动作。三家指定的被驱动项互不相同，因而不能同为决定项——"
              "而且各自的日课正是另一家诊断出的病：切成模块（工程）是艺术判定的病根，"
              "为一切定价（经济）是工程判定的病根，宣示不可替代的作者性（艺术）是经济判定的病根。"),
    "sources": [
        ("经济学 · 任务位移与稀缺的搬家",
         "Acemoglu &amp; Restrepo, Tasks, Automation, and the Rise in US Wage Inequality "
         "(Econometrica 90:5, 2022)；另见 Autor et al., New Frontiers (QJE 139:3, 2024)",
         "https://doi.org/10.3982/ECTA19815",
         "主张过去四十年美国工资结构的变化主要由任务的位移解释，而非技能偏向型技术进步；"
         "并指出平庸的自动化——生产率提升微弱却足以替代劳动——对分配的破坏最大。"),
        ("艺术学 · 生成式工具改写的是分工位置",
         "Epstein, Hertzmann et al., Art and the science of generative AI (Science 380:6650, 2023)；"
         "另见 Hertzmann, Can Computers Create Art? (Arts 7:2, 2018)",
         "https://doi.org/10.1126/science.adh4451",
         "主张生成式人工智能改变的不是作品，而是作者与工具之间的分工位置——"
         "创作的努力从执行移向指定、筛选与策展。"),
        ("工程学 · 形态由接口决定，而接口可以反过来先动",
         "Conway, How Do Committees Invent? (Datamation, 1968)；"
         "MacCormack, Baldwin &amp; Rusnak (Research Policy 41:8, 2012)；"
         "Sculley et al., Hidden Technical Debt in Machine Learning Systems (NeurIPS 2015)",
         "https://doi.org/10.1016/j.respol.2011.11.005",
         "镜像假说的实证：系统架构与组织架构互为镜像。而这一门每天在用的逆康威操作把它反过来用——"
         "先改组织结构以得到想要的架构，正是这条实践证明了谁先动可以被调换。"),
    ],
}]
