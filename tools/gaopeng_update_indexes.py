...
from gaopeng_meta import PUBDATE_CN

ROOT = Path(__file__).resolve().parents[1]
STU = ROOT / "public" / "students" / "gao-peng"
IDX, PROF = STU / "works" / "index.html", STU / "index.html"
REPORT = json.loads((ROOT / "tools" / "gaopeng_report.json").read_text(encoding="utf-8"))
BY = {p["slug"]: p for p in REPORT["papers"]}
ORDER = ["hollowed-substratum", "light-of-remedy",
         "night-watch-of-unknowing", "encountered-charge"]
...
def do_index():
    ...
    assert t.count('<div class="work">') == 58, "卡片数应为 58"
    assert t.count("<html") == 1 and t.count("</html>") == 1
    IDX.write_text(t, encoding="utf-8")
    print(f"works 索引：新增 4 张卡片置顶（54 → 58），退役旧「新作」标记 {n_old} 处")
...
def do_profile():
    ...
    t = t[:old_p.start()] + (
        ...
        f'同一原初问题的三个金点子与一篇涌现新典范。另附三种读法：网页长文、在线 PDF 与下载 PDF。</p>'
    ) + t[old_p.end():]

    old_n = re.search(r'浏览全部(\d+)篇', t)
    assert old_n, "找不到浏览全部 N 篇"
    t = t[:old_n.start()] + "浏览全部58篇" + t[old_n.end():]

    assert t.count("<html") == 1 and t.count("</html>") == 1
    PROF.write_text(t, encoding="utf-8")
    print(f"主页：新作清单置顶 4 条，页脚 {old_n.group(1)} → 58 篇")
