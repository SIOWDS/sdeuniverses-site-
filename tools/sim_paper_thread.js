...
/* ⚠ 这条判据把字数区间**和它后面那半句话**绑在一起（「，结尾留一个」）。后来那半句被改成
   「，之后留一个……」，正则当场落空，报出来的是「实得 ?」——看着像字数超标，其实是判据自己瞎了。
   守的东西是「深度档一轮的字数上限」，就只锚那个区间，别锚它的邻居。 */
const _deepA = W.indexOf("else if (reflect && neigong) {");
const _deepSeg = _deepA > 0 ? W.slice(_deepA, _deepA + 6000) : W;
const mAnsLen = _deepSeg.match(/\*\*(\d+)–(\d+) 字\*\*/);
ok(!!mAnsLen, "抠得到深度档那一轮的字数区间（锚点变了先改本脚本，别当成字数超标）");
ok(!!mAnsLen && Number(mAnsLen[2]) <= 2200,
  "深度档问答字数上限 ≤ 2200（107 秒写不完 2800）· 实得 " + (mAnsLen ? mAnsLen[1] + "–" + mAnsLen[2] : "?"));
...
/* 2026-08-21 这一句从「有／没有」升级成「这一份出自哪里、多少字」：
   有没有心得此前是唯一的观测口，而它连缺的是哪个键、为什么缺都说不出来。 */
ok(/_stat\(reflect\s*\n?\s*\? \("✅ 内功与心得就绪 · 心得 " \+ reflect\.length/.test(W),
  "内功／心得装完报一声，并说清字数与这一份的来源（缓存／同厂商回退／本场现生成）");
