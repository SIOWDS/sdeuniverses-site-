#!/usr/bin/env python3
"""Final source-credit and journal-label correction; never changes an original PDF."""
...
def main():
    ...
    report=q.read_json(OUT/'report.json');report['characters']=sum(x['chars'] for x in manifest);report['final_polish']={'pages':len(changed),'multiple_credit_pages':sum(len(x['credits_retained'])>1 for x in changed),'changes':changed}
    ...
    print(json.dumps(report['final_polish'],ensure_ascii=False,indent=2))
