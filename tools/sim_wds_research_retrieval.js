...
(async () => {
  ...
  if (box) {
    ...
    ok("十道里除第 6 道外都配了后缀（第 6 道由敌意近邻链接管）", [1, 2, 3, 4, 5, 7, 8, 9, 10].every((i) => (box.RES_WEB_SUFFIX[i] || []).length >= 1) && !box.RES_WEB_SUFFIX[6]);
  }
  ...
  ok("第 6 道仍走敌意近邻链（wantNbr 在 wantWeb 之前）", CHAT.indexOf("if (wantNbrG) {") > 0 && CHAT.indexOf("if (wantNbrG) {") < CHAT.indexOf("} else if (wantWeb) {") && /const RES_NBR_STAGES = \{ 6: 1 \};/.test(SRC));
