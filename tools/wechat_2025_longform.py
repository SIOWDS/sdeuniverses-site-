...
def convert(pdf,dest,url,archive,filename,public,source_url=None,expected_sha=None):
    ...
    for page_num,page in enumerate(doc,1):
        ...
        for row in rows:
            ...
            elif (y<35 or y>page.rect.height-45) and (re.match(r'^https?://mp\.weixin\.qq\.com/',compact(text)) or re.fullmatch(r'\d+\s*/\s*\d+',text) or re.match(r'^20\d{2}/\d{1,2}/\d{1,2}\s+\d{1,2}:\d{2}',text)):reason='浏览器打印页眉页脚'
