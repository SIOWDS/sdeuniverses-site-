...
'day3': dict(
  tag='老师、学生、教室，这三样都不是现成的——是这节课做出来的',
  keys=['上午 9:30–11:30', '下午 14:00–16:30',
        '第三天不上机', '每人带走一节改好的真课'],
  lead='第三天回到四十分钟本身。这一天只讲一件事：'
       '<b>一节课里有三样东西在轮流长——老师、学生、教室。</b>'
       '它们不是上课之前就摆在那里的，是这节课自己做出来的；'
       '哪一样没长出来，这节课就在哪里没发生。',
  blocks=[
    ('上午 · 9:30–11:30　先看一节课，再拆开它', [
      ...
      ('p', '不先讲要点。开场就上一节<b>真课</b>，二十来分钟，'
            ...
            '与昨天同一个办法——昨天先看做好的教材，今天先看上完的课。'),
      ('h3', '一、这三样都不是现成的'),
      ('big', '讲台后面站着一个人——<b>他还不是老师</b>。<br>'
              '座位上坐着四十个孩子——<b>他们还不是学生</b>。<br>'
              '一间摆着桌椅的屋子——<b>它还不是教室</b>。<br><br>'
              '不是待在那个地方的就叫老师、学生、教室。'
              '<b>这三样都是这节课自己做出来的东西</b>：'
              '课上成了，它们才出现；课没上成，铃响到铃响，'
              '屋里始终只有一个念稿子的人、一群旁听的人，和一间会场。'),
      ('h3', '二、各自在哪一刻被做出来'),
      ('table', [['这一样', '它出现的那一刻', '在此之前它是什么', '当堂能看的记号'],
                 ['老师', '第一次<b>因为学生说的话</b>改了自己下一句',
                  '一台放录音的机器', '这节课你改了几次？零次就是在放录音'],
                 ['学生', '第一次<b>押上一个自己的说法</b>（哪怕是错的）',
                  '看客', '押过说法的有几个人'],
                 ['教室', '屋里第一次出现<b>一件谁都还没定下来的事</b>，全场一起等着它',
                  '会场', '这样悬着的时间，全节课加起来有多久']]),
      ('note', '这三个记号就是下午要带回去的三个数。'
               '它们都不用问学生、不用考试、不用等——当堂就有。'),
      ('h3', '三、三样是轮着长的'),
      ('p', '一节课不是三样各长各的，是一圈一圈轮着来：'),
      ('ol', ['<b>学生</b>押上一个说法——学生这一头动了；',
              '<b>老师</b>因为这个说法改了下一句——老师这一头动了；',
              '<b>全场</b>因此一起悬在那儿——教室这一头动了；',
              '悬着的地方又岔出一个新问题——回到第一步。']),
      ('big', '这一圈转不起来，这节课就没发生。'
              '而且好定位：<b>三样里哪一样一直不动，问题就在哪一样。</b>'),
      ('h3', '四、四种「没发生」，其实就是哪一样没长'),
      ('table', [['看上去的样子', '其实是哪一样没长', '该怎么补'],
                 ['整节课很流畅，没人卡住', '教室没长出来（全程没有一件没定下来的事）',
                  '故意做一道会撞墙的题'],
                 ['孩子很配合，可没人说自己的想法', '学生没长出来（全是看客）',
                  '开口之前先让他押一个'],
                 ['讲得很好，可跟昨天讲的一模一样', '老师没长出来（在放录音）',
                  '停下来，先听一句，再决定下一句'],
                 ['怎么讲都进不去', '孩子心里那条规矩上了锁',
                  '先开锁（第二天那三句话），再讲']]),
      ...
      ('card d', '两人一组，规则只有一条',
       ...
       '这三十秒练的就是「教室」那一样——'
       '全场一起悬着的时间，全靠老师忍住不把它关掉。'),
    ]),
    ('下午 · 14:00–16:30　改一节自己的真课，带走三个数', [
      ('h3', '一、课堂用语：不要说 / 要说'),
      ('table', [['不要说', '改成'],
                 ['「记住，就是这样。」', '「如果不这样，会怎么样？」'],
      ...
      ('p', '右边这五句不是话术。它们的共同点是：'
            '<b>把说话的权利还回去</b>——先有学生开口，老师才有可改的下一句，'
            '全场才可能一起悬着。三样是这么被启动的。'),
      ('h3', '二、改自己下周那节真课，只改三处'),
      ('ol', ['<b>哪一刻让他押一个说法</b>——学生那一样从这里长；',
              '<b>他卡住时你说哪一句</b>——从上面那张表里挑，写下来；',
              '<b>下课前那一问怎么问</b>——照抄也行。']),
      ('note', '不重写教案，不做课件，不换环节。'
               '只改这三处——<b>改得动的东西越少，回去越用得上。</b>'),
      ('h3', '三、两两试讲，只回答三句'),
      ('big', '听的人不评教态、不评环节、不评课件，只回答三句：<br>'
              '<b>①你改了几次下一句？</b>（老师长了没有）<br>'
              '<b>②有几个人押了说法？</b>（学生长了没有）<br>'
              '<b>③悬着的时间加起来多久？</b>（教室长了没有）<br>'
              '越界的当场打断。'),
      ('h3', '四、三个数，怎么带回去'),
      ('p', '就是上午那三个记号，一样一个。三个都不用考试、不用等，当堂就有；'
            '每周记一次，一个学期就是三条看得见的曲线。'
            '<b>哪一条平着不动，就知道该补哪一样。</b>'),
      ('h3', '五、开学以后：怎么不靠热情把它接着做下去'),
      ('p', '三天散会，热情大概能撑两周。要它撑一个学期，只有一个办法：'
            '<b>把这三个数放进教研组本来就在做的事里</b>，'
            '不新增会议、不新增表格。'),
      ('card e', '两处最省力的接口',
       '<b>听课单加三栏</b>：把原来那些评教态、评环节的格子留着不动，'
       '末尾加「老师改了几次」「几个人押了说法」「悬着多久」三栏。'
  ...
  take=['一句话：老师、学生、教室都不是现成的，是这节课做出来的',
        '三样各自出现的那一刻，以及当堂能看的三个记号',
        '四种上砸的课，各是哪一样没长出来',
        '一张课堂用语对照表，明天就能用',
        '一节改好的真课——只改了三处，回去就能上',
        '三个数，和把它们放进教研组的两个接口'],
...
def build_index():
    ...
    return '\n'.join([
        ...
        '<td>先上一节课给你们看，再拆开它'
        '<br><span class="slot">老师 · 学生 · 教室：三样是怎么轮着长的</span></td>'
        '<td>改一节自己的真课，带走三个数'
        '<br><span class="slot">只改三处 · 试讲只答三句 · 开学后怎么接着做</span></td></tr>'
        '</table>',
        '<p class="slot">每一天都有<b>逐环节的时间安排</b>（上午 5 段、下午 6 段，'
        '分钟数已排到位）、现场要准备的东西，以及可以先读的站内文章——点开上面三张卡。</p>',
        '<h2>给谁</h2>',
        '<p>现场是重庆巴渝学校小学部<b>全体教师</b>，<b>不分学科</b>——'
        '语文、数学、英语、音乐、美术、体育、科学都在内。'
        '这是学校的内部教师培训；<b>本栏只放课程内容</b>。</p>',
        '<div class="note">三天<b>一个新名词都不用记</b>。'
        '所有内容都用日常话讲，举的例子都是小学课堂上真会发生的事。'
        '听不懂的地方，当场就该打断。</div>',
        # ⚠ 王德生 2026-08-22 两次定下：**报名、线上观摩、收费、赠书、退费一概不写在站上**，
        #   随后又要求**连承办人的名字一并去掉**。原先这里有一张条款表与一节「线上观摩」，
        #   都已整块撤掉；页脚只剩主讲人。
