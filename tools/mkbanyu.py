...
'day2': dict(
  ...
  blocks=[
    ('上午 · 9:30–11:30　课本是怎么被磨平的', [
      ...
      ('p', '课本要在四十分钟里讲清楚一件人类花了几百年才定下来的事，'
            '只能压缩。压缩的办法只有两个：<b>把过程去掉、只留结论</b>，'
            '再<b>按最省事的顺序排好</b>。'),
      ('big', '<b>这是课本的本分，不是课本的毛病。</b>'
              '毛病出在我们照着这个顺序讲——'
              '课本是给<b>已经会了的人</b>看的一份摘要，'
              '我们却拿它当<b>还不会的人</b>的行军路线。'),
      ('h3', '二、一页课本上，哪些是结论，哪些是路'),
      ('p', '现场投影一页课本，当场翻，不预先挑。逐句问三件事：'
            '这一句是结论还是路？这个顺序是唯一的顺序吗？'
            '被压掉的那一段，还找得回来吗？'),
      ('card', '一眼能看出来的记号',
       '凡是出现「我们规定」「叫作」「记作」「因此」的地方，'
       '几乎都是一处被压掉的岔路口——那里当年有过别的选法，'
       '课本替我们选完了，并且没说它选过。'),
      ('h3', '三、还原课本的三句话（全学科通用）'),
      ('table', [['要问的一句', '它在做什么'],
      ...
      ('p', '这三句话跟学科无关。语文、数学、英语、音乐、美术、体育、科学都能用。'
            '难的不是记住它们，难的是<b>问完之后忍住不替他回答</b>。'),
      ('h3', '四、为什么补课常常没用'),
      ('big', '有些孩子不是没听懂，是心里那条规矩<b>已经上了锁</b>——'
              '「这东西本来就是这样，不用想」。'
              '锁不打开，你把当年的路原样还给他，他也走不进去。'
              '<b>所以次序是硬的：先开锁，再还路。</b>'),
      ('p', '开锁用的就是上面那三句话。它们看起来太简单，'
            '简单到常被当成课堂上的调剂——其实这是全场最难的一个动作，'
            '因为它要求老师<b>当着全班承认：这件事本来可以不是这样。</b>'),
      ('note', '一个自查：你上一次在课堂上说出「这里我们其实可以不这么规定」，'
               '是什么时候？'),
    ]),
    ('下午 · 14:00–16:30　各科各做一个：本科知识是怎么长出来的', [
      ('p', '上午拆的是课本这本书，下午拆的是<b>你这一科的知识本身</b>。'
            '分组，每一科挑<b>下周就要上的那一节真课</b>，'
            '用上午那三句话现场做一遍，做完当场讲给别科的老师听。'),
      ('big', '判准只有一句：<b>别科的老师听不懂，就是还没拆开。</b>'
              '拆开了的东西，一定能用日常话说清楚；'
              '还得靠本科术语撑着的，说明你还站在结论那一侧。'),
      ('h3', '任务卡四行，散会前要填满'),
      ('ol', ['这一课当年有过<b>哪条岔路</b>；',
              '孩子心里原来那本账<b>写的是什么</b>；',
              '这一课靠什么<b>把那本账改掉</b>；',
              '如果没改掉，<b>明天在什么地方会塌</b>。']),
      ('note', '第四行是最有用的一行。它把「这节课上得好不好」'
               '换成了一个下周就能验的具体预言。'),
      ('h3', '几个已经拆过的，供各组参考'),
      ('table', [['科目', '拆过的地方', '孩子心里原来那本账'],
                 ['数学', '位值：为什么要进位', '「数字就是一个一个往上排的」'],
                 ['数学', '等号：它是「算出来是」还是「两边一样」', '「等号后面写答案」'],
                 ['语文', '一篇作文的第一句是怎么起来的', '「开头要写得漂亮」'],
                 ['英语', '一个词是用会的，不是背会的', '「记住意思就算会了」'],
                 ['科学', '一条结论当年被哪些说法顶过', '「书上写的就是对的」'],
                 ['音乐 · 美术', '一段旋律、一张画，在哪一刻成立', '「好看就是好」']]),
      ('note', '这些只是参考。真正要拆的是<b>你们班上周刚讲砸的那一课</b>。'),
      ('h3', '一组做完的验收'),
      ('ul', ['四行卡填满了；',
              '别科老师听懂了；',
              '说得出这一课在哪一刻孩子会被顶一下；',
  ...
  take=['三句还原课本的话，全学科通用',
        '一条硬次序：先开锁，再还路',
        '本组一节真课的完整拆解，四行卡填满',
        '一个下周就能验的预言：没改掉的话，会在哪里塌'],
),

'day3': dict(
  tag='一节课，究竟是在哪一刻发生的',
  keys=['上午 9:30–11:30', '下午 14:00–16:30', '当场备一节真课',
        '专著《SDE课堂发生学》'],
  ...
  blocks=[
    ('上午 · 9:30–11:30　四种「没发生」，长什么样', [
      ...
      ('table', [['样子', '实际发生了什么', '该怎么补'],
                 ...
                 ['怎么讲都进不去', '心里那条规矩上了锁', '先开锁（昨天那三句话），再讲']]),
      ('big', '次序上有一条硬规矩：<b>先查是不是上了锁。</b>'
              '锁着的时候，另外三种补救全部无效——'
              '这是我们最常浪费掉的一整节课。'),
      ('h3', '老师站在哪里'),
      ('p', '这一节课里，老师的位置不是<b>知道答案的那个人</b>，'
            '是<b>守住那个还没定下来的地方、不让它提前关上的那个人</b>。'
            '这句话听起来轻，做起来是全场最难的一件事。'),
      ('h3', '一节课里，只有三个位置要紧'),
      ('table', [['位置', '这一刻要做的'],
                 ['开口之前', '先让他押一个说法——哪怕是错的。没有押注，后面全是听讲'],
                 ['卡住的那三十秒', '什么都不做。这三十秒是这节课最贵的三十秒'],
                 ['下课之前', '问那一问（见下午）。不问，这节课就没有读数']]),
      ('note', '其余的三十几分钟怎么安排，各人有各人的办法，不必统一。'
               '这三个位置错了，别的再好也白搭。'),
    ]),
    ('下午 · 14:00–16:30　当场备一节真课', [
      ('p', '下午每人拿出<b>下周要上的一节真课</b>（自己的，不是本组的），'
            '现场改，改完互相试讲十分钟。'),
      ('h3', '课堂用语：不要说 / 要说'),
      ('table', [['不要说', '改成'],
                 ['「记住，就是这样。」', '「如果不这样，会怎么样？」'],
