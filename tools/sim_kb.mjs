...
sec("⑥ 社区页");
{
  ...
  /* ⚠ 断言只判「kb 在不在」，**不许钉顺序、不许钉尾巴、不许钉相邻元素**。
     这已经是同一个坑第三次：sim_home 钉过顺序、这份钉过尾巴（`"lib","kb"]`），
     这次是钉相邻（`v==="kb"||v==="who"`）—— 并发线在 kb 后面插了个 "key"，
     两条当场变红，而功能一点问题都没有。
     show() 的白名单必然被后续功能插队，钉位置的断言迟早自己红给你看。 */
  {
    const arr = /\[((?:"[a-z]+",?)+)\]\.forEach/.exec(H);
    ok(!!arr, "找不到视图白名单数组");
    ok(!!arr && /"kb"/.test(arr[1]), "白名单·视图切换 漏了 kb");
  }
  ok(/v==="kb"(\||\))/.test(H), "白名单·返回键 漏了 kb");
