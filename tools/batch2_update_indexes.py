...
def insert_profile(sl, ps):
    ...
    t = path.read_text(encoding="utf-8")
    anchor = None
    for pat in (r'<a class="works-entry"[^>]*>', r'<a class="works"[^>]*>', r'<div class="works">'):
        m = re.search(pat, t)
        if m:
...
def main():
    papers = json.loads((ROOT / "tools" / "batch2_report.json").read_text(encoding="utf-8"))["papers"]
    groups = {}
    for p in papers:
