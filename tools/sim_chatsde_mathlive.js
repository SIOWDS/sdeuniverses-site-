...
/* ── 二之半 · \text{中文} 例外（2026-09-08）──
   汉字闸原本一见汉字就拦，代价是量具名排不出来：立题卡里几乎都写成
   $\text{拒收率}R_x$、$\text{增量}R^2$。改成先挖空 \text{…}\mathrm{…} 这类
   文本命令的花括号再查汉字 ⇒ 命令内的中文放行，命令外的中文照旧拦。 */
[
  ["\\text{中文}+下标", "拒收率写作 $\\text{拒收率}R_x$，逐步加邻证信息。", "\\text{拒收率}R_x"],
  ["\\text{中文}+上标", "读数是 $\\text{增量}R^2$ 而非总量。", "\\text{增量}R^2"],
  ["\\mathrm{中文}", "均值记为 $\\mathrm{代理}P_r$。", "\\mathrm{代理}P_r"],
  ["两段 \\text 夹符号", "定义 $\\text{拒收}=\\text{撤销}/N$。", "\\text{拒收}"],
  ["命令与纯 ASCII 混排", "控制后 $\\text{增量}\\Delta AUC \\ge 0.02$ 才算。", "\\Delta AUC"],
].forEach(([n, md, tex]) => ok("\\text 例外 · " + n, has(md, tex)));

/* 挖的是花括号内容，不是整条 ⇒ 命令外还有中文时仍须拦住 */
[
  ["命令外有中文（仍拦）", "写成 $\\text{增量}R^2 与 y$ 两项。", "与 y"],
  ["无命令的纯中文（仍拦）", "他花了 $5 买咖啡；变量 A$B 与报价。", "5 买咖啡"],
  ["假命令名不放行", "记作 $\\zhongwen{增量}R$ 如此。", "增量"],
].forEach(([n, md, bad]) => ok("\\text 例外不越界 · " + n, !has(md, bad)));

/* 块级从来不受汉字闸限制，别因为这次改动把它弄反了 */
ok("\\text 例外 · 块级中文公式照旧", has("$$\\text{拒收率}R_x = \\frac{撤销}{N}$$", "\\text{拒收率}"));

/* 源码级：挖空必须发生在汉字判定之前，且只认白名单命令 */
ok("\\text 例外 · 汉字闸走 texStripText()", /test\(texStripText\(c\)\)/.test(SRC));
ok("\\text 例外 · 白名单里有 text 与 mathrm", /TEX_TXTCMD\s*=\s*\/[^\n]*\btext\b[^\n]*\bmathrm\b/.test(SRC));
