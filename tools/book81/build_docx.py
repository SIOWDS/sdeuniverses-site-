...
def add_header():
    ...
    setfont(p.add_run('答 案 随 时 可 得 之 后'), SANS, 8, False, ACC2)
    pPr = p._p.get_or_add_pPr()
    pbdr = OxmlElement('w:pBdr')
    bot = OxmlElement('w:bottom')
...
# ---------- 扉页 ----------
para('', after=105)
para('答案随时可得之后', size=30, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0,
     color=ACC, spacing=1.2)
para('论知识不是存量，及重新推出它的那一段为何不入账', size=12, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, before=14, color=ACC2)
para('', after=88)
...
for s in ['本书十章原为「学科通融」专栏已发表长文，编号为之三十一、之四、之十五、之九十一、之三十七、之九十二、之十九、之二十七、之六十四、之五，发表于二〇二六年七至八月，作者王德生＋Claude。装书改动限于三处：篇内自称由「本文」改为「本章」；对本书之外专栏文章的引用改为「学科通融专栏之N」；各章末参考文献抽出合并为书末分章文献表。正文内容未作删改。',
          '前言、导读、导论、四篇编序、枢纽章、合章、结语、三附录与后记为装书时新写，约二万字。按本栏规程，参与写作者不得为本书出具创新智商认证分；盲评待未参与写作的一方独立完成后公布。',
