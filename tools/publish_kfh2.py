# -*- coding: utf-8 -*-
"""孔凡鹤四篇（心理治疗 · 二次审稿打磨版） · 建页 + PDF + 三种读法。

用法： python3 tools/publish_kfh2.py --src /home/claude/kfh2
<head> 从孔凡鹤自己的既有论文页复制，主题天然一致（暗金）。
...
from kfh2_meta import PAPERS, STUDENTS, PUBDATE_CN
from kfh2_supp import NOTES, REFERENCES
...
RENAME = [
    # 招牌词 → 心理治疗／临床研究的本地说法（长词在前，避免被短词吃掉）
    ("心理治疗发生学框架内构造", "心理治疗改变机制框架内构造"),
    ("发生学本体论", "改变机制的存在论"),
    ("发生学重判", "机制重判"),
    ("发生学本体", "改变的生成本体"),
    ("发生学术体", "生成实体"),
    ...
    ("发现学最阴险的复辟形式", "把结论当起点这种思路最阴险的复辟形式"),
    ("发现学的旧路", "定义先行的旧路"),
    ("发现学", "定义先行的思路"),
    ("发生论", "生成论"),
    # 其余招牌词
...
# (源篇号, 类型, 锚, 替换)
CLEANUP = [
    ("Q2", "drop", "——对心理治疗整合运动中一个根本预设的再审视", ""),
    ("Q3", "drop", "——论心理治疗起效的发生学本体，兼论", ""),
]
...
<p>本批四篇评分排名第 {paper["rank"]}。打磨走外科手术式定点修改，只动审稿记下的扣分点——跨领域形式检验、不可还原性的重心迁移、正向跨域预测、证伪协议的可预注册化——作者的核心判断与章节结构一律不动。提升后分数为编辑自评，待独立复评。</p></div>
{''.join(body)}
...
def main():
    ...
    (ROOT / "tools" / "kfh2_report.json").write_text(json.dumps(
