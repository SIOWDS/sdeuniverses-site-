...
/* ⑫⑬⑭ v3.8 跟改三条（敌意拓宽五方向／可引原句闸／单一口径律） */
step("⑫ v3.8：敌意拓宽·占位者搜捕（唯一直接抬不可还原性的一道）", () => {
  ok(s.includes("7.6.4之二 敌意拓宽"), "7.6.4之二 节在");
  ok(s.includes("经典层(1980 年前)") && s.includes("实务正主") && s.includes("方法学占位者")
     && s.includes("专著与非期刊层") && s.includes("自家已发的"), "五个方向齐");
  ok(/≥2 位来自 1980 年前[,，]\s*≥2 位来自应用领域内部/.test(s), "两个硬配额：1980 前 ≥2、实务正主 ≥2");
  ok(s.includes("合计 ≥8 位") || s.includes("合计:__ 位(<8 → 不算过)"), "合计 ≥8 位");
  ok(s.includes("必须写出检索词") && s.includes("未核验"), "必须写出检索词，未附即标未核验");
  ok(s.includes("剥到形式层"), "剥到形式层这个动作写了");
  ok(s.includes("汇聚系列") && s.includes("唯一不许的是绕过它不提"), "三条活路，汇聚系列排第一");
  ok(s.includes("同名同指") && s.includes("同名反指") && s.includes("读数名同形"), "命名闸三档齐");
  ok(/同名反指[\s\S]{0,120}必须改名/.test(s), "同名反指判为最严重且必须改名");
});

step("⑬ v3.8：可引原句闸（选源第五道判据）", () => {
  ok(s.includes("7.6.1之二 第五道判据"), "7.6.1之二 节在");
  ok(s.includes("必须是能引到原文的一句") && s.includes("不许由你代写"), "核心判断句须可引到原文、禁代拟");
  ok(/一处坏就退回[,，]\s*两处坏整篇作废/.test(s), "坏引注一处退回、两处作废");
  ok(s.includes("示意性转写") && s.includes("中间那种是致命的"), "示意性转写被点名为致命");
  ok(s.indexOf("7.6.1之二 第五道判据") > 0 &&
     s.indexOf("7.6.1之二 第五道判据") < s.indexOf("7.6.2 定型的做法"), "排在定型之前（选源期就要过）");
});

step("⑭ v3.8：单一口径律与真跑", () => {
  ok(s.includes("7.6.4之三 单一口径律"), "7.6.4之三 节在");
  ok(/正文、证伪条款、赌注三处必须引用同一个定义/.test(s), "三处同一定义同一阈值");
  ok(s.includes("读数名:") && s.includes("粒度:") && s.includes("编码规则") && s.includes("不适用:"), "清点手册各栏齐");
  ok(s.includes("本篇三处引用一致性自查"), "三处引用一致性自查在");
  ok(s.includes("尤其是不利结果"), "真跑：不利结果必须进正文");
  ok(s.includes("跑不动也算过"), "跑不动也算过，但要写清原因");
});
