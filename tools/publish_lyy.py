...
from lyy_meta import PAPERS, STUDENT, PUBDATE_CN
...
def load_source(src: Path, sid: str):
    lines = [re.sub(r"[ \t]+", " ", x).strip()
             for x in (src / f"刘言言_{sid}.txt").read_text(encoding="utf-8").splitlines()]
...
EXTRA_CSS = """
.scorebox{border:1px solid rgba(138,90,68,0.42);border-left:3px solid #8A5A44;padding:16px 22px;margin:22px 0;font-size:15px;border-radius:8px}
.scorebox b{color:#8A5A44}
.scorebox p{margin:6px 0 0;font-size:14px;opacity:.85}
.ref{font-size:14.5px;padding-left:2em;text-indent:-2em;opacity:.86}
.endbox{text-align:center;border-top:1px solid rgba(138,90,68,0.28);margin-top:48px;padding:38px 20px;opacity:.9}
"""
...
PRINT_CSS = """
@page{size:A4}
body{font-family:"Noto Serif CJK SC","Noto Serif SC",serif;color:#1F1A16;font-size:10.5pt;line-height:1.85;margin:0}
.cover{text-align:center;padding-bottom:13pt;border-bottom:1.2pt solid #8A5A44;margin-bottom:16pt}
.eyebrow{color:#8A5A44;letter-spacing:.3em;font-size:7.8pt;margin-bottom:10pt}
h1{font-size:18.5pt;line-height:1.44;margin:0 0 8pt;color:#33261F}
.sub{font-size:10pt;color:#6F5B50;margin:0 auto 10pt;max-width:34em;line-height:1.7}
.by{font-size:9pt;color:#6F5B50}.by b{color:#8A5A44}
.abs{background:#F5EEE8;border-left:3pt solid #8A5A44;padding:11pt 13pt;margin:0 0 12pt;font-size:9.4pt;line-height:1.75;text-align:justify}
.abs .lb{letter-spacing:.32em;color:#33261F;font-weight:700}
.kw{font-size:9pt;color:#6F5B50;margin:0 0 16pt}
h2{font-size:13pt;color:#33261F;padding-left:8pt;border-left:3.5pt solid #8A5A44;margin:19pt 0 9pt;page-break-after:avoid}
p{text-indent:2em;text-align:justify;margin:0 0 8pt}
.ref{text-indent:-1.6em;padding-left:1.6em;font-size:8.8pt;color:#4A4034;margin:0 0 4pt;text-align:left}
"""
...
<style>html,body{{margin:0;height:100%;background:#2B211B}}
header{{height:56px;background:#33261F;display:flex;align-items:center;justify-content:space-between;padding:0 18px;font-family:"Songti SC","Noto Serif SC",serif;font-size:14px;border-bottom:1px solid rgba(138,90,68,0.4);color:#E4D8CE}}
header a{{color:#D8A98C;text-decoration:none}}
...
def main():
    ...
    for paper in PAPERS:
        ...
        print(f'  {sid} → {paper["slug"]:24s} {chars:>6d}字 {pages:>3d}页 '
              f'章节{heads:>3d} 文献{refs:>3d}条 之{cn(paper["num"])} IQ{paper["score"]}')

    (ROOT / "tools" / "lyy_report.json").write_text(json.dumps(
