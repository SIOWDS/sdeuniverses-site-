...
from hzy2_supp import SUPPLEMENTS, REFERENCES
...
STRIP = (
    re.compile(r"^提升·论文"),          # 投稿包内的分篇标记
    re.compile(r"思想拓展声明"),        # 发生器声明
    re.compile(r"金点子发生器"),
    re.compile(r"^——(一项关于|以全球大学排名)"),   # 与 meta 副标题重复的破折号行
)
...
def load_source(src: Path, sid: str):
    ...
    return [x for x in lines if x and not any(p.search(x) for p in STRIP)]
...
def main():
    ...
    args = ap.parse_args()
    src = Path(args.src)
    tmp = Path("/tmp/hzy2_print"); tmp.mkdir(exist_ok=True)
    css_cache = {}
    ...
    (ROOT / "tools" / "hzy2_report.json").write_text(json.dumps(
    ...
    print(f'\n十一篇建页完成 · 总 {sum(p["pages"] for p in PAPERS)} 页')
