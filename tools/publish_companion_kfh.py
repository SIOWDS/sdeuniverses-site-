...
# -*- coding: utf-8 -*-
"""
配套读物生成器（孔凡鹤批次专用；与共用的 publish_companion_student.py 分开，避免并发改同一文件）—— 给 /students/<author>/<slug>/ 的理论母文各配两篇长文：
  · explain.html   白话解释文（日常生活类比，人人能读懂）
  · practice.html  技术与方法实践文（步骤、判据、检查表、失败模式）
并在母文页「摘要」正下方（正文第一个小标题之前）插入两个入口按钮，不放文末。

用法：
  python3 tools/publish_companion_kfh.py <author> <slug>          # 生成两篇 + 插入入口
  python3 tools/publish_companion_kfh.py <author> <slug> --dry    # 只看不写

内容源：$COMPANION_CONTENT/<slug>.explain.txt / <slug>.practice.txt（默认 /home/claude/content）
格式：
  TITLE: 标题
  SUB: 副标题
  ABS: 摘要（一段）
  == 小标题
  正文段落（空行分段；行首 - 为要点列表；行首 > 为引述块；**粗体**）

与 tools/publish_companion.py 的区别：① 路径可带作者层；② 页面不署作者（用户指定）；
③ 入口条锚点统一落在「正文第一个非摘要 h2 之前」，兼容本站三种母文骨架。
"""
...
KIND = {
    "explain": dict(
        ...
        who="这一篇写给：读母文时被术语挡在门外、但想真正弄懂它在说什么的人。全篇不假设你读过任何理论书，也不要求你记住任何术语。",
    ),
    "practice": dict(
        label="技术与方法实践文", icon="🛠",
...
<div class="cmpn-bar">
  ...
  <div class="cmpn-g">
    <a href="/{rel}/explain.html"><b>🌱 白话解释文 · {ex_title}</b><span>不用术语，用日常生活的类比把本文讲一遍 · 约5000字</span></a>
    <a href="/{rel}/practice.html"><b>🛠 技术与方法实践文 · {pr_title}</b><span>把本文的判断落成步骤、判据与检查表 · 约5000字</span></a>
  </div>
</div>
<!-- /COMPANION-READS -->
"""
...
def nchars(secs):
    return sum(len(re.findall(r"[\u4e00-\u9fff]", "".join("".join(b) for _, b in s["blocks"]))) for s in secs)
...
def render_page(rel, kind, meta, secs, mother_title, mother_short, sib_title, author_home):
    k = KIND[kind]
...
<link rel="canonical" href="https://sdeuniverses.com/{rel}/{kind}.html">
<style>{css}</style>
</head>
<body>
<nav class="cnav"><div class="wrap">
  <a class="cnav-logo" href="/browse/">SDE Universes</a>
  <a class="cnav-back" href="/{rel}/">← 回母文《{esc(mother_short)}》</a>
</div></nav>
...
<header class="art"><div class="wrap">
  ...
  <div class="art-meta">SDE Universes · 配套读物 · 2026年8月 · 约 {n} 字</div>
  <div class="swap">
    <a href="/{rel}/"><b>← 母文 · {esc(mother_short)}</b><span>完整论证在这里</span></a>
    <a href="/{rel}/{other}.html"><b>{ok['icon']} {ok['label']} · {esc(sib_title)}</b><span>{ok['hint']}</span></a>
  </div>
</div></header>
...
<div class="endbox">
  ...
  <p style="margin-bottom:10px">这一篇只是入口。完整的论证、边界与反驳都在母文里：<a href="/{rel}/">《{esc(mother_title)}》</a>。</p>
  <p style="margin:0">另一条路：<a href="/{rel}/{other}.html">{ok['icon']} {ok['label']}《{esc(sib_title)}》</a>——{ok['hint']}。</p>
</div>
</article>
...
def find_anchor(h):
    """插入点＝正文第一个非「摘要/关键词」小标题之前。回退：摘要盒之后 → 文章头之后。"""
    for m in re.finditer(r"<h2\b[^>]*>(.*?)</h2>", h, re.S):
        t = re.sub(r"<[^>]+>", "", m.group(1)).replace(" ", "").replace("\u3000", "").strip()
        if t in ("摘要", "关键词", "Abstract", "abstract"):
            continue
        return m.start(), f"正文首个小标题《{t[:18]}》之前"
    m = re.search(r'<div[^>]*class="[^"]*(?:abs|abstract|absbox|deck)\b[^"]*"', h)
    if m:
        depth = 0
...
def main():
    ...
    dry = "--dry" in sys.argv
    author, slug = args[0], args[1]
    rel = f"students/{author}/{slug}"
    mdir = os.path.join(PUB, "students", author, slug)
    mpath = os.path.join(mdir, "index.html")
    assert os.path.isfile(mpath), f"找不到母文 {mpath}"
    author_home = f"/students/{author}/works/" if os.path.isdir(os.path.join(PUB, "students", author, "works")) else f"/students/{author}/"

    h, mtitle = mother_meta(mpath)
    short = re.split(r"[：:—]", mtitle)[0].strip()

    pages = {}
    for kind in ("explain", "practice"):
        pages[kind] = parse(os.path.join(CONTENT_DIR, f"{slug}.{kind}.txt"))

    ok = True
    for kind in ("explain", "practice"):
        meta, secs = pages[kind]
        sib = pages["practice" if kind == "explain" else "explain"][0]["TITLE"]
        out = render_page(rel, kind, meta, secs, mtitle, short, sib, author_home)
        n = nchars(secs)
        flag = "✅" if n >= 4800 else ("⚠ 偏短" if n >= 4400 else "❌ 太短")
        if n < 4800:
            ok = False
        print(f"  {kind:9s} {n:5d}字  {len(secs):2d}节  {flag}")
        if not dry:
            open(os.path.join(mdir, f"{kind}.html"), "w", encoding="utf-8").write(out)
    ...
    bar = BAR_TPL.format(style=BAR_STYLE, rel=rel,
