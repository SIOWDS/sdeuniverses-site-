...
ok(B1.sys.includes("【本段的形态】") && B1.sys.includes("挂 M2.3.1"),
  "第一段：分离点要挂到具体节点编号上");
ok(B2.sys.includes("沿用第一段的节点编号"),
  "第二段：编号贯通（另起一套＝下游按编号取料时取空）");
ok(B1.sys.includes("本场岔出、与《缘起之问》接不上的内容"),
...
ok(B0.MAXTOK === 12000 && B1.MAXTOK === 32000 && B2.MAXTOK === 32000,
  "三刀预算 12000／32000／32000 · 实得 " + [B0.MAXTOK, B1.MAXTOK, B2.MAXTOK].join("/"));
ok(B1.sys.includes("〔第一段完·待续〕") && B2.sys.includes("〔全文完〕"),
  "两段各自的收尾标记都在（〔全文完〕必须落在最后那一段上——早一段就等于告诉基底写完了）");
/* 收短后九栏全在第一段，第二段只写〔十〕。 */
ok(B1.sys.includes("四、反复被触到的分离点") && B1.sys.includes("五、敌意最近邻清单")
  && B1.sys.includes("九、明确不写什么"), "九栏全部落在第一段（并段之后一栏都不许掉）");
ok(B2.sys.includes("十、论文观点与分章大纲") && !B2.sys.includes("五、敌意最近邻清单"),
  "第二段只写第十栏，不重复九栏");
