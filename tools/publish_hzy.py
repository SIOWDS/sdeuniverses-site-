...
from hzy_supp import SUPPLEMENTS, REFERENCES

ROOT = Path(__file__).resolve().parents[1]
STU = ROOT / "public" / "students"
FALLBACK_SKELETON = ("hu-zhiying", "metabolic-polarity")   # 新学员借这套骨架
CN = "零一二三四五六七八九十"
...
# ("源篇号", 类型, 锚, 替换)
CLEANUP = [
    # P03 "三界支撑"与"现实界/理念界/自我界"是母体口音，换为学科本地说法
    ("P03", "sub", "共同构成了构成性语法的“三界支撑”：在现实界（实践操作的效率）、理念界（追问格式的语法可及性）、自我界（哲学工作者的身份认同）三个层面",
     "共同构成了构成性语法的三重支撑：在操作层面（实践的效率）、语言层面（追问格式的语法可及性）、身份层面（哲学工作者的自我认同）三个层面"),
    ("P03", "sub", "它是一个被三界厚度共同支撑着的", "它是一个被这三重厚度共同支撑着的"),
]
...
<p>本批 26 篇评分排名第 {paper["rank"]}。提升集中于补上被放跑的最近邻理论、核心命题的操作化与可执行的证伪设计。提升后分数为编辑自评，待独立复评。</p></div>
{''.join(body)}
...
def main():
    ...
    args = ap.parse_args()
    src = Path(args.src)
    tmp = Path("/tmp/hzy_print"); tmp.mkdir(exist_ok=True)
    css_cache = {}
    ...
    for paper in PAPERS:
        ...
        print(f'  第{paper["rank"]:>2d} {sid}→{sl}/{paper["slug"]:24s} '
              f'{chars:>6d}字 {pages:>3d}页 作者文献{refs:>3d}条 {numlabel(sl, paper["num"])}')

    (ROOT / "tools" / "hzy_report.json").write_text(json.dumps(
    ...
    print(f'\n十五篇建页完成 · 总 {sum(p["pages"] for p in PAPERS)} 页')
