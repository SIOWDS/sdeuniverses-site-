...
PAPERS = [{
    "slug": "someone-always-starts-it",
    "src": Path("/home/claude/conf56.md"),
    "title": "总有人替它起步",
    "sub": "论学习产物要发起下一轮需要一段未被归类的成熟时间，及为什么外部启动越可靠，这条通路越无法被观察到",
    "cross": "教育学 × 化学反应动力学 × 管理学",
    "no": "之六十",
    "deck": ("课堂当轮生成了正确答案、概念修正与高参与度，而这些产物很少成为下一轮的原因。"
             "对「下一轮该由谁发起」，三家给出三条不能同时为真的答案：教育学答由教师依诊断发起、且反馈越及时越好；"
             "管理学答由制度发起，因为下放必被公开指标做成表演；反应动力学答由产物发起，"
             "因为不回到条件位的产物只有局部转化、没有传播。三家共有一个没说出口的前提——"
             "一个产物在它被生成的那一轮就已经具备或不具备接手下一轮的资格。"
             "推翻它的材料来自反应动力学自己：**自催化的诱导期**。产物积累到阈值之前速率几乎观察不到，"
             "**资格不是产物一生成就有的属性，是一个需要时间达到的状态**；"
             "而在诱导期之内测速率，得到的结论是「这个反应不发生」，可它正在发生。"
             "带回课堂，结论是刺的：**课堂的一切结算都发生在诱导期之内。**"
             "由此给出两个时间量（产物被判定之前的未归类窗口、产物长到能被接手所需的成熟时间）"
             "与两个速率，推出五条结论，其中三条方向违反通行判断："
             "**任何缩短未归类窗口的改进都单调降低续发率**——而即时反馈、自动批改、实时正确率、即时解答"
             "恰是教育技术二十年的主要方向；**外部启动越可靠，这条通路越不显形**，"
             "不是它被损害，是底物在它显形之前已被另一条通路消耗完；"
             "以及一条可用现有元分析数据直接检验的：**教学法标签不含这个变量，"
             "故同一种教学法内部的差异可能大于不同教学法之间的差异**。"
             "文中另论证形成性评价同时持有「证据必须改变教学」与「反馈应当及时」两条处方，"
             "在此条件下不能同时成立，且它现有的出路是循环的；"
             "并从化学借来一条少见的**正面判据**——加料实验。"),
    "clash": ("第一家主张下一轮应由教师依据诊断发起，因为最小指导对新手无效，"
              "且反馈应当及时、具体、指向下一步；第二家主张应由制度发起，因为协调、覆盖与责任不可下放，"
              "而任何把发起权交给现场的做法都会被公开度量的反应性做成表演；"
              "第三家主张应由产物发起，因为自催化物提供的是一条活化能更低的替代成产通路，"
              "不让产物回到入口，体系每一轮都得由外部重新点火。三条不能同时为真，"
              "且各自的日课正是另一家诊断出的病：教师依诊断作的调整会被评价系统在下一轮压回原计划，"
              "而「及时反馈」恰恰是把产物尽快归类的机制（管理判教育）；"
              "永远由制度发起的体系没有自催化，它每一轮所需的外部投入不随轮次下降（化学判管理）；"
              "把发起权交给一个尚未成形的产物，对新手就是最小指导（教育判化学）。"),
    "sources": [
        ("教育学 · 形成性评价、及时反馈与对最小指导的批评",
         "Black &amp; Wiliam, Assessment and Classroom Learning (<i>Assessment in Education</i> 5:1, 1998)；"
         "另见 Hattie &amp; Timperley, The Power of Feedback (<i>Review of Educational Research</i> 77:1, 2007)；"
         "Kirschner, Sweller &amp; Clark (<i>Educational Psychologist</i> 41:2, 2006)",
         "https://doi.org/10.1080/0969595980050102",
         "主张证据必须改变后续教学，而调整权驻于教师；同一支又主张反馈应当及时、具体、指向下一步。"
         "两条核心处方在同一套操作里互相削减——及时的反馈正是把产物尽快放进类目的那个动作。"),
        ("化学 · 自催化的动力学签名与诱导期",
         "Hanopolskyi, Smaliak, Novichkov &amp; Semenov, Autocatalysis: Kinetics, Mechanisms and Design "
         "(<i>ChemSystemsChem</i> 3:1, 2021)；另见 Bissette &amp; Fletcher, Mechanisms of Autocatalysis "
         "(<i>Angew. Chem. Int. Ed.</i> 52:49, 2013)；Bentea, Watzky &amp; Finke "
         "(<i>J. Phys. Chem. C</i> 121:9, 2017)",
         "https://doi.org/10.1002/syst.202000026",
         "这一支写明自催化由**动力学签名**定义而非由机理定义，且滞后段本身不构成证据——"
         "中间体的积累同样会造成滞后段，区分二者必须做加料实验。前一条否掉了一整类基于形态的课堂判断，"
         "后一条既给出本文唯一的正面判据，也给出与本栏一篇先行研究的判决性分离。"),
        ("管理学 · 组织例程与公开度量的反应性",
         ...
         "Feldman &amp; Pentland (<i>Administrative Science Quarterly</i> 48:1, 2003)",
         "https://doi.org/10.1086/517897",
         "主张例程是能力得以跨年度保存的方式，而公开度量会反过来重塑被度量者，"
         "故把发起权交给现场不可能稳定。这一支自曝的一处是：同一条反应性论证，"
         "同样适用于它自己以及本文所提出的每一个新读数。"),
    ],
}]
