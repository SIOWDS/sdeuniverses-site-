...
CLEANUP = [
    ...
    ("Q2", "sub", "此后的全部论证都成了一个现成定义的展开，那是发现学。",
     "此后的全部论证都成了一个现成定义的展开——那是把结论当起点。"),
    ("Q3", "sub", "这是发现学的操作——解法预先存在，只是尚未被揭示。",
     "这是一种以既成事实为前提的操作——解法预先存在，只是尚未被揭示。"),
    ("Q3", "sub", "但这个叙事本身的预设仍然是发现学的：",
     "但这个叙事本身的预设仍然是既成事实式的："),
    ("Q4", "sub", "那么本文就滑回了它所批判的发现学：",
     "那么本文就滑回了它所批判的那种预设——把有待发生的东西当作早已存在、只是被遮蔽："),
    # Q2 证伪条件末尾那段留了「即便被证伪概念仍存活」的退路，使条件失效；增补二已给出收束后的版本
    # 证伪条件末尾这一段主张「即便被证伪、概念仍以另一身份存活」，使刚立好的条件不再构成风险，
    # 故整段删除（它在段中而非独占一行，用 sub 精确切除）。收束后的版本见增补二。
    ("Q2", "sub",
     "本文的预判是：这一区分将始终存在，因为被注入的满足——即使它在现象学上完美复制了“我做的”的感觉——"
     "它的发生史中没有一个真实的、由接受者亲自走过那条感知之路的历史。而“强制开荒”所结算的主体性实有，"
     "其真实性恰恰不只在体验的质地上，更在体验的发生学来源上：它是一次真实劳作的真实记忆。"
     "如果一个技术可以连这个记忆也植入，那么它将不是在模拟强制开荒，而是在生产一个新的、"
     "与强制开荒在现象学上不可区分、但在本体论上截然不同的事件。本文的命名，仍将为区分这两个事件提供必不可少的辨别维度。",
     ""),
]
...
<p>本批剩余稿评分排名第 {paper["rank"]}。提升集中于补上被放跑的最近邻理论、核心命题的操作化与可执行的证伪设计。提升后分数为编辑自评，待独立复评。</p></div>
{''.join(body)}
...
def main():
    ...
    (ROOT / "tools" / "qinli3_report.json").write_text(json.dumps(
