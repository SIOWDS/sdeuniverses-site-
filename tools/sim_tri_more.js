...
console.log("\n[二·补 2026-09-04] 条款要求做的事，代码得给材料");
ok(/rd > 1 && carry[\s\S]{0,120}上一轮的结算（逐字/.test(C), "第三席在续轮真的拿到上一轮的结算 —— 此前它被要求拿本轮跟上一轮比，而上一轮那句话根本没递给它");
ok(/末尾附着上一轮的结算原文/.test(W), "第三席的提示语明说材料末尾附着上一轮结算");
ok(/if \(rd > 1\) \{[\s\S]{0,900}return "IDLE";/.test(C) && /triIdle/.test(C), "续轮在第一席设闸：说不清差别落在哪个词、也不说撞到底，就当场停，不再跑二三席");
ok(/var spun = res && res\.verdict/.test(C) && /triSpun/.test(C), "结算席自报空转时不再给「继续对撞」——拿空转当靶子只会越撞越空");
ok(/【结算】/.test(W) && /【结算】\(\[\\s\\S\]\*\?\)/.test(C), "第④句用固定标记单独起行，续轮只把这一句当靶子（不再递五节全文）");

console.log("\n[二·补 B 2026-09-04] 第三席先量一句：两家站在同一维上吗");
ok(/先量一件事 · 两家站在同一维上吗/.test(W), "结算之前先做维度判定 —— 三家对撞撞的是厂商不是维度，这把尺子是唯一的补救");
ok(/显露/.test(W) && /路径/.test(W) && /纠缠/.test(W), "三样东西按改姓口径写（显露／路径／纠缠），不出现 S/D/E 字母");
ok(/各引它们的一句原话作凭据/.test(W), "维度指认要落到原话上，不许凭印象");
ok(/同维复述/.test(W) && /然后停——不要往下结算/.test(W), "同维复述当场停，不出结算 —— 同一副眼镜看同一处，共有前提只是眼镜本身");
ok(/全文不许出现「【结算】」这四个字/.test(W), "同维复述不许打结算标记（那个标记是给下一轮当靶子的）");
ok(/同维复述/.test(C), "前端把同维复述与空转同等对待，一样不给「继续对撞」");
...
ok(/var tgt = \(m && m\[1\]\.trim\(\)\)/.test(C) && /startRound\(nx, rd \+ 1, tgt\)/.test(C), "下一轮的靶子是上一轮的**结算句**（旧断言断的是「原样递全文」，2026-09-04 已改：递五节全文时第一席最易抓住的反而是复述那一段）");
