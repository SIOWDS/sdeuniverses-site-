...
console.log("②c 三个图标四周烧 TOKEN");
{
  ...
  // 入口页刻意**只烧一团柔火**：三个入口并排是门面，三团大火会压过三角形与「爱思乐园」。
  // 熊熊大火只留在回入口的 △ 上（见 sim_sde_modes.js / sim_wds_mode_v2.js）。
  ok(/@keyframes sdepFlick/.test(SRC), "火在明灭，不是一张静止的橙色贴纸");
  ok(!/sdepBurn/.test(SRC) && fires[0].querySelectorAll("b").length === 1,
    "入口页是单层柔火，不做三层火舌的大火（喧宾夺主）");
}
