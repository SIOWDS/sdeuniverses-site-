...
from qinli4_meta import PAPERS, STUDENTS, PUBDATE_CN
from qinli4_supp import NOTES, REFERENCES
...
CLEANUP = [
    ...
    ("Q1", "drop", "### ——「缺余」的发生论与艺术不可替代性的重释", ""),
    # 「发现学」是内部的「发生 vs 发现」对置，学科读者不解其意，逐句换成本地说法
    ("Q1", "sub", "那是发现学的操作。发生学要问的是：",
     "那是把结论当起点的操作。要问的是："),
    ("Q1", "sub", "那个解释是发现学的——", "那个解释是把结果当前提的——"),
    ("Q1", "sub",
     "[2] “发现学”是本文为与“发生学”对照而临时提出的概念，用以指称那种预设艺术的意义或效力如同某种"
     "已被埋藏之物，仅需被主体“发现”即可触发的理论倾向。发生学则追问该经验在具体身体、场域和条件中如何被生成。",
     "[2] 本文以“把结论当起点”指称那种理论倾向：它预设艺术的意义或效力如同某种已被埋藏之物，"
     "仅需被主体“发现”即可触发。与之相对，本文所做的追问是：该经验在具体的身体、场域与条件中如何被生成。"),
    ("Q4", "sub", "但这个回答本身是发现学的——", "但这个回答本身是把结论当起点的——"),
    ("Q4", "sub", "而不是一个需要被发生学追踪的历史产物。一个真正的发生学追问应该是：",
     "而不是一个需要在生成层面被追踪的历史产物。一个真正的生成层面的追问应该是："),
    ("Q4", "sub", "不是一个发现学的考古收获。", "不是一次考古学式的发现。"),
]
...
<p>本批四篇评分排名第 {paper["rank"]}。提升集中于补上被放跑的最近邻、把核心命题做跨域形式锚定、并把证伪条件从概念层落到经验层。提升后分数为编辑自评，待独立复评。</p></div>
{''.join(body)}
...
def main():
    ...
    (ROOT / "tools" / "qinli4_report.json").write_text(json.dumps(
