...
/* 这一条守的用意是「正文那一趟的时钟要配得上它要写的量」，不是那两个数字本身。
   2026-08-19 起按「这一家能不能关思考」分两档：关得掉 60/270，关不掉 120/600
   （思考与正文吃同一份时钟，270 秒在那两家身上每趟都被我方掐断）。 */
ok("正文那一趟的时钟按能否关思考分两档", /wdsClock\(_noPlain \? 120000 : 60000, _noPlain \? 600000 : 270000\)/.test(WSRC));
ok("★ 关不掉思考的那一支时限更长（≥600s）", /_noPlain \? 600000/.test(WSRC));
ok("_noPlain 由 wdsCanPlain 判，不是写死某一家", /const _noPlain = !wdsCanPlain\(VC\);/.test(WSRC));

/* 【拆趟机器不许要「交代职能」的开场】真跑里整节只剩一句
   「本节承担全文 I 维的承重职能：把核心命题放在最近邻的密集区域里逐一划界」——
   那句话是**我们自己在提示语里要的**。各节分开跑，交代职能既没有读者，
   又足以让基底认为这一趟已经交差。规范层与机器层两层都要改（只改一层是站内反复犯的病）。 */
const SKILL_SRC = require("fs").readFileSync(__dirname + "/skills/sde-academic-paper.md", "utf8");
ok("★ 机器层：禁止交代职能的开场", /禁止写「本节承担全文的承重职能」/.test(WSRC));
ok("★ 机器层：旧那句「先交代…承担什么职能」已拿掉", !/本节开头第一句先交代这一节在全文里承担什么职能/.test(WSRC));
ok("★ 规范层同步：也写着这条禁令", /禁止写「本章承担全文的承重职能」/.test(SKILL_SRC));
/* ⚠ 只认「行首的活规则」：改动说明里引用了原句（那是**记着为什么改**，不是仍在生效），
   按整句去数会被自己的说明喂成假红——站里第五次踩「锚到注释/说明里的串」。 */
ok("★ 规范层：旧那条已不再是生效的规则行", !/\n- 每一章开头第一句先交代/.test(SKILL_SRC));
