...
/* ⚠ 2026-08-21 契约翻面：末段是**交付件**（结论·注释·文献·附录·声明），不再是论证段，字数另算。
   旧判据要求每一段都写「4800–5400」，而末段的分工里同时写着「3200–4200 即可」——
   同一份提示语两个字数，基底选哪个都算它没听话。新口径：**每段有且只有一个字数区间**。 */
ALL.forEach((x, i) => {
  const nums = (x.sys.match(/(\d{4})–(\d{4}) 字/g) || []);
  ok(nums.length === 1, "第 " + (i + 1) + " 段有且只有一个字数区间 · 实得 " + JSON.stringify(nums));
});
ok(ALL.slice(0, ALL.length - 1).every((x) => x.sys.includes("写满约 4800–5400 字")),
  "论证段各约五千字——两万字靠加段数，不靠加长单段（加长必被平台时钟杀在思考阶段）");
ok(/写满约 3200–4200 字/.test(PLAST.sys), "末段（交付件）字数单独一档，比论证段短");
...
/* ⚠ 2026-08-21 契约翻面：越界的 part 从此**钳到末段**，不再回落第一段。
   旧口径正是「重复写了两篇」的成因——前端多要一段、服务端上限写小了，那一段被当成第一段，
   于是题名与摘要在第九章后面又出现一次。往下钳最多重复末段（读者一眼看得见），
   往回钳则安静地给稿子安上第二个开头。两头都不许发空规格。 */
ok(build(9).sys === PLAST.sys, "越界的 part 钳到末段（绝不回落第一段——那会给稿子安上第二个开头）");
ok(build(0).sys === P1.sys, "小于 1 的 part 钳到第一段，不发空规格");
...
ok(B1.sys.includes("它是清单与施工图，不是参考"), "入口资料被定性为必须逐条兑现的清单与施工图");
/* 【2026-08-13】新增第十栏〔论文观点与分章大纲〕。栏写了没人读＝纯浪费，所以成文那一侧
   必须真的把它当施工图用，并且冲突时要**说明为什么改**，不许悄悄绕开。 */
ok(B1.sys.includes("〔十、论文观点与分章大纲〕是**这篇论文的施工图**"), "成文把第十栏当施工图读");
ok(B1.sys.includes("必须在正文里说明为什么改"), "施工图与固定骨架冲突时要写明理由，不许悄悄绕开");
ok(B1.sys.includes("供料自查"), "第十栏末尾的〔供料自查〕漏项，成文要接手");
