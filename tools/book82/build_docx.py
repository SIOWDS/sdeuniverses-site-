...
SRC = '/home/claude/sl/manuscript.md'
OUT = '/home/claude/sl/out/山路会自己长回去.docx'
SERIF, SANS = 'Noto Serif CJK SC', 'Noto Sans CJK SC'
PAGEMAP = '/home/claude/sl/pagemap.json'
...
def add_header():
    ...
    setfont(p.add_run('山 路 会 自 己 长 回 去'), SANS, 8, False, ACC2)
    pPr = p._p.get_or_add_pPr()
    pbdr = OxmlElement('w:pBdr')
    bot = OxmlElement('w:bottom')
...
# ---------- 扉页 ----------
para('', after=105)
para('山路会自己长回去', size=30, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0,
     color=ACC, spacing=1.2)
para('十件必须由本人反复做才存在的事', size=12, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, before=14, color=ACC2)
para('', after=88)
para('王德生　＋　Claude', size=12, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, before=58)
para('据学员专栏十位作者的十篇论文编成', size=9, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, before=8, color=ACC2)
...
def is_chap(b): return bool(b.startswith('## '))
...
while i < len(blocks):
    ...
    if is_h1(b):
        ...
        para(b[5:].strip(), size=10, bold=True, indent=0, before=8, after=3, color=ACC2)
    elif is_h3(b):
        ...
        para(b.replace('\n', ''), after=8, indent=0, align=WD_ALIGN_PARAGRAPH.CENTER,
             size=10.5, color=ACC if b.startswith('山路会自己长回去') else None,
             bold=b.startswith('山路会自己长回去'), spacing=1.5)
    else:
        para(b.replace('\n', ''), after=1.5)
    i += 1
