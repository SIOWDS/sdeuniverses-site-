...
console.log("\n[二] 摘要端点：它是配菜，不许占正菜的档位与时钟");
{
  ...
  ok(/llmText\(VC, KEY, sys, usr, mode === "profile" \? 1800 : 1600, MEMO_MS\)/.test(seg), "预算按'它本来该写多长'给（1600/1800），并把短截止传进 llmText");
...
console.log("\n[五] 两端对齐与接线");
{
  ...
  ok(/if \(r\.code === "rate"\)/.test(PAGE), "撞上限流时**停下并说明**，不是闷头把剩下的全打一遍");
