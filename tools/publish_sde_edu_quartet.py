...
PAPERS = [
    ...
    dict(md='本来就是这样_教育是什么.md', slug='what-was-always-so',
         title='本来就是这样',
         sub='论一个传承体系必须把所传之物分成「不许改的」与「必须自己重做的」两份，而这次分派在它自己的记载里一律被写成发现',
         kw='判形；未判之域；判形滞后期；留造分配；传承体系；教育本体',
         desc='圣经学、佛学与道家共同假定：所传之物里哪一部分是它本身、哪一部分只是外壳，由它自己决定。推翻它的材料在圣经学自己手里——死海古卷之后的文本多元期。本文提出：教育是传承体系每次交接时作出的一次分派。含两次回溯检验与一处构念裂缝的公开订正。'),
    dict(md='教育是什么_先空着的那一处.md', slug='place-left-empty',
         title='先空着的那一处',
         sub='论教育由一批无人认领的空处构成，以及为什么把它标出来就等于开始填它',
         kw='无认空；认领时刻；构成通道；空处的四阶段；组织冗余；教育本体',
         desc='营养学、水力学与园林学共同假定：能不能得到是供给端的事。推翻它的材料来自营养学自己——铁调素的负反馈使吸收率成为接受者当前那个「空」的函数。本文提出：教育是在接受端维持一批尚未被任何人认领的空处，而认领这个动作本身就是填充的第一步。'),
    dict(md='设对__教育是什么.md', slug='the-set-counterpart',
         title='设对',
         sub='论教育是对设对的一次设置，及为什么有一类比较判断的另一侧根本不存在',
         kw='设对；虚对；设对清单；非同一性；参照点；教育本体',
         desc='历史学、正义学与体育学共同假定：一次教育的功过是一笔可以被分配完的量。推翻它的材料来自正义学自己——帕菲特非同一性问题。本文提出：教育是对设对的一次设置；设对指任何一次比较性判断被放在另一侧的那样东西，而它有时根本不存在。含九十五条现行游泳世界纪录的回溯检验。'),
]
