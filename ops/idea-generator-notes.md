...
## 界面结构纪律（2026-07-12 全面化改造后）

- 页面顶部信息架构：四卡完整管线（六篇→涌现五重检验→四篇双审→发生引擎冲150）+ 两个 `<details>` 折叠区（八大基底一览表 / 两次提智心得机制说明）。**新增基底时三处必须同步**：四卡底注的基底清单、八大基底一览表加一行、Key 申请链接行。
- 下拉框共 6 个（modelSel / fourRevA / fourRevB / reviewModelSelA / reviewModelSelB / writerModelSel），前者与后三者为 8 optgroup 结构（每家一组），fourRev 两个为"浏览器直连/经本站代理"两组。**新增基底必须同时加入全部 6 个下拉的正确分组**——历史教训：曾把 Kimi 塞在智谱组、把经代理的 MiniMax 塞进直连组。
- fourRevB 的 glm:pro 带 selected（审稿人 B 默认智谱），重建下拉时必须保留。
